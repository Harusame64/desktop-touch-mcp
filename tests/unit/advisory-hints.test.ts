 ...
 * Gate (ADR-022 dogfood): keyboard(type) + Edit/Document + hasValue + NOT a browser
...
const edit = (): PostElementInfo => ({ name: "Text Editor", type: "Edit", hasValue: true });
...
describe("maybeAdvisory — keyboard(type) → desktop_act", () => {
  ...
  it("emits for a Document control type too", () => {
    const hint = maybeAdvisory(
      ...
      { name: "Doc", type: "Document", hasValue: true },
      "winword",
    );
    expect(hint).not.toBeNull();
  });
...
describe("maybeAdvisory — unnamed text input (#352 follow-up, ADR-022 §5.5)", () => {
  ...
  it("fires for an unnamed Edit with hasValue:true (an empty value still counts as ValuePattern PRESENT)", () => {
    const hint = maybeAdvisory(
      "keyboard",
      { action: "type", windowTitle: "App", text: "x" },
      { name: "", type: "Edit", hasValue: true }, // name-empty editable, ValuePattern exposed
      NATIVE,
    );
    expect(hint).not.toBeNull();
    expect(hint!.preferredPath).toBe("desktop_act");
  ...
  it("does NOT fire for an unnamed Edit with hasValue:false (no ValuePattern)", () => {
...
describe("maybeAdvisory — suppression (no hint)", () => {
  it("returns null when the focused element is not a text input (UIA-blind / wrong control)", () => {
    expect(
      maybeAdvisory("keyboard", { action: "type", text: "x" }, { name: "Canvas", type: "Pane", hasValue: true }, NATIVE),
    ).toBeNull();
  });

  it("returns null when the focused element exposes no value (no ValuePattern)", () => {
    // An Edit with hasValue:false = UIA did not expose ValuePattern → suppress.
    expect(
      maybeAdvisory("keyboard", { action: "type", text: "x" }, { name: "Text Editor", type: "Edit", hasValue: false }, NATIVE),
    ).toBeNull();
  });
  ...
  it("returns null for a Chromium web-area root (Document + value=URL + RootWebArea automationId)", () => {
    ...
    expect(
      maybeAdvisory(
        "keyboard",
        { action: "type", text: "x" },
        { name: "ホーム / X", type: "Document", hasValue: true, automationId: "RootWebArea" },
        NATIVE,
      ),
    ).toBeNull();
  });
