...
describe("PROBE", () => {
  it("prints the envelope for unreadable/native and element_not_found", async () => {
    for (const [why, via] of [["unreadable", "native"], ["element_not_found", "powershell"], ["read_failed", "none"]] as const) {
      vi.mocked(getElementBounds).mockResolvedValue({ found: null, why, via } as Awaited<ReturnType<typeof getElementBounds>>);
      const result = await scopeElementHandler(ARGS);
      const text = (result.content as Array<{ type: string; text?: string }>).find((c) => c.type === "text")?.text ?? "{}";
      console.log("PROBE", why, via, text);
    }
  });
});
