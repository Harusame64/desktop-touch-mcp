...
describe("B-4-3: success_count >= 3 で suggest 候補", () => {
  it("3 回成功 + 0 失敗 + no destructive → suggest 1 件", () => {
    ...
    expect(result.successful_macros).toHaveLength(1);
    expect(result.successful_macros[0]?.tools).toEqual(tools);
    expect(result.successful_macros[0]?.success_count).toBe(3);
  });
  ...
  it("2 回成功のみ → suggest 対象外 (閾値未満)", () => {
    ...
    expect(result.successful_macros).toEqual([]);
  });
});
...
describe("B-4-8: makeQueryWrapper 経由 envelope.successful_macros inject", () => {
  it("3 回 record + query → envelope.successful_macros[0] 出る", async () => {
    ...
    expect(parsed?.successful_macros?.successful_macros).toHaveLength(1);
    expect(parsed?.successful_macros?.successful_macros?.[0]?.tools).toEqual(
      tools,
    );
    expect(
      parsed?.successful_macros?.successful_macros?.[0]?.success_count,
    ).toBe(3);
  });
});
