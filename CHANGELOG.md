...
| `keyboard` `type` / `press` in the background into Notepad after this server moved the focus to another window | Nothing was typed — the window's thread had no focused control, so the characters went nowhere (measured on the 2.0.0 release candidate before this fix; 1.16.0 has the same code) | `BackgroundTargetHasNoFocus` with `method: "background"` or `"foreground_flash"`, nothing sent; where `DTM_BG_AUTO=1` makes `"auto"` choose the background road, the foreground road is taken instead. A single-window app that takes keystrokes on its own window (other than a console) is refused the same way while its thread has no focus |
...
- **The diagnostic log now records how a terminal window relates to the server
  ...
  of processes it was launched from; then a `keyboard` or `terminal` write that
  resolves to a terminal window appends a `topology_relation` record saying who
  ...
  Reads are not recorded, so a long `terminal(action='run')` does not bury the
  writes under its own polling.
  ...
  it. Turn the whole log off with `DESKTOP_TOUCH_DIAGNOSTIC_LOG_DISABLE=1`.
...
- **`keyboard` write actions (`type` / `press` / `sequence`) now require a
  ...
  included. A `windowTitle` that is empty or only spaces counts as no target at
  all. A window that has no title can be addressed by `hwnd`, but only while it
