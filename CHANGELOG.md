...
## [1.4.2] - 2026-05-10 — ADR-013 Option E `foreground_flash` channel + F4-bis ValuePattern fallback hotfix + Phase 6 dogfood F4 re-open

epic ADR-013 Phase 4 stretch (issue #185) と Phase 6 dogfood F4 re-open + F4-bis hotfix を 1 patch release で消化。Option E は `method: 'foreground_flash'` 明示 opt-in 経路として Windows Terminal (WinUI/XAML pipeline、`CASCADIA_HOSTING_WINDOW_CLASS`) への BG 入力を妥協 BG path で復活、issue #173 silent-success 構造的回避を維持しつつ v1.3.2 PR #174 で外した WT 経路に新 channel として再到達手段を提供。F4-bis hotfix で v1.4.1 release 内 PR #233 の ValuePattern fallback dead path を解消、Win11 New Notepad の `keyboard:type method:'background'` が `delivered` を返却。**ADR-013 Status は Draft 維持** (Phase 2 R1 PASS + R1 mitigation docs 達成済、dogfood 1-2 週間期間後の patch release で user 判断 Accepted flip 予定)。Dogfood Pass は Option E (Phase 1f unit 14 pass + Phase 2 R1 100% PASS) + F4-bis (PR #235 19 unit case pin) の confidence で本 release では skip、post-release dogfood follow-up で補完予定。

### Added

- **`method: 'foreground_flash'` channel for `keyboard:type` / `terminal:send` (ADR-013 v1.4 / Option E、PR #240 + PR #241 + PR #242).** Windows Terminal (`CASCADIA_HOSTING_WINDOW_CLASS`) は WinUI/XAML pipeline で `WM_CHAR` を silent drop するため、issue #173 で `method: 'background'` 経路から WT を除外していた (v1.3.2、PR #174)。本 PR で **明示 opt-in な妥協 BG path** として `method: 'foreground_flash'` channel を追加、WT を含む WM_CHAR 不対応 window への BG injection を offer。
