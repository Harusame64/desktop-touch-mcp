...
/**
 * ADR-036 — `hwnd` is a decimal window handle as a string. When present the engine resolves the
 * window through `IUIAutomation::ElementFromHandle` and searches no titles, so a same-titled
 * sibling cannot answer instead. Hand-written here and regenerated by the build; the Rust side
 * is the authority (`src/uia/*.rs`).
 */
export declare function uiaGetElements(opts: { windowTitle: string; maxDepth?: number; maxElements?: number; fetchValues?: boolean; hwnd?: string }): Promise<NativeUiElementsResult>
...
export declare function uiaClickElement(opts: { windowTitle: string; name?: string; automationId?: string; controlType?: string; hwnd?: string }): Promise<NativeActionResult>
export declare function uiaSetValue(opts: { windowTitle: string; value: string; name?: string; automationId?: string; hwnd?: string }): Promise<NativeActionResult>
export declare function uiaInsertText(opts: { windowTitle: string; value: string; name?: string; automationId?: string; hwnd?: string }): Promise<NativeActionResult>
...
export declare function uiaGetTextViaTextPattern(opts: { windowTitle: string; timeoutMs: number; hwnd?: string }): Promise<string | null>
...
export declare function win32WindowAnswers(hwnd: bigint, timeoutMs: number): boolean | null
