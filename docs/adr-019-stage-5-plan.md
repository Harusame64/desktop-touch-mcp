...
| **`src/engine/dxgi-broker.ts`** (NEW in ADR-020 SR-4 PR-SR4-1) | `DirtyRectBroker` class + `BrokerSubscription` interface + `BROKER_CONSTANTS` SSOT for the 3-TTL state machine. ~740 lines. Holds exactly one native `DirtyRectSubscription` per `outputIndex` and fan-out multiplexes Stage 5 (polling consumer, PR-SR4-2) + vision-gpu (callback consumer, PR-SR4-3 carry-over). |
