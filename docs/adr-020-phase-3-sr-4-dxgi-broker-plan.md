...
§8.3 (a) は元々 dual-monitor 環境を想定したが、本 epic の dogfood host が **single-monitor 構成のみ利用可** であり (`docs/adr-019-stage-5-followups.md:32` "primary dogfood environment is single-monitor only" と整合)、user 諮問の結果 single-monitor + vision-gpu activation 同時実行で代替検証を行うことを **§8.3 (a) acceptance に含める**:
