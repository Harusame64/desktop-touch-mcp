...
- **dogfood pass observable criteria** (Round 2 P2-3、user judgment 依存軽減で §8.4 trial & error 2 回上限カウント明確化、**PR-SR4-4 Round 1 P1: §8.5 single-monitor 環境ルート受理を明文化**):
  - **(a) race-free 実証**: **dual-monitor or single-monitor + vision-gpu activation 同時実行** (§8.5 で受理) で `VisualMotionObservation.cacheState == "hit-subscription"` を **N=10 cycle 連続観測** (race-loss 軸消滅実証)
...
### 8.5 single-monitor dogfood 環境受理 rationale (PR-SR4-4 Round 1 P1 追加)

§8.3 (a) は元々 dual-monitor 環境を想定したが、本 epic の dogfood host が **single-monitor 構成のみ利用可** であり (`docs/adr-019-stage-5-followups.md` § "Test matrix (template)" maintainer note: "primary dogfood environment is single-monitor only" と整合)、user 諮問の結果 single-monitor + vision-gpu activation 同時実行で代替検証を行うことを **§8.3 (a) acceptance に含める**:

- **broker 北極星整合**: race-loss `NotCurrentlyAvailable` の根本原因は **同一 output index への concurrent DXGI `DirtyRectSubscription`**。single-monitor 構成下でも Stage 5 と vision-gpu が同時に `outputIndex: 0` を取得しようとする状況は dual-monitor 構成と同じ contention pattern を生み、broker subscription dedup が機能していれば `cacheState == "hit-subscription"` が観測される。よって race-loss 軸消滅実証 (PR-SR4-4 北極星 §8.1) は single-monitor でも十分検証可能。
- **dual-monitor 固有 scenario の deferral**: secondary monitor (`outputIndex >= 1`) 上での broker 動作 / cross-monitor straddle window 振る舞いは **本 PR scope 外**、`docs/adr-019-stage-5-followups.md:48-53` Dual-monitor scenarios (DEFERRED) と整合して dual-monitor 環境利用可能後の Stage 5c 計画で再検証。
- **acceptance 文言整合**: §8.3 (a) は本 §8.5 受理により "dual-monitor or single-monitor + vision-gpu activation 同時実行" と読む。dogfood report (`docs/adr-019-stage-5-followups.md`) には実環境 (single-monitor) を明記、§8.3 (a) で deferred な dual-monitor 検証は別 PR で carry-over。
