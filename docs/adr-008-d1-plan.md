...
- [x] **bench で TS 版より latency 1/10** — **steady-state lookup で達成** (ratio ~75,000×、view ~145ns vs TS p99 ~11.2ms)。「update latency 1/10 未達」表記は **PR-2 で SLO 文言正確化により closure** (Opus 諮問判断 2026-05-02、views-catalog §3.1 4 種分解): release-to-view は N3 partial-order contract により `shift_ms` 律速の構造的下限、production 観測 SLO は MCP round-trip で D2-B-4 6.22ms steady-state (with-point baseline 9.58ms 比 1.5× 既達)、TS / 10 acceptance は steady-state lookup で達成済。`docs/adr-008-d1-followups.md` §2.5 (Resolved) 参照。real L1 ring 込み bench は D2-B-4/5 で MCP transport 込み計測完了
...
**D1 phase 完了表記後の D2 carry-over 項目**: `docs/adr-008-d1-followups.md` に永続記録 (強制命令 9 — memory ではなく docs/ に書く)。**bench 範囲拡張 4 項目** (真の p99 / production gap / MCP transport bench / **real L1 ring → view 全経路 bench**) + **SLO 文言正確化** (PR-2 で views-catalog §3.1 4 種分解、Opus 諮問判断 2026-05-02 §5、option C 永久 defer) + **D1-3 残 follow-up 6 項目** (transient stale read / tie-breaker semantics / starvation 再評価 / reduce 経由 hwnd 死活 test / L4 envelope pivot 搬送 / shutdown drain 検証) = 計 11 項目。
