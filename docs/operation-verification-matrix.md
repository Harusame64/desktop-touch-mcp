...
| `ForegroundFlashUnsupported` | keyboard(action:`type`) / terminal(action:`send`) `foreground_flash` | ✅ Emitted (ADR-029 OQ8) | channel resolver が全 channel を拒否 (chromium / uwp_sandboxed / elevated_target 等、context.reason に理由)。`BackgroundInputUnsupported` の flash 版 — channel 軸が違うため別 code (SUGGESTS 1:1 mapping 維持) |
| `ForegroundFlashFailed` | keyboard(action:`type`) / terminal(action:`send`) `foreground_flash` | ✅ Emitted (ADR-029 OQ8 follow-up) | channel は取れたが flash paste sequence 自体が途中失敗 (foreground_steal_denied / focus_wait_timeout 等、context.reason に step)。Unsupported (pre-flight) と Failed (mid-flight) で recovery が違うため分離。classify arm は message tail の snake_case reason (`focus_wait_timeout` ⊃ "timeout") が generic UiaTimeout arm に poach されないよう **generic arm より前に配置** (SpawnFailed と同型) |
