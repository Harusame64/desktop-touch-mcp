...
| `ForegroundFlashUnsupported` | keyboard(action:`type`) / terminal(action:`send`) `foreground_flash` | ✅ Emitted (ADR-029 OQ8) | channel resolver が全 channel を拒否 (context.reason は resolver が実際に produce する 4 値 = chromium / uwp_sandboxed / class_unknown / no_supported_channel。`elevated_target` / `user_disabled_foreground_flash` は union に居るだけで**どの経路も emit しない** ため suggest / docs で例示しない — Round 3 P2-2)。`BackgroundInputUnsupported` の flash 版 — channel 軸が違うため別 code (SUGGESTS 1:1 mapping 維持) |
...
ADR-029 OQ8 追加 5 件 (`ForegroundFlashRequiresTarget` / `ForegroundFlashUnsupported` / `ForegroundFlashFailed` / `TabDragBlocked` / `CrossWindowDragBlocked`): 既存 code と同名・substring 包含なし (`foregroundflashunsupported` は `foregroundflashnotapplicableto*` の substring ではないため family 内 arm 順序は非依存)。ただし `ForegroundFlashFailed` のみ message tail に snake_case reason が付くため classify 内の**配置が order-dependent** (generic `timeout` arm より前、§5.2 該当行と `_errors.ts` classify コメント参照)。`tests/unit/oq8-failwith-suggest-routing.test.ts` が全件 pin。
