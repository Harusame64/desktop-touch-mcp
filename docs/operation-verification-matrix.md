...
| `terminal` (action:`send` `foreground_flash`) | Win32 SetClipboardData + AttachThreadInput / Alt unlock + SendInput Ctrl+V (+ optional Enter) + clipboard restore | **Implementation Land (ADR-013 v1.4 / Option E)** — `method:'foreground_flash'` 明示 opt-in path、`background` 契約とは分離した妥協 BG (foreground を 50-80ms 一時占有)、native `win32_foreground_flash_inject` で input 制限 (改行禁止 + UTF-16 < 5KiB) + 3 point clipboard sequence + paste warning ContentDialog scan + foreground restore ladder + verify | (本 PR で SSOT 規範) `hints.backgroundChannel:"clipboard_flash"` + `typingLeakRisk:true` + `flashDurationMs` + `foregroundStealMethod` + `foregroundRestored` + `foregroundRestoreMethod` + `clipboardRestored` + `clipboardSkippedFormats[]`。WT class への明示 opt-in 専用 (`canInjectViaPostMessage` は touch せず、`background` で WT は引き続き unsupported) | reason: `input_contains_newline` / `input_exceeds_paste_warning_threshold` / `foreground_steal_denied` / `focus_wait_timeout` / `clipboard_lock_contention` / `foreground_restore_failed` / `wt_paste_warning_intercepted` / `send_input_failed` (§4.3 reason enum 参照、本 PR で追加) | (本 PR、ADR-013 v1.4) |
...
| `keyboard` (action:`type` `foreground_flash`) | Win32 SetClipboardData + AttachThreadInput / Alt unlock + SendInput Ctrl+V + clipboard restore | **Implementation Land (ADR-013 v1.4 / Option E)** — terminal:send foreground_flash と同 channel、Enter は default 送らない (`pressEnter: false`)。それ以外の検証契約は terminal:send foreground_flash 行と同一。**`replaceAll: true` は wm_char fallback 経路では `BackgroundModifierComboUnsupported` で拒否（送らない。PostMessage の Ctrl は押下状態にならず "a" を打つため、v2.0.0 の dogfood で実測）、clipboard_flash 経路では `warnings: ["ReplaceAllNotSupportedOnClipboardFlash"]` を返す** (Round 2 P1-3、postKeyComboToHwnd は WT XAML pipeline で silent drop、native side `select_all_first` option は followups §2.3 carry-over) | (本 PR で SSOT 規範) `hints.backgroundChannel:"clipboard_flash"` + `typingLeakRisk:true` + `flashDurationMs` + `foregroundStealMethod` + `foregroundRestored` + `foregroundRestoreMethod` + `clipboardRestored` + `clipboardSkippedFormats[]` | reason: terminal:send foreground_flash 行と同 enum (本 PR で §4.3 に追加) | (本 PR、ADR-013 v1.4) |
| `keyboard` (action:`press` `foreground_flash`) | (rejected) | **Early reject** — key combo paste は semantics に合わない (Ctrl+V を combo paste で送る循環構造)。handler が即 `ForegroundFlashNotApplicableToKeyPress` で fail | (本 PR) suggest に keyboard:type / terminal:send への誘導 | code: `ForegroundFlashNotApplicableToKeyPress` (新規、§5.2) | (本 PR、ADR-013 v1.4) |
...
| `keyboard` (action:`press` BG) | PostMessage WM_KEYDOWN/UP | **None** — `postKeyComboToHwnd` の ack のみ。**Ctrl/Shift/Alt を含む combo は送らず** `BackgroundModifierComboUnsupported`（明示の BG）／FG へ回す（auto） | **Indirect** — combo の semantic 効果 (e.g. escape でダイアログが閉じる、pagedown でスクロール位置が動く) は target ごとに分岐するため `verifyDelivery: "unverifiable"` hint で degradation を明示。**例外**: enter / tab / arrow は terminal-class なら post-send UIA read-back で文字位置 / 行追加で検出可 | code: `BackgroundKeyNotDelivered` (新規、§5) | #177 |
...
| `keyboard` (action:`sequence` FG) | SendInput (atomic, `withKeyboardLock`) | **Strict (focus 不到達 → suppress)** — focus 不到達なら `ForegroundRestricted` で step 1 つも実行しない。step 0 以降は **hwnd ベース** `checkForegroundOnce` で各 step 直前に foreground 確認、奪われたら `MenuFocusLostMidSequence` + `context.remaining: Step[]` で abort（title rename mid-sequence にも堅牢）| **Indirect (規範)** — Alt メニュー mnemonic の semantic 効果（menu open）は直接観測不能なので全 step 完走後に `hints.verifyDelivery: {status:'focus_only', reason:'menu_state_not_observable', channel:'sendinput'}` を emit。**N=1 含む全 N で一律 `focus_only`** (#278 issue 規約 option (a)、`focus_only` は本 tool のシグネチャ — N=1 を `keyboard:press` の `delivered` / `unverifiable` 契約に派生させない: 1-step sequence は呼び側が `keyboard:press` で書き直せば press 契約を得られるため、sequence tool 内では observation 契約を一貫させる)。並行 `screenshot` / `desktop_state` は input lock を取らないため sequence 中に並行可能だが、mid-sequence で modifier が物理的に down のスナップショットを観測しうる旨を caller は認識する必要あり | code: `ForegroundRestricted` / `MenuFocusLostMidSequence` (本 PR #257); BG/foreground_flash は **schema-time reject** (typed `BackgroundNotApplicableToSequence` / `ForegroundFlashNotApplicableToSequence` は SUGGESTS に登録、defense-in-depth 用) | (本 PR、#257) |
...
| `scroll` (action:`smart`) | 多経路 (CDP / UIA / image) | **Indirect** — 各 strategy の成功条件を strategy 別に判定 (CDP: scrollIntoView 後の rect、UIA: ScrollItemPattern、image: perceptual hash diff) | (現状維持) | code: strategy 別既存 (`VirtualScrollExhausted` / `OverflowHiddenAncestor`) | — |
...
| `notification_show` | Win32 toast | **Unverifiable hint emit** (PR #217 / J1 land 後) — `hints.verifyDelivery: {status: "unverifiable", reason: "user_visible_side_effect_uninspectable", channel: "win32_balloon_tip"}` を successful response 全件に emit | **Unverifiable** (規範) — toast が user に reach したかは原理的に観測不能 (OS が consent UI を出すか sink するかは user の Focus Assist 設定依存)。`hints.verifyDelivery: {status: "unverifiable", reason: "user_visible_side_effect_uninspectable", channel: "win32_balloon_tip"}` を返す (channel 値は production NotifyIcon API surface 由来) | (unverifiable hint で表現、追加 code 不要) | PR #217 (J1) |
| `browser_click` | CDP `Runtime.evaluate` で click() dispatch | **Indirect** — element が viewport 内、click() 後の DOM ack | **Indirect (強化、hint で表現)** — click() 直前 / 直後で DOM mutation (例: `aria-pressed` toggle、URL 変化、window onload) のいずれかを確認 + 一定時間内に **何も変わらない** click は `unverifiable` hint。SPA ボタンが何も attach されていない silent-fail を catch。**観測経路 (規範)**: `Runtime.evaluate` で `MutationObserver(document.body, {subtree:true,childList:true,attributes:true})` を click 直前に install → click() dispatch → 500ms timeout で `anySignal = mutationCount > 0 \|\| urlChanged` を判定。`activeElementChanged` は `observedSignals` で報告するが **delivery 信号には含めない** (focusable control への click は handler 不在でも focus を更新するため、focus-only を `delivered` に流すと silent-fail regression を見逃す — browser.ts:1107-1114 で意図的に除外、PR #181 Codex P1 の出発点)。`anySignal` 0 件 → `unverifiable` (reason ∈ {`iframe_context_mismatch`, `no_dom_mutation`, `probe_install_failed`, `probe_read_failed`})。**`BrowserClickNotDelivered` typed code は §5.2 で予約のみ** — false-positive risk が高いため emit せず、degradation は `verifyDelivery: unverifiable` hint で表現 (browser.ts:1136 「we don't escalate to BrowserClickNotDelivered fail」と整合、SUGGESTS と classify() は登録済) | hint: `verifyDelivery.status` **2 値** (`delivered` / `unverifiable`) — CDP 経路では `focus_only` は semantic N/A (UIA 経路 mouse_click family でのみ 3 値、PR #219 J4 narrow); code: 予約のみ | #181 |
...
> **`foreground_flash` 系 8 値は「native の typed enum」であって `context.reason` の値域ではない**（§5.2 `ForegroundFlashFailed` 行 / Round 4 P2-4）。`ClipboardError::as_reason()` の 4 値（`clipboard_empty_failed` / `clipboard_alloc_failed` / `clipboard_set_data_failed` / `hidden_owner_create_failed`）が native message から透過し、未知の native error（panic 文言含む）も raw で載る（原文は `context.rawError`）。また **失敗 envelope に載るのは `reason` / `rawError` / `windowTitle` のみ** — `foregroundStealMethod` / `foregroundRestored` 等の hints は**成功時 result 専用**なので、失敗時の回復案内はそれらを前提にできない（Round 6 P2-1: `already_foreground` かどうかが失敗側から判別できないため、`focus_window` 案内は hedge してある）。
...
| `input_contains_newline` | `method:'foreground_flash'` の input pre-flight で改行 (LF / CR) 検出 | ADR-013 §3.3.1 構造的回避、WT `multiLinePasteWarning` を trigger させない。caller suggest = 改行除去 + 分割 inject。Opus PR #240 Round 1 P1-3 で size threshold と分離 |
| `input_exceeds_paste_warning_threshold` | `method:'foreground_flash'` の input pre-flight (UTF-16 >= 5KiB) で reject | ADR-013 §3.3.1 構造的回避、WT `largePasteWarning` を trigger させない。caller suggest = 分割 inject |
| `foreground_steal_denied` | `method:'foreground_flash'` ladder 段 1 (AttachThreadInput) + 段 2 (Alt unlock) 全 fail | Win11 foreground-stealing protection で両段拒否、本 channel が caller bypass を諦めたケース |
| `focus_wait_timeout` | `method:'foreground_flash'` で WT が 30ms 以内に focus を取れず | ladder で foreground 移動はしたが GUI thread が hwndFocus を確立せず |
| `clipboard_lock_contention` | `method:'foreground_flash'` の `OpenClipboard` retry 上限 (100ms / 10 retry) 超過 | 他 process が clipboard をロック中 |
| `foreground_restore_failed` | `method:'foreground_flash'` で restore ladder + verify が 2 retry 超 fail | original_fg への復帰が確認できず、user に foreground 残存 risk |
| `wt_paste_warning_intercepted` | `method:'foreground_flash'` で WT `Microsoft.UI.Xaml.Controls.ContentDialog` を UIA scan 検出 → Esc dismiss + fail | 構造的回避 (§3.3.1) が破られた fail-safe path、本来 trigger 確率 ~0 |
| `send_input_failed` | `method:'foreground_flash'` で `SendInput` が 0 件 inject (Win11 input restriction 等) | 物理 input simulation 拒否 |
...
| `ForegroundFlashRequiresTarget` | keyboard(action:`type` `foreground_flash`) | ✅ Emitted (ADR-029 OQ8) | `foreground_flash` は windowTitle/hwnd 必須 (flash 先が無い)。`NotDelivered` 系でなく pre-flight reject なので `Requires*` 命名。既存 `ForegroundFlashNotApplicableTo*` と family prefix 整合 |
| `ForegroundFlashUnsupported` | keyboard(action:`type`) / terminal(action:`send`) `foreground_flash` | ✅ Emitted (ADR-029 OQ8) | channel resolver が全 channel を拒否 (context.reason は flash 経路で実際に produce され得る 3 値 = chromium / uwp_sandboxed / class_unknown。両 flash call site が `allowedChannels: ["wm_char", "clipboard_flash"]` を渡すため `wt_xaml_pipeline` は channel 化され、拒否は必ずこの 3 値のいずれか。`no_supported_channel` は wm_char が allowed から除外されたときだけ返る (`background-channel-resolver.ts` step 1) が flash call site はどこも除外しないため emit されない — Codex round 6 / Round 7 P2-1。`elevated_target` / `user_disabled_foreground_flash` は union に居るだけで**どの経路も emit しない** ため suggest / docs で例示しない — Round 3 P2-2)。`BackgroundInputUnsupported` の flash 版 — channel 軸が違うため別 code (SUGGESTS 1:1 mapping 維持) |
| `ForegroundFlashFailed` | keyboard(action:`type`) / terminal(action:`send`) `foreground_flash` | ✅ Emitted (ADR-029 OQ8 follow-up) | channel は取れたが flash sequence が完走しなかった。**`context.reason` の値域は `ForegroundFlashErrorReason` の 8 値だけではない** (Round 4 P2-4): `ClipboardError::as_reason()` の 4 値 (`clipboard_empty_failed` / `clipboard_alloc_failed` / `clipboard_set_data_failed` / `hidden_owner_create_failed`) が native message からそのまま透過し、未知の native error（panic 文言含む）も `bg-input.ts` の `KNOWN_FLASH_REASONS` を通らず raw で載る（その場合は `context.rawError` が原文）。**recovery が同一でないので SUGGESTS は reason 別に書く** (Round 3 P1)。3 群: **未貼付** = validate reject 2 種 + clipboard 系 4 種 + `foreground_steal_denied` / `focus_wait_timeout` + addon 欠落 shortcut (`bg-input.ts`)。**「未貼付」≠「副作用ゼロ」**: step 4-10 は 1 つの IIFE なので step 5 (`focus_wait_timeout`) / step 6・8 (`send_input_failed`) の早期 return は末尾の **foreground restore を飛ばし、target が前面に残る**（IIFE 後に走るのは clipboard restore のみ）。`foreground_steal_denied` は steal 前なので前面は変わらないが、ladder の `alt_unlock` 試行で Alt down/up が旧前面窓に入り得る。**`focus_wait_timeout` と `send_input_failed` の 2 つだけ**が SUGGESTS で `focus_window` を案内するのはこのため（`foreground_steal_denied` は前面が変わらないので案内しない。`already_foreground` = 元から target が前面だった場合はどの reason でも戻す先が無いので、文言は hedge してある）、**貼り付け済み** = `foreground_restore_failed`（ただし `inner?` が `paste_warning_detected` 判定より先に伝播するため、intercept を**マスクし得る** → 断定せず target を読む）、**判別不能** = `send_input_failed`（step 6 の Ctrl+V 失敗 = 未貼付 と step 8 の Enter 失敗 = 貼付済み が同一 reason）。clipboard paste は原子的なので「truncate」する reason は存在しない。Unsupported (channel が無い) と Failed (channel はあった) で recovery が違うため分離。classify は先頭の declared-code arm が `<Code>:` prefix を detail 走査前に解決するため message tail の snake_case reason (`focus_wait_timeout` ⊃ "timeout") は poach されない — 専用 arm は wrapper prefix 付きで substring cascade に落ちる shape のための defense として **generic arm より前に配置** を維持 (SpawnFailed と同型)。**今日この wrapper shape を emit する producer は src に無い** (prefix 合成は `cdp-bridge.ts` の `CDP: ${inner}` のみで `CDP` は SUGGESTS key ではない) — defense-in-depth |
| `ForegroundFlashChannelNotImplemented` | keyboard(action:`type`) / terminal(action:`send`) `foreground_flash` | **🔒 Reserved (not emitted)** | flash 経路の channel narrow-reject (`keyboard.ts` / `terminal.ts`)。`wm_char` branch は必ず return し、`unsupported` は手前で return するため、この reject に到達するのは `kind === "cooperative_bridge"` の時だけ — そして resolver は `cooperative_bridge` を**構築しない**（`background-channel-resolver.ts` は型 union に持つだけで生成箇所ゼロ、ADR-013 Option F = Phase 3 MVP scope 外）。よって現状到達不能 (Round 7 P2-3)。SUGGESTS / classify に pre-register するのは上記 False-positive policy と同型（classify は専用 prose arm を持たず leading-code arm でのみ解決 — compact producer のため）。**CHANGELOG では announce しない**: user が遭遇し得ない code を「新しく返る」と書くのは誤り |
| `TabDragBlocked` | mouse_drag | ✅ Emitted (ADR-029 OQ8) | tab-strip 起点 drag の安全 gate (allowTabDrag:true opt-in で解除)。delivery 失敗でなく intent 確認 gate なので `Blocked` 命名 (`AutoGuardBlocked` と同系) |
| `CrossWindowDragBlocked` | mouse_drag | ✅ Emitted (ADR-029 OQ8) | start/end が別 window (or desktop) に跨る drag の安全 gate (allowCrossWindowDrag:true opt-in)。message prose は hwnd 数値のみ — 先頭 `<Code>:` prefix は classify の declared-code arm が detail 走査前に解決するため poach 免疫だが、**wrapper prefix 付きで substring cascade に落ちる shape では window title interpolate が generic "window not found" arm に poach され得る** ので title-free を維持。今日この wrapper shape を emit する producer は無く defense-in-depth (routing test が現行文字列と wrapper prefix 変種の両 shape を pin) |
...
**`_errors.ts` の `SUGGESTS` が SSOT** — 本節に entry 名を列挙しない。列挙は必ず腐り、しかも「列挙は散文より強く読まれる」ため腐った列挙は積極的に誤誘導する（Round 4 P3-7: 旧列挙は 35 件で止まっていたが実体は 80 件だった）。衝突チェックは列挙の目視ではなく **grep で行う**: 追加したい code 名で `Grep "<Code>" src/tools/_errors.ts` を叩き、① `SUGGESTS` に同名 entry が無いこと ② `classify` の既存 arm の判定文字列と**部分一致の包含関係**が無いこと（包含があると cascade 上流に poach される）を確認する。なお producer が `<Code>: detail` 形で emit する場合、先頭の declared-code arm（SUGGESTS 登録 key の exact-case prefix 一致）が substring cascade より先に解決するため、detail 側の keyword 衝突は leading form には及ばない — 包含チェックが効くのは bare form / wrapper prefix 付き shape。
...
ADR-029 OQ8 追加 6 件 (`ForegroundFlashRequiresTarget` / `ForegroundFlashUnsupported` / `ForegroundFlashFailed` / `ForegroundFlashChannelNotImplemented` / `TabDragBlocked` / `CrossWindowDragBlocked`): 既存 code と同名・substring 包含なし (`foregroundflashunsupported` は `foregroundflashnotapplicableto*` の substring ではないため family 内 arm 順序は非依存。`foregroundflashchannelnotimplemented` も family のどの判定文字列とも包含関係が無く、専用 prose arm を持たない — leading-code arm でのみ解決するため cascade 位置の制約も生じない)。ただし `ForegroundFlashFailed` のみ message tail に snake_case reason (`focus_wait_timeout` ⊃ "timeout") が付く。先頭 `<Code>:` 形は classify 先頭の declared-code arm が detail 走査前に解決するため poach されず、専用 arm の**配置が order-dependent** (generic `timeout` arm より前) なのは **substring cascade に落ちる shape — wrapper prefix 付き — に限る**。今日この shape を emit する producer は無く defense-in-depth (§5.2 該当行と `_errors.ts` classify コメント参照)。`tests/unit/oq8-failwith-suggest-routing.test.ts` が production の先頭形 (declared-code arm 経由の end-to-end) と wrapper prefix 変種 (producer 文字列を mouse.ts 等から導出した wording tripwire) の両方を pin。

**Cascade 順序そのものの検証 (Round 10 以降、Round 11 で firing set 単位に一般化)**: 上記の grep 目視に加え、`tests/unit/classify-cascade-order-invariant.test.ts` が classify の substring arm をソースから出現順に機械抽出する (Round 14 以降は TypeScript compiler API による AST 走査 — 整形に依存しない)。compound (`&&`) arm も skip せず、条件式を DNF 展開して **firing set**(全 token が message に揃ったとき arm が発火する token 集合)の列としてモデル化する — Round 10 の literal 単位検証の一般化。検証は毎回全数で: ① 各 firing set の合成メッセージが wrapper 付きで自 arm に到達すること (firing-set self-routing — 包含関係のある arm 順序違反はここで落ちる)、② 全 arm 順序対 × firing set 組合せの合成メッセージで先行 arm が勝つこと (抽出忠実性)、③ モデル水準の well-posedness 2 補題 — 各 firing set 自身のメッセージが自 arm に route する shadow lemma + pair メッセージが第三の arm を起こさない pair lemma (Round 10 の「包含 literal を持つ arm が被包含側より先」ルールを firing set に置き換えた一般形)、④ compound arm の firing-set shape (code → sets) の**完全 pin** — disjunct 追加・compound arm の新設/解体は必ずここで落ち、明示的な判断を強制する (Round 11)、⑤ wrapper-immune disjunct (`m === "disabled"` / `m.startsWith("guard failed")`) の**所有 arm exact pin + 素の message の end-to-end routing pin** — wrapper が defeat する disjunct はモデルの routing からは落とすが、「落とすものも owner + shape で pin する」一般則に従い、他 arm への graft / 移動 / 削除 / 重複は必ずここで落ちる (Round 13 — 到達可能な firing set を既に持つ arm に `|| m === "disabled"` を graft する変異が、全 rule green のまま production の "disabled" routing を変えた、Codex P2 実測)。抽出完全性は Round 14 で「region 内 regex 5 カウント一致」から **AST 走査の全 statement モデル化**に置換 (Codex P2 実測 — 行頭に `if` / `return` が来ない `} else if (…) return classify(…);` 形の tail 単文 arm が arm regex と 5 カウント全てをすり抜け 8/8 green だった): classify() 本体の全 statement を parser が列挙し、frame 4 種 (`const declared` + declared-code guard arm / `const m = message.toLowerCase()` / `const leadingCode` + leading-code guard arm / ToolError fallback return) は位置と shape で exact pin、cascade 内は if / else-if chain の arm のみ許可、想定外の statement 種・else 分岐・「単一の `return { code: "…" }` object literal」以外の arm body・モデル外の条件式 (`m.startsWith` の別引数を含む) はすべて named bail する。これにより Round 12 の既知限界「region 外に arm を足す形は原理的に不可視」は classify() 本体については解消 — 残る trust anchor は `_errors.ts` の `classify` FunctionDeclaration そのもので、classify の外 (failWith / errorFromMessage 等) で行われる routing は引き続き本 test の視野外 (oq8 end-to-end 側の領分)。抽出は lazy (memoized) 化され先頭の専用 named test が最初に起動するため、bail は file collection error ではなく原因 assertion 付きの test failure として報告される (Round 13 — Round 12 時点は module scope 実行で "no tests" 表示だった)。包含クラスの順序 pin は手書き不要。**非包含の順序 intent** (例: `SpawnFailed` arm を generic `window not found` arm より先に置く) はソースから導出不能なため、手書き intent anchor **4 本**が引き続き担う: `SpawnFailed` = `phase7-f3-spawn-failed-typed-code.test.ts` case #6 / `CoordinateOutsideReachableBounds` と `CursorPlacementBlocked` = `reachable-bounds.test.ts` の両 "wins over the generic classify arms" loop / `ForegroundFlashFailed` = `oq8-failwith-suggest-routing.test.ts` の "keeps the timeout-bearing flash reason out of the generic UiaTimeout arm" (wrapper prefix 付き `CDP: ForegroundFlashFailed: focus_wait_timeout` で "timeout" 競合を実搬送)。
...
なお SUGGESTS に key を登録した時点で、`<Code>: detail` / bare `<Code>` の 2 形は classify の declared-code arm（先頭）/ leading-code arm（末尾）が code ごとの専用 arm 無しでも解決する。手順 2 の substring arm は wrapper prefix 付きで message 中程に code が現れる shape のための defense-in-depth として引き続き追加する。
