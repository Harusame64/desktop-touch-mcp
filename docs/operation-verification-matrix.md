...
| `keyboard` (action:`type` FG) | SendInput | **Indirect** — `detectFocusLoss` で focus 維持確認 | (現状維持) FG SendInput は focus が取れていれば届く前提が成立 | warning: `ForegroundNotTransferred` (現行) | — |
...
| `keyboard` (action:`press` FG) | SendInput VK_* | **Indirect** — `detectFocusLoss` | (現状維持) | warning: `ForegroundNotTransferred` (現行) | — |
| `mouse_click` | SendInput MOUSEEVENTF_LEFTDOWN/UP | **Indirect (限定)** — `detectFocusLoss` で foreground 変化のみ確認、click が届いたかと focus 維持の区別なし | **Indirect (強化)** — pre-click element under cursor (UIA `ElementFromPoint`) と post-click foregroundWindow / focusedElement / scrollPos の diff で「click が何かに届いた」を確認。focus 維持は click 失敗の sufficient condition ではない (受け側が focus 拒否する case あり) ので別観点で `verifyDelivery` 値を 3 値化: `delivered` / `focus_only` / `unverifiable` | code: `MouseClickNotDelivered` (新規、§5) | #178 |
| `mouse_drag` | SendInput sequence | **None** — drag 完了後の状態確認なし | **Indirect** — drag start/end 周囲の `ElementFromPoint` diff、または target window scroll/move の鏡映観測。tab-drag は別 heuristic (既存 `detectTabDragRisk`) が前段で gate 済なので drag 自体の delivery 検証はその下流 | code: `MouseDragNotDelivered` (新規、§5、click と分離: drag 固有 failure mode = sequence 中断 / modifier-key state / mid-drag release で suggest が異なる) | #178 (子のサブ) |
...
| `focus_window` | SetForegroundWindow + AttachThreadInput | **Indirect** — post enum で `isActive` 確認、SW_RESTORE 後の rect 取得 | (現状で SSOT 候補) — Windows foreground-stealing protection に対しては既に `ForceFocusRefused` warning で degradation 表現済 | code: `WindowNotFound` または warning: `ForceFocusRefused` (現状維持) | — |
...
合計: **17 commit 軸 tool / 28 行**(複数 action を持つ tool は action 別に行を立てる)。うち **9 行が現時点で verification 不足** で child issue の対象: keyboard(type BG / press BG)、`mouse_click`、`mouse_drag`、scroll(raw)、clipboard(write)、`notification_show` (規範 unverifiable に変更)、`browser_click`、`browser_fill`。残り 19 行は SSOT 候補 = 現状実装を規範とする。`terminal` 各 action は v1.3.2 (PR #174) で完了済のため対象外。
...
| `MouseClickNotDelivered` | mouse_click | issue #176 body の表に既出。`Background…` 系列とは channel が SendInput で別、SUGGESTS は click 固有 (UIA InvokePattern fallback、focus 維持確認 vs click 配信の区別、desktop_act 経路) |
| `MouseDragNotDelivered` | mouse_drag | drag は click sequence の特殊形だが failure mode が click とは質的に違う: sequence 中断 (途中で release が抜ける)、modifier-key state (drag 中に Shift/Ctrl が外れる)、tab-drag 検出 (`detectTabDragRisk` 後段)、dragdrop API 経路 (DROPEFFECT 取得失敗) など。`MouseClickNotDelivered` と suggest が異なるので分離 (SUGGESTS dictionary の 1:1 mapping を保つ) |
