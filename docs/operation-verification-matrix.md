...
| `ForegroundFlashFailed` | keyboard(action:`type`) / terminal(action:`send`) `foreground_flash` | ✅ Emitted (ADR-029 OQ8 follow-up) | channel は取れたが flash sequence が完走しなかった (context.reason に step。native の 8 値 = `ForegroundFlashErrorReason`)。**8 値は recovery が同一でないので SUGGESTS は reason 別に書く** (Round 3 P1): `input_contains_newline` / `input_exceeds_paste_warning_threshold` は inject 前の validate reject で**何も送っていない**ためリトライは同じ結果、`foreground_restore_failed` は**貼り付け済み**で復元だけ失敗（再送は二重入力）、`send_input_failed` は途中まで届いた可能性あり、残り 3 つ (`foreground_steal_denied` / `focus_wait_timeout` / `clipboard_lock_contention`) が transient。Unsupported (channel が無い) と Failed (channel はあった) で recovery が違うため分離。classify arm は message tail の snake_case reason (`focus_wait_timeout` ⊃ "timeout") が generic UiaTimeout arm に poach されないよう **generic arm より前に配置** (SpawnFailed と同型) |
...
`_errors.ts:SUGGESTS` 現行 entry (確認済、Phase 6 cleanup 後 = PR-A -3 + PR-B +1): `InvalidArgs` / `WindowNotFound` / `ElementNotFound` / `InvokePatternNotSupported` / `BlockedKeyCombo` / `UiaTimeout` / `ElementDisabled` / `BrowserNotConnected` / `TerminalWindowNotFound` / `TerminalTextPatternUnavailable` / `BrowserSearchNoResults` / `BrowserSearchTimeout` / `ScopeNotFound` / `WaitTimeout` / `ScrollbarUnavailable` / `OverflowHiddenAncestor` / `VirtualScrollExhausted` / `GuardFailed` / `AutoGuardBlocked` / `LensNotFound` / `BackgroundInputUnsupported` / `BackgroundInputIncomplete` / `BackgroundInputNotDelivered` / `SetValueAllChannelsFailed` / `WorkingMemoryNUpperBoundExceeded` / `EpisodicMemoryNUpperBoundExceeded` / `SemanticMemoryKUpperBoundExceeded` / `ProceduralMemoryKUpperBoundExceeded` / **`CoordinateOutsideReachableBounds` / `CursorPlacementBlocked`（ADR-029 Phase 1 / 2a）** / **`ForegroundFlashRequiresTarget` / `ForegroundFlashUnsupported` / `ForegroundFlashFailed` / `TabDragBlocked` / `CrossWindowDragBlocked`（ADR-029 OQ8）**

> 上の列挙は SSOT ではなく**確認時点のスナップショット**（列挙は下の散文より強く読まれるため、追加時はここも更新する — Round 3 P3-8）。実体は `_errors.ts` の `SUGGESTS` 定義。
