...
**Cascade 順序そのものの検証 (Round 10 以降)**: 上記の grep 目視に加え、`tests/unit/classify-cascade-order-invariant.test.ts` が classify の substring arm をソースから出現順に機械抽出し、① 各判定文字列が wrapper 付きで自 arm に到達すること (literal self-routing — 包含関係のある arm 順序違反はここで落ちる)、② 全 arm 順序対の合成メッセージで先行 arm が勝つこと (抽出忠実性)、③ 包含 literal を持つ arm が被包含側より先に居ること、を毎回全数検証する。包含クラスの順序 pin は手書き不要。**非包含の順序 intent** (例: `SpawnFailed` arm を generic `window not found` arm より先に置く) はソースから導出不能なため、手書き intent anchor (`phase7-f3-spawn-failed-typed-code.test.ts` case #6 / `reachable-bounds.test.ts` の両 ordering loop) が引き続き担う。
