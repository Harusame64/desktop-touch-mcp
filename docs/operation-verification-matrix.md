...
| `CrossWindowDragBlocked` | mouse_drag | ✅ Emitted (ADR-029 OQ8) | start/end が別 window (or desktop) に跨る drag の安全 gate (allowCrossWindowDrag:true opt-in)。message prose は hwnd 数値のみ — **window title を将来 interpolate すると classify の generic "window not found" arm に poach され得る** (routing test が現行文字列を pin) |
