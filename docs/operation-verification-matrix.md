...
| `keyboard` (action:`type` `foreground_flash`) | Win32 SetClipboardData + AttachThreadInput / Alt unlock + SendInput Ctrl+V + clipboard restore | **Implementation Land (ADR-013 v1.4 / Option E)** — terminal:send foreground_flash と同 channel、Enter は default 送らない (`pressEnter: false`)。それ以外の検証契約は terminal:send foreground_flash 行と同一 | (本 PR で SSOT 規範) `hints.backgroundChannel:"clipboard_flash"` + `typingLeakRisk:true` + `flashDurationMs` + `foregroundStealMethod` + `foregroundRestored` + `clipboardRestored` + `clipboardSkippedFormats[]` | reason: terminal:send foreground_flash 行と同 enum (本 PR で §4.3 に追加) | (本 PR、ADR-013 v1.4) |
