...
- [ ] 親 plan §11.1 「主要 view 4」項目 (本 D2-C plan PR で `dirty_rects_aggregate` 復帰反映済) で `dirty_rects_aggregate` 行を 🚧 → ✅ status へ flip (本 impl PR で実装完了)
...
- [ ] **主要 view 4 の declarative 実装 (D2-C0 ゲート carry-over → P5c-2 PR #102 trigger 完了で `dirty_rects_aggregate` 復帰、本 D2-C plan PR で sub-plan land、本 impl PR で実装)** の `dirty_rects_aggregate` 行を 🚧 → ✅ に flip
...
- 本 PR 完了で **ADR-008 §4 D2 「主要 view 4」の dirty_rects_aggregate 完成** (P5c-2 PR #102 merge で carry-over 復帰、本 plan PR で sub-plan land、impl PR で実装)、§3.bis ledger L1 strikethrough は本 plan PR で完了済
