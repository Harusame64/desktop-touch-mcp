...
| 3 | napi export が TS 側から呼べる | **compile-time guard** (`check:native-types` + `index.d.ts` 自動生成 + tsc build pass、§3.10 push 6 ガード)。runtime smoke (vitest live) は §1.3 carry-over (P5c-2 sub-plan §5 follow-up と同 phase) |
| 4 | `monitor_index` field を含む view output が TS 側から observable | **compile-time guard** (`NativeDirtyRectsResult` 型に `monitor_index: u32` field 必須、`check:native-types` で TS SoT 同期確認) + **Test G2-2** (per-monitor isolation、Rust 側で `monitor_index` field の意味的分離を pin) |
...
**検証粒度の判断**: G2 #3 / #4 の "TS 側から呼べる / observable" を runtime smoke (vitest live integration) で踏むのは **trunk scope 外** (本 PR は Rust mock-based のみ)。compile-time guard (`check:native-types` + napi binding compile pass) で API contract regression 防止は十分、runtime smoke は walking-skeleton trunk 完了後の expansion phase で carry-over。理由: trunk 段階で vitest live integration を要求すると Notepad/Edge fixture-based test が必要となり (P5c-2 sub-plan §5 follow-up と同 carry-over)、200-300 line trunk size 制約と緊張する。
...
### 3.8 D2-C-7: G2 contract minimal Rust integration test (~50 line) [S2 trunk]

mock L1Sink-based、no DXGI 必要。**G2 contract 検証に必要な最小 test**:

- [ ] **Test G2-1: per-frame count aggregation**: `(monitor=0, frame=1)` で 3 rects push → `view.get(0, 1)` で `Some(3)` を assert
- [ ] **Test G2-2: per-monitor isolation** (CLAUDE.md §3.2 PR #102 教訓): `(monitor=0, frame=1)` 2 rects + `(monitor=1, frame=1)` 3 rects → `view.get(0, 1) = Some(2)`、`view.get(1, 1) = Some(3)` (frame_index 衝突しても monitor で分離)
- [ ] **Test G2-3: focus view + dirty rects view 同 scope 共存** (G2 contract 必須): S1 で確立した同 scope に両 view を build、focus event push と dirty rect event push を交互に発行、両 view が独立に正しく更新される

[expansion] additional tests (per-frame aggregation 詳細 / out-of-order partial-order / 100ms eviction / recent_n / recent_window) は trunk 完了後の expansion phase で追加。
...
|---|---|---|
| **D2-C (本 PR、merged sub-batch)** | 3.1 ledger 消化 + 3.2 trait/Cmd 拡張 + 3.3 view module (count-only) + 3.4 spawn 拡張 + 3.5 pump + 3.6 pipeline + 3.7 napi (count getter) + 3.8 G2 contract test 3 件 + 3.9 bench (regression guard) + 3.10 ガード | **200-300 line** (count-only 簡素化で当初想定 300-450 → 200-300 に縮小、trunk contract spike として最小) |
...
- **D2-E0 dataflow scope refactor の追加形 refactor** (`build_dirty_rects_aggregate(scope, stream) -> (Arranged, View)` 形を採用、D1 `current_focused_element` も同 signature に揃える refactor は別 PR D2-E0 PR-η)
- **`recent_window(N_ms)` の time-travel API**: D3 で arrangement の time slice 機能と一緒に提供
...
### 8.1 G2 contract (`docs/walking-skeleton-trunk-selection.md` §4 S2 完了基準、§0 検証戦略 matrix と整合)
...
- [ ] **G2 #3 napi export が TS 側から呼べる** — **compile-time guard** で踏む (§3.10 `check:native-types` + `index.d.ts` 自動生成 + `npm run build` pass)、runtime smoke は §1.3 carry-over (vitest live integration、P5c-2 sub-plan §5 follow-up と同 phase)
- [ ] **G2 #4 `monitor_index` field を含む view output が TS 側から observable** — **compile-time guard** (`check:native-types` で `NativeDirtyRectsResult.monitor_index: u32` 強制) + Rust **Test G2-2** (§3.8 per-monitor isolation、CLAUDE.md §3.2 PR #102 教訓)
...
- [ ] D2-C-7 G2 contract test 3 件全 pass (per-frame count / per-monitor isolation / focus + dirty 共存)
