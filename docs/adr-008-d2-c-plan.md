...
**G2 ゲートの目標 (`docs/walking-skeleton-trunk-selection.md` §4 S2 完了基準より)**:
1. DXGI emit (#102 で landed) を入力に view が更新される integration test
2. D2-E0 同 scope (S1 で確立) で focus view と dirty rects view が共存
3. napi export が TS 側から呼べる
4. `monitor_index` field を含む view output が TS 側から observable
5. **G2 ゲート判定**: focus view + dirty rect view の共存が確認できた時点で、S3-S5 に進む価値と scope を再確認
...
- [x] `docs/adr-008-d2-plan.md` §3.bis ledger L1 row を strikethrough + Resolved 化 (本 D2-C plan PR で実施済、commit `72b0f7f` / `47d7be4`)
- [x] 親 plan §5 spec を「`hwnd` drop + `monitor_index` 追加 + `frame_id → frame_index` 命名統一」に reconcile (実施済)
- [x] 親 plan §11.1 「主要 view 4」項目で `dirty_rects_aggregate` 行を 🚧 → ✅ status へ flip (本 impl PR で実装完了、本 plan PR で復帰反映済)
...
- [ ] criterion bench 1 本: `view_get_hit` (count return latency) — regression guard 用途
...
### 8.1 G2 contract (`docs/walking-skeleton-trunk-selection.md` §4 S2 完了基準)

- [ ] **DXGI emit (#102 で landed) を入力に view が更新される** integration test (Test G2-1)
- [ ] **D2-E0 同 scope (S1 で確立) で focus view と dirty rects view が共存** (Test G2-3)
- [ ] **napi export が TS 側から呼べる** (`view_get_dirty_rects(monitor_index)` 経由)
- [ ] **`monitor_index` field を含む view output が TS 側から observable** (CLAUDE.md §3.2 PR #102 教訓、Test G2-2 で per-monitor isolation 確認)
