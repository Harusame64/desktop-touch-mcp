...
| typed reason enum | 1 種類 (`LeaseExpired` 第一候補) | 残り 49 codes |
...
| ADR-010 P2 typed reason 拡充 | 残 49 codes の `_errors.ts` SUGGESTS 連動 (うち lease 経路 4 codes は trunk pattern コピー) | ✓ 並走可 (code ごと独立) | 4-5 |
...
- **trunk 直系で Sonnet を遊ばせず、別 Sonnet session を並走で動かす**: trunk 中の Sonnet 並走は trunk PR を触らず、以下の **trunk 完了直後に消費される事前作業** を進める:
  ...
  - typed reason 残 49 codes の `_errors.ts` SUGGESTS 事前マッピング
...
| G3 | 2026-05-01 (S4 impl PR open、merge 時に commit hash 追記) | **継続** | commit 軸 wrapper (`makeCommitWrapper`) + query 軸 wrapper (`makeQueryWrapper`) で `desktop_discover` / `desktop_act` の挙動を破壊せず envelope 化、lease 4-tuple validation `expired → LeaseExpired` typed reason が 1 path 完全動作 (try_next: `desktop_discover` 1 path、ADR-010 §5.4)、L1 既存 `ToolCallStarted (=100)` / `ToolCallCompleted (=101)` payload schema 拡張 (新規 EventKind 追加なし、既存 `args_json` field 名不変、`lease_token: Option<LeaseTokenSummary>` 末尾 optional 追加)、failure envelope size 216B (LeaseExpired) / 152B (lease residual + handler threw) で < 5KB SLO 維持 (S3 bench harness で計測)、既存 4-arg napi callers (`l1-capture-panic-fuzz` + `l1-capture` 計 6 callsite) 無修正 pass。S5 caused_by linkage で ToolCall event を session 内 history buffer 経由展開する base 確立、tool_call_id seq の per-session 採番 (`${sessionId}:${seq}`) も pin 済 | (なし、S5 は in-memory buffer + 直近 N 件構成で着手予定、shrink 不要) |
| G5 | 2026-05-01 (S5 impl PR open、merge 時に commit hash 追記) | **継続** | per-session history buffer (TS `Map<sessionId, ring 8 件 cap>` + LRU eviction 1k entry / TTL 24h) + causal window 境界 (左端 ToolCallStarted event_id / 右端 (a) latestEventId frontier check + (b) monotonic timeout 200ms `performance.now()` 由来) + buildCausedBy projection (4 field: `your_last_action, tool_call_id, elapsed_ms, produced_changes`) + buildBasedOn projection (envelope トップレベル、`events: string[]` u64 decimal で JSON.stringify TypeError 完全回避、`sources` UIA/DXGI 由来動的 build) + buildProducedChanges (focus delta + dirty_rect per-monitor count、`monitor_index` 維持) で `desktop_act` → `desktop_state(include=causal)` シーケンス integration test 18/18 pass。Causal envelope size: causal_minimal 668B / causal_typical 818B / causal_max 1390B で < 2KB SLO 維持 (S3 bench harness 拡張、ADR-010 §5.6.1 causal include +1KB)、既存 raw client 互換 default opt-out で破壊なし (`include=causal` 未指定時 caused_by/based_on field 不在)。`desktop_state` wiring は `src/tools/desktop-state.ts` の module-scope `desktopStateRegistrationHandler` に `makeQueryWrapper(causedByProjector, getSessionId)` で wrap、`run_macro` 経路 (`TOOL_REGISTRY.desktop_state`) も同 instance 共有 (PR #112 shared registration handler pattern)、handler internal logic + Zod schema + 戻り値 shape 不変 (ADR-010 §1.5 SSOT)。sentinel guard `"multi:disabled"` runtime path closed loop で cross-session leak 完全回避 (Round 3 P1 Opus + Codex 重複 fix)。OQ #5 既存 `l1GetCaptureStats().eventIdHighWater` reuse で新 napi binding 追加なし。S6 trunk 完了判定 (expansion tool 1 件追加が L5 wrapper 修正のみ + CI assert 化) に進める contract base 確立 | (なし、S6 expansion plan 起草に進む) |
| G6 | 2026-05-01 (S6 impl PR open、merge 時に commit hash 追記) | **完了** | walking skeleton trunk completion: click_element lease 不在 commit wrap PoC (`src/tools/ui-elements.ts` の `clickElementRegistrationHandler` module-scope export、`run_macro` 経路 `TOOL_REGISTRY.click_element` 同 instance 共有、PR #112 shared registration handler pattern 維持) + `.github/workflows/expansion-pr-guard.yml` 新設 (PR title/label `expansion` 検出時 trunk lock layer 改変ゼロ enforce) + `scripts/check-expansion-disjoint.mjs` ローカル equivalent (`npm run check:expansion-disjoint`) + `docs/walking-skeleton-expansion-plan.md` 新規起草 (7 swimlane × worktree 並走戦略 + 30 分タイムアタック template + merge conflict 防止) + ADR-008 D2-G 部分着手 (trunk 確定 5 view = current_focused_element / latest_focus / dirty_rects_aggregate / focus_pump pipeline / lease_store の status update)。**TRUNK_LOCK_PATHS 4 path** (bit-equal sync、Round 2 P1-1 fix で実 repo 構造と整合): `crates/engine-perception/` + `src/l1_capture/` + `src/l3_bridge/` + `src/engine/perception/`。trunk 完了判定 = expansion tool 追加が L5 wrapper 修正のみで完了することの仕組み的強制を CI + local 2 重 pin、worktree 並走 expansion phase 着手の根拠確定。click_element wrap PoC contract test 4/4 pass、L5 wrapper のみで `caused_by.your_last_action = "click_element(...)"` 動作確認 | (なし、expansion phase 着手) |
