...
[全 OS / DirectML/WinML で動かない / op 未対応モデル]
  Layer 3: Vulkan compute (vendor-neutral, AMD/NVIDIA/Intel 共通)
    - ncnn (high-level, ONNX importer 経由)        ← Florence-2 等 op coverage が良い
    - Kompute (low-level, 自前 op kernel が必要)   ← Phase 4c で R&D 用
