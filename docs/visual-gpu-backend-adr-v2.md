...
#### 4b-5a — Florence-2 Stage 1 real inference (sub-batch 1/5 〜 5/5)
...
- [x] **4b-5a-3**: Florence-2 Stage 1 multi-session loading + encoder forward — `Florence2Stage1Sessions` struct (4 sub-session Arc bundle)、`EncoderOutputs` struct (encoder_hidden_states Array3<f32> + encoder_attention_mask Array2<i64>)、`encoder_forward` method (vision_encoder → embed_tokens → encoder_model 直列 pipeline)、`run_vision_encoder` / `run_embed_tokens` / `run_encoder_model` helper 3 関数、`init_florence2_stage1_blocking` (4 sibling ONNX files を composite key でpool insert)、`init_session_blocking` に `florence-2-base:` prefix 分岐追加、`stub_recognise_with_session` に encoder forward 3-step wiring (sub-sessions 揃う時)、`image` / `ndarray` / `tokenizers` を `optional = true` + `vision-gpu` feature gate に refactor (R1)、`resize_bilinear_rgb` の `expect()` 4 箇所を `Result` 経路に格上げ (4b-5a-1 R1)、`encoder_tests` mod 5 ケース追加。cargo check 3 feature set 全 exit 0、tsc noEmit exit 0、vitest 4 file regression 0。詳細は `docs/phase4b-5a-3-florence2-encoder-design.md`
- [x] **4b-5a-4**: Florence-2 decoder + KV cache + autoregressive loop — `FLORENCE2_NUM_LAYERS/NUM_HEADS/HEAD_DIM/VOCAB_SIZE/DECODER_START_TOKEN/EOS_TOKEN/DEFAULT_MAX_LENGTH` 定数、`LayerKvCache` / `KvCache` struct (24 tensors、`empty()` / `num_layers()` / `tensor_count()` method)、`DecoderStepOutput` struct、`decoder_forward` method (Vec<(String, DynValue)> による 28+ dynamic inputs、bool tensor `use_cache_branch`、`extract_kv_output` helper)、`generate_tokens` method (greedy argmax autoregressive loop)、`greedy_argmax` helper、`stub_recognise_with_session` への generate_tokens 接続、4b-5a-3 RECOMMEND R1 lite path 削除、4b-5a-3 NIT N1 fix (`format!("{outputs:?}")` パターン)、`decoder_tests` mod 8 ケース追加。cargo check 3 feature set 全 exit 0、tsc noEmit exit 0、vitest 4 file regression 0 (e2e 2 件は pre-existing)。詳細は `docs/phase4b-5a-4-florence2-decoder-design.md`
- [x] **4b-5a-5**: `<loc_X>` token parse → bbox RawCandidate[] — **Florence-2 Stage 1 完結** — `Florence2Tokenizer::decode(&[i64])` method 追加 (tokenizers `skip_special_tokens=false` で `<loc_X>` 保持)、`extract_loc_tokens(text)` 手書き parser (no regex dep)、`parse_region_proposal_output(text, image_w, image_h) → Vec<RawCandidate>` (quantized 0-999 → pixel 座標復元、degenerate bbox skip、class="region" label="" provisional=true confidence=0.5)、`florence2_stage1_recognise(req, sess) → Result<Vec<RawCandidate>>` 全 5-stage pipeline entry point、`stub_recognise_with_session` を session_key dispatch のみに簡素化 (全 pipeline を florence2_stage1_recognise に集約)、`tokenizer_path_for_session` を `pub(crate)` に格上げ、`tracing` crate 追加 (optional、vision-gpu feature gate) + 全 `eprintln!` → `tracing::warn!(target: "florence2", ...)` 置換 (R3 完全対応)、`parse_tests` mod 11 ケース追加。cargo check 3 feature set 全 exit 0、tsc noEmit exit 0、vitest 4 file regression 0 (e2e 2 件は pre-existing)。詳細は `docs/phase4b-5a-5-florence2-parse-design.md`

#### 4b-5b — OmniParser-v2 Stage 2 real inference (UI element detector) — **Stage 2 完結**

- [x] **4b-5b**: OmniParser-v2 icon_detect (YOLO11-based) single-pass forward — **Stage 2 完結** — `omniparser.rs` 新規 (`preprocess_image`: RGBA→f32 NCHW [1,3,1280,1280]、[0,1] normalize 無 ImageNet shift、`omniparser_stage2_recognise`: single ort::Session::run → YOLO output decode + NMS → `RawCandidate[]` class="ui_element"、`decode_yolo_output`: YOLOv8/v11 `[1, 4+num_classes, num_anchors]` shape decode + cxcywh→xyxy ROI 座標復元、`non_max_suppression`: greedy per-class NMS、`iou`: intersection-over-union)、`mod.rs` に `pub mod omniparser;` 追加、`inference.rs::stub_recognise_with_session` に `omniparser-v2-icon-detect:` prefix dispatch 追加 (`tracing::warn!(target: "omniparser", ...)`)。cargo check 3 feature set 全 exit 0、tsc noEmit exit 0、vitest 4 file regression 0 (e2e 1 件は pre-existing context-consistency フレーキー)。`omniparser.rs::tests` 9 ケース (preprocess_output_shape / preprocess_unit_range / decode_yolo_returns_empty_on_wrong_shape / decode_yolo_filters_below_confidence / decode_yolo_emits_candidate_when_above_threshold / decode_yolo_scales_to_roi_dimensions / iou_full_overlap_is_1 / iou_no_overlap_is_0 / nms_drops_lower_confidence_overlap)。詳細は `docs/phase4b-5b-omniparser-design.md`

- [x] **4b-5c**: PaddleOCR-v4 Stage 3 text recognition + 4b-5 post-review B1 legacy 除去 — **Stage 3 完結 + 4b-5 シリーズ完結** — `paddleocr.rs` 新規 (`PaddleOcrDict::from_file`: ppocr_keys.txt ロード / `preprocess_image`: RGBA→f32 NCHW [1,3,48,W] aspect-preserving dynamic width (W∈[32,640])、PaddleOCR normalize (px/127.5-1) / `paddleocr_stage3_recognise`: dict load + per-ROI preprocess + `run_rec` (ort input="x") + `ctc_greedy_decode` → `RawCandidate[]` class="text" / `ctc_greedy_decode`: argmax-collapse-blank CTC greedy decoder)、`mod.rs` に `pub mod paddleocr;` 追加、`inference.rs::stub_recognise_with_session` に `paddleocr-v4-server:` prefix dispatch 追加 3rd arm、`onnx-backend.ts::ensureWarm` の `typeof visionInitSession !== "function"` guard 削除 (B1 clean)、`onnx-backend.ts::recognizeRois` の legacy path fall-through 削除 (B1 clean、warm 未達 → `[]` 返却)、`NativeRecognizeRequest` import 不要削除、`visual-gpu-onnx-backend.test.ts` Block A/B accuracy 追従 (Block A: visionInitSession 不在 → "evicted"、Block B: ensureWarm 未呼出 → `[]` 返却)、`visual-gpu-session.test.ts` Block A accuracy 追従 (同)。cargo check 3 feature set 全 exit 0、tsc noEmit exit 0、vitest 101 file (1763 passed / 11 skipped、regression 0、e2e 12 件は pre-existing browser-cdp failures)。`paddleocr.rs::tests` 15 ケース。詳細は `docs/phase4b-5c-paddleocr-design.md`

- [x] **4b-6**: Cross-check (multi-engine 投票) を optional flag で実装 — `DESKTOP_TOUCH_VISUAL_CROSS_CHECK=1` で PaddleOCR-v4-server (primary) + PaddleOCR-v4-mobile (secondary) 2-way Levenshtein voting 有効化。`cross-check.ts` 新規 (`levenshteinDistance` O(m*n) 2-row DP、`crossCheckLabels` arbitration、`WinOcrFallbackFn` type、`CROSS_CHECK_AGREEMENT_THRESHOLD=0.2`)、`stage-pipeline.ts` に `StageSessionKeys.stage3b?` + `StagePipelineInput.winOcrFallback?` 追加、`runStagePipeline` に stage3b 並列実行 + `crossCheckLabels` 接続、`onnx-backend.ts` に env var 判定 + `initStageSessions` stage3b optional init + `winOcrTierInfinity` (win-ocr.exe spawnSync Tier ∞) 注入。cargo check 3 feature set 全 exit 0、tsc noEmit exit 0、vitest 128 file (2000 passed / 31 skipped、regression 0)。`vision-gpu-cross-check.test.ts` 16 ケース + `stage-pipeline.test.ts` 2 ケース追記。詳細は `docs/phase4b-6-cross-check-design.md`
- [x] **4b-7**: `BenchmarkHarness` を実 backend で動かし、cold/warm/idle を測定 — `bench-runner.ts` 新規 (`BenchmarkRunner.run`: OnnxBackend wiring + cold/warm/idle 3 scenario + evicted 判定)、`runAndWrite` (JSON 永続化、`~/.desktop-touch-mcp/bench.json`)、`warmP99` (warm metrics p99 計算)、`defaultBenchPath`、`scripts/run-bench.mjs` CLI (argv parse → runAndWrite → bench.json 書き出し)、`benchmark.ts::BenchmarkResult` に `capabilityProfile?: NativeCapabilityProfile` 追加。cargo check 3 feature set 全 exit 0、tsc noEmit exit 0、vitest 129 file (2012 passed / 30 skipped、regression 0)。`vision-gpu-bench-runner.test.ts` 10 ケース新規。実測値は user dogfood 必須 (artifact 不在時は "evicted" 記録して exit 0)。詳細は `docs/phase4b-7-bench-runner-design.md`
- [x] **4b-8**: vendor matrix benchmark aggregator + markdown report — **Phase 4b 最終 batch 完了** — `bench-aggregator.ts` 新規 (`aggregateOne`: BenchmarkResult → VendorRow 変換 + warmP99/cold/evicted 判定、`aggregate`: 複数 VendorRow を warmP99 昇順ソート・evicted last、`readBenchFile`: bench.json 読込、`formatMarkdownTable`: GitHub-flavored markdown table 生成 (pipe escape / em-dash / _evicted_ フォールバック)、`formatBenchMarkdown`: BENCH.md 全文生成 + ADR-005 L1/L4/L6 callout)、`scripts/generate-bench-report.mjs` CLI (--input 複数 bench.json → --output BENCH.md、stdout fallback)。tsc noEmit exit 0、cargo check 3 feature set 全 exit 0、vitest 132 file (2021 passed / 30 skipped、regression 0)。`vision-gpu-bench-aggregator.test.ts` 10 ケース新規 (aggregateOne 3 / aggregate 3 / formatMarkdownTable 3 / formatBenchMarkdown 1)。実機 dogfood (RX 9070 XT + CPU) は `node scripts/run-bench.mjs` → `node scripts/generate-bench-report.mjs` のパイプで実施可能。詳細は `docs/phase4b-8-bench-aggregator-design.md`。**Phase 4b (4b-1〜4b-8、4b-2 は ADR-006 移管除く) 全 batch 完了。**
