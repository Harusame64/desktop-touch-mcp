...
E. **`getSessionId` source 拡張** (S4 では default `() => "default"` + production override `args.lease?.viewId ?? "default"`):
  ...
  - **E-2. query-axis `desktop_state`**: lease 不在のため **`args` から viewId 取得不可**。trunk では **`"default"` fallback** で commit-axis と同 key 共有 (single-session prototype = trunk 範囲、CLAUDE.md §3.2 既存 e2e test の single-server-process semantic に整合)
  - **E-3. multi-session 並列 LLM client への対応**: **OQ #3 で carry-over** — MCP transport context 由来の session_id (= MCP request session header) を session_id source とする schema は別 PR (S5 trunk skeleton では single-session で十分、production multi-LLM-client は ADR-011 work)
...
- **OQ #3 — multi-session session_id source**: MCP transport context 由来 session_id (request session header / connection-id 等) は ADR-011 (Cognitive Memory Taxonomy) で finalize、本 S5 は single-session prototype で十分。本 sub-plan §8 OQ #3 で carry-over。
...
export interface ToolCallEvent {
  ...
  /** ToolCallStarted の wallclock (causal window timeout 200ms 計算 + caused_by.elapsed_ms 計算) */
...
export const defaultL1Emitter: CommitL1Emitter = {
  pushStarted({ tool, argsJson, sessionId, toolCallId, leaseToken }) {
    ...
    // S5 NEW: history buffer 二重記録 (L1 失敗時も entry は記録、causal window 計算は wallclock 由来で動作可能)
    pushHistoryStarted({
      toolCallId, toolName: tool, argsSummary: argsJson, eventIdStarted,
      wallclockStartMs: Date.now(), leaseToken,
    });
  },
...
/** caused_by 5 field projection (ADR-010 §5 line 200-213 + walking-skeleton §4 S5 line 258-269) */
export interface CausedByShape {
  ...
  based_on: { events: number[]; sources: string[] };  // [event_id_started, event_id_completed], sources e.g. ["UIA","DXGI"]
  elapsed_ms: number;            // ToolCallStarted ↔ ToolCallCompleted wallclock 差
  produced_changes: string[];    // L3 view diff projection
}
...
export function buildCausedBy(
  ...
  options?: { causalWindowTimeoutMs?: number }  // default 200ms (walking-skeleton §4 S5 line 264)
  ...
  // causal window 右端 (b) timeout 200ms 判定
  const timeoutMs = options?.causalWindowTimeoutMs ?? 200;
  if (viewSnapshot.queryWallclockMs - lastEvent.wallclockStartMs > timeoutMs) return undefined;

  // produced_changes projection (§2.3)
  const producedChanges = buildProducedChanges(/* before view = previous query 時点、本 trunk skeleton では現 view diff のみで近似、§7 R5 で deep-diff carry-over */);
  ...
  return {
    ...
    based_on: {
      events: [
        ...(lastEvent.eventIdStarted !== undefined ? [Number(lastEvent.eventIdStarted)] : []),
        ...(lastEvent.eventIdCompleted !== undefined ? [Number(lastEvent.eventIdCompleted)] : []),
      ],
      sources: ["UIA", "DXGI"],  // S5 trunk: 2 source 固定、S5 expansion で動的化
    },
...
mcp.tool(
  ...
  makeQueryWrapper(desktopStateRawHandler, "desktop_state", {
    getSessionId: (args) => {
      // S5 trunk: lease 不在のため args から resolve 不可、`"default"` fallback (single-session、§1.1 E-2)
      // multi-session は OQ #3 carry-over
      return "default";
    },
...
| **右端 (a) 次 query frontier** | `viewSnapshot.latestEventId` (= L1 ring 末尾 event_id at query time) | `buildCausedBy` 内 `based_on.events[1]` 上限 | §2.2 |
| **右端 (b) timeout 200ms** | `queryWallclockMs - wallclockStartMs > 200ms` で window expired return undefined | `buildCausedBy` 内 timeout check | §2.2 |
...
- [ ] `src/tools/_envelope.ts` 拡張 (S4 既存 module に追加):
  ...
  - [ ] `defaultL1Emitter` 拡張: `pushStarted` / `pushCompleted` 内に `pushHistory*` 呼出を追加 (L1 push 失敗時も history 記録継続、best-effort fail-safe)
...
- [ ] §3 sub-batch 数 (S5-1〜S5-9 = **9 件**)、§7 Risks 数 (**8 件**、R1-R8)、§8 OQ 件数 (**3 件**)、size 想定 (~300-500 line / 3-5 日 = walking-skeleton §4.1 line 306 整合) が本 sub-plan 内で bit-equal か?
- [ ] `Grep "300-500 line\|3-5 日\|9 件 sub-batch\|3 件 OQ\|8 件 Risks\|R1-R8\|G5-S5-1.*G5-S5-10\|10/10 test\|2537 test"` で本 sub-plan 内 numeric counts が bit-equal か?
...
| R3 | `buildCausedBy` の causal window check で wallclock_ms drift (system clock 修正 / NTP sync で時刻飛び) が timeout 200ms 判定を誤らせる | 低 | wallclock 軸は walking-skeleton §3.4 「`同 logical_time` を wallclock 同一と誤読しない」と整合、本 trunk では time-monotonic 軸の代用として wallclock を採用 (Date.now())、production drift mitigation は expansion (OQ #2 first stable observation で stable detect ベース、wallclock 非依存判定) |
...
## 8. Open Questions (S5 trunk-relevant、3 件)

| # | OQ | 決定タイミング | 推奨 (Opus 判断委譲) |
|---|---|---|---|
| 1 | `_historyBuffers` Map LRU eviction schema (1k entry 上限 / TTL 24h など) — 本 S5 trunk で impl するか、S5 後半 (= mid-life)/expansion carry-over か。S4 sub-plan OQ #1 で carry-over 済、S5 で本格運用化したため leak risk が顕在化 | 本 S5 impl PR Round 1 で finalize | **本 S5 trunk で impl 推奨** (§7 R1 mitigation、`G5-S5-10` test で leak path 確認、1k entry / TTL 24h / 上限超過時 head drop で skeleton 段階で十分)、Rust ring buffer / SQLite persist は ADR-011 carry-over |
...
| 3 | multi-session session_id source — MCP transport context 由来 (request session header / connection-id 等) を S5 で扱うか ADR-011 carry-over か。本 S5 trunk では `"default"` fallback の single-session prototype | ADR-011 (Cognitive Memory Taxonomy) で finalize | **ADR-011 carry-over 推奨** (§1.1 E-3 + §1.3 carry-over、本 S5 は single-session prototype で十分、multi-LLM-client は session 跨ぎ episodic memory と統合検討、MCP transport context 由来の session_id schema が必要なため別 ADR で finalize) |
...
END OF S5 sub-plan (Drafted v0.1)。
