...
- 概念設計:
  - `docs/adr-010-presentation-layer-self-documenting-envelope.md` §5 envelope schema (line 200-213 caused_by 5 field) + §5.2 任意拡張 (`include=causal` → caused_by) + §5.5 P3 phase (caused_by + invariants_held) + §5.6 Envelope Size SLO (causal include +1KB 以内、§5.6.1 表)
...
D. **`desktop_state.ts` 経路で `include=causal` 受領 → `_envelope.ts` 経由 `caused_by` 展開**:

  - `src/tools/desktop-state.ts` の handler は **不変** (ADR-010 §1.5 SSOT、tool 個別実装は修正不要)
  - MCP server 登録 site (`src/tools/desktop-register.ts`) で `makeQueryWrapper(desktopStateRawHandler, "desktop_state", { causedByProjector: defaultCausedByProjector })` 形式で wrapper option 経由で causal projection を inject
  - `desktopStateRawHandler` 自体は raw `ToolResult` を返すまま、wrapper 側で envelope assembly + causal include 解釈 + caused_by 注入
...
- **CLAUDE.md 強制命令 3.1 (ADR/plan 複数表 fact 整合)**: 本 PR では **5 SSOT** を bit-equal に揃える sweep:
  ...
  3. ADR-010 §5 envelope schema (line 200-213 caused_by 5 field) + §5.2 任意拡張 (`include=causal`、line 256) + §5.5 P3 phase (line 358) + §5.6 envelope size SLO (`include=causal` +1KB 以内、line 380)
...
/** caused_by 5 field projection (本 sub-plan §1.1 A-3 + walking-skeleton §4 S5 line 258-269 + architecture §6 line 213-215 worked example)
 *
 * **ADR-010 §5 SSOT divergence note** (Round 2 P1 Opus #2 反映):
 * ADR-010 §5 example (line 200-206) は 4 field (`your_last_action, session_id, elapsed_ms, produced_changes`) で
 * `tool_call_id` も `based_on` も含まず、architecture §6 worked example (line 213-215) は逆に `tool_call_id` あり
 * `session_id` なしで両者が divergence。CLAUDE.md §3.1 「主要表 / 見出しは注記より強く読まれる」原則に従い、
 * 本 S5 trunk では **「architecture §6 worked example が L4 envelope 組立て worked example の SSOT」** とし、
 * `tool_call_id` を含める形で採用。`session_id` は ADR-010 §4 識別子ヒエラルキーで envelope 全体に共通する pivot
 * のため CausedBy field 内には重複させず、envelope wrapper の `_session_id` 等の別 field で L4 から L5 wrapper に
 * 渡す semantic とする (本 S5 では `session_id` は include せず、`tool_call_id` の prefix から逆引可能で十分)。
 * ADR-010 §5 example の後追い更新は §6 follow-up OQ #6 carry-over。
 *
 * 5 field 集合:
 ...
 *   - based_on.events: bigint[] (L1 event_id u64 monotonic、Round 2 P2 Opus/Codex 反映で precision loss 回避)
...
export interface CausedByShape {
  ...
  /**
   * Round 2 P2 (Opus/Codex 重複) fix: events を `bigint[]` に変更、`Number(bigint)` precision loss を回避
   * (event_id は u64 monotonic、long-running server で 2^53 超達成 risk)。
   * sources は trunk では UIA/DXGI 観測 driven の動的 build (Round 2 P2 Opus #3 反映、UIA/DXGI 両方が causal
   * window 内に発火していない場合は empty array)。
   */
  based_on: { events: bigint[]; sources: string[] };
...
export function buildCausedBy(
  ...
  // Round 2 P2 (Opus #3) fix: sources は produced_changes の出処から動的 build
  // (UIA = focus 遷移由来 / DXGI = dirty_rect count > 0 由来)、観測されていない source は include しない
  ...
  return {
    ...
    based_on: {
      // Round 2 P2 (Opus/Codex 重複 line 311) fix: bigint[] 維持で precision loss 回避
      events: [
        ...(lastEvent.eventIdStarted !== undefined ? [lastEvent.eventIdStarted] : []),
        ...(lastEvent.eventIdCompleted !== undefined ? [lastEvent.eventIdCompleted] : []),
      ],
      sources,  // Round 2 P2 (Opus #3) 動的 build
    },
...
- **causal window 右端 (a) は viewSnapshot.latestEventId を `based_on.events` の上限として使用**: history entry の `eventIdCompleted` が L1 push 失敗で undefined の場合は `based_on.events` から 1 entry 落ちる (best-effort、§7 R3 で記述)
...
export interface QueryWrapperOptions extends MakeEnvelopeAwareOptions {
  /**
   * S5 NEW: caused_by projection を envelope に注入する callback。
   ...
   *   1. sessionId を args から resolve (default "default" fallback)
   *   2. view から ViewSnapshot を build (focus + dirty_rects)
   *   3. buildCausedBy(sessionId, snapshot) を呼出
   * Tests inject a deterministic projector to pin G5-S5-1〜3 path.
   */
  causedByProjector?: (args: unknown) => Promise<CausedByShape | undefined>;
  /**
   * S5 NEW: getSessionId source (S4 既存 commit-axis と semantic 共有)。
   * Default `"default"` — single-session prototype、multi-session は OQ #3 carry-over。
   */
  getSessionId?: (args: unknown) => string;
}
```

`makeQueryWrapper` の internal flow に **include peek + causedByProjector 呼出** を追加:

```typescript
// makeQueryWrapper (S5 で拡張、S4 既存 flow + include peek 追加)
async function wrapped(rawArgs: unknown): Promise<unknown> {
  ...
  if (includeArgs.includes("causal") && options.causedByProjector) {
    causedBy = await options.causedByProjector(handlerArgs);            // S5 NEW
  }
  const envelope = buildEnvelope(handlerResult, { causedBy });          // S5: causedBy を envelope に inject
...
`src/tools/desktop-register.ts` で MCP server 登録 site での wiring (handler 内部 logic + Zod schema + 戻り値 shape 不変):

```typescript
// src/tools/desktop-register.ts (S5 で追加、S4 既存登録に causedByProjector を inject)

mcp.tool(
  "desktop_state",
  desktopStateRegistrationSchema,
  makeQueryWrapper(desktopStateRawHandler, "desktop_state", {
    getSessionId: (args) => {
      // Round 2 P1 (Codex #1) fix: concurrent MCP session が単一 history buffer を共有して
      // cross-session caused_by leak が起きる risk を防ぐため、production wiring では
      // `getMcpTransportSessionId()` (MCP transport context 由来、SDK Server.session 等から resolve) を
      // 第一候補とし、resolve 不可な場合 (ADR-011 完了前 single-session prototype 環境) のみ
      // `"default"` fallback。production multi-LLM-client wiring は **§3.4 desktop-register.ts impl で
      // detectMultiSessionContext() で warn / opt-out**、本 S5 では single-session prototype の guarded fallback。
      const transportSessionId = getMcpTransportSessionId();  // ADR-011 で finalize、本 S5 trunk では存在チェックのみ
      if (transportSessionId !== undefined) return transportSessionId;
      // single-session prototype 確認 (production で multi-LLM client が同 server にぶら下がっていない前提)
      if (!isSingleSessionPrototype()) {
        // multi-session 検出 → caused_by を NOT 付ける選択 (cross-session leak 回避)
        // 本 S5 trunk では impl PR §3.4 で warn log + sessionId="multi:disabled" 等の sentinel を返し
        // buildCausedBy で sentinel detect → undefined return で caused_by 付与 skip
        return "multi:disabled";
      }
      return "default";  // single-LLM-client prototype (本 S5 trunk スコープ範囲、§1.1 E-2)
    },
    causedByProjector: async (_args) => {
      ...
      return buildCausedBy(sessionId, snapshot);
    },
  }),
);
```
...
- [ ] `src/tools/_envelope.ts`:
  ...
  - [ ] `CausedByShape` interface (§2.2、ADR-010 §5 line 200-213 5 field)
  ...
  - [ ] `EnvelopeMinimalShape` 拡張: `caused_by?: CausedByShape` optional 末尾追加 (S3/S4 既存 minimal shape 不変、`stale` confidence + `if_unexpected` field と並列の optional)
  - [ ] `buildEnvelope` 既存関数の signature 拡張: `(data, options?: { causedBy?: CausedByShape, ... })` で causedBy を envelope に inject (S3/S4 既存 caller 無修正)

### 3.3 S5-3: `makeQueryWrapper` 拡張 (`include=causal` opt-in) (~50 line) [S5 trunk]

- [ ] `src/tools/_envelope.ts`:
  - [ ] `QueryWrapperOptions` interface 拡張 (`causedByProjector?` + `getSessionId?` 末尾 optional 追加、S4 既存 caller 無修正で pass)
  - [ ] `makeQueryWrapper` internal flow 拡張:
    - [ ] include peek + `includeArgs.includes("causal")` 判定
    - [ ] `causedByProjector` 設定時は呼出して causedBy を取得、未設定または非 causal include 時は `undefined`
    - [ ] `buildEnvelope(handlerResult, { causedBy })` 経由で envelope inject
...
- [ ] `src/tools/desktop-register.ts`:
  - [ ] `desktop_state` 既存登録 site を `makeQueryWrapper(desktopStateRawHandler, "desktop_state", { getSessionId: () => "default", causedByProjector })` で wrap
  - [ ] `defaultCausedByProjector(args)` closure 実装 (§2.5)、production wiring
...
- [ ] `tests/unit/desktop-state-causal-include.test.ts` 新設 (caused_by linkage を 1 file で網羅):
  - [ ] **G5-S5-1**: 基本 path — `desktop_act` (commit) → history buffer entry 記録 → `desktop_state(include=["causal"])` で envelope.caused_by 5 field 全部埋まる (`your_last_action`, `tool_call_id`, `based_on.events`, `elapsed_ms`, `produced_changes`)
...
- [ ] **本 PR で同梱 (S4 sub-plan PR #111 同型 pattern)**: `docs/walking-skeleton-trunk-selection.md` Appendix C 末尾に判定結果 append (本 sub-plan §3.8 完了基準を merge 前に踏む形で先行 commit、Opus PR #111 Round 1 P2 #4 反映 pattern 継承):
  ```markdown
  | G5 | 2026-05-XX | 継続 | per-session history buffer (TS Map<sessionId, ring 8 件 cap>) + causal window 境界 (左端 ToolCallStarted event_id / 右端 (a) latest event frontier + (b) timeout 200ms) + buildCausedBy projection (5 field) + buildProducedChanges (focus delta + dirty_rect per-monitor count) で desktop_act → desktop_state(include=causal) シーケンス integration test 10/10 pass、envelope size 1.X KB (causal_typical) で < 2KB SLO 維持 (S3 bench harness で計測)、既存 raw client 互換 default opt-out で破壊なし。S6 trunk 完了判定 (expansion tool 1 件追加が L5 wrapper 修正のみ + CI assert 化) に進める contract base 確立 | (なし) |
  ```
...
- [ ] **`buildCausedBy` 内 timeout 判定が wallclock_ms 軸で動作することを runtime 確認** (compile-time 通過だけで安心しない、§4.2)
...
- [ ] §3 sub-batch 数 (S5-1〜S5-9 = **9 件**)、§7 Risks 数 (Round 2 で R9/R10 追加で **10 件**、R1-R10)、§8 OQ 件数 (Round 2 で OQ #4/#5 追加で **5 件**)、size 想定 (~300-500 line / 3-5 日 = walking-skeleton §4.1 line 306 整合) が本 sub-plan 内で bit-equal か?
- [ ] `Grep "300-500 line\|3-5 日\|9 件 sub-batch\|5 件 OQ\|10 件 Risks\|R1-R10\|G5-S5-1.*G5-S5-10\|10/10 test\|2537 test"` で本 sub-plan 内 numeric counts が bit-equal か?
...
- **OQ #6 (S5 follow-up + ADR-010 後追い更新)** (Round 2 P1 Opus #2 反映): ADR-010 §5 envelope schema example (line 200-206) の `caused_by` field 集合が **本 sub-plan §2.2 + architecture §6 worked example と divergence**。本 sub-plan は architecture §6 を SSOT に採用 (5 field: `your_last_action, tool_call_id, based_on, elapsed_ms, produced_changes`)、ADR-010 §5 example を後追い同期更新する PR を S5 impl PR と並走または S6 完了時に出す
...
| R9 | (Round 2 P2 Opus/Codex 重複 line 311 反映) `Number(bigint)` 変換で event_id が 2^53 (9PB events) 超過時 precision loss、long-running server で `based_on.events` の event_id 識別が破綻 | 低 | **Round 2 で resolve**: `CausedByShape.based_on.events` を `bigint[]` 型維持、JSON serialize 段で string 化 (TS native bigint serialize 経路、JSON.rawJSON 等)、`Number()` 変換禁止を §4.5 push 6-guard に追加、Codex review (cross-layer linkage 軸) で type contract 確認 |
| R10 | (Round 2 P1 Codex #1 反映) concurrent MCP session で `getSessionId() = "default"` hardcode により単一 history buffer 共有 → cross-session caused_by leak、別 LLM client の commit が誤って attributed | 中 | **Round 2 で resolve**: §1.1 E-2 で sentinel guard (`"multi:disabled"`) + warn log impl、`isSingleSessionPrototype()` + `getMcpTransportSessionId()` helper を §3.1 S5-1 sub-batch に追加、multi-session 検出時は caused_by skip で leak 回避、完全 finalize は ADR-011 (OQ #3) carry-over |
...
| 4 | (Round 2 P1 Opus #2 反映) ADR-010 §5 envelope schema example の `caused_by` field 集合 後追い同期 — 本 sub-plan §2.2 + architecture §6 worked example と divergence、本 S5 で SSOT を architecture §6 に採用したため ADR-010 §5 example を後追い更新する PR が必要 | S5 impl PR と並走または S6 完了時 | **S6 で ADR-010 §5 example update PR 起草推奨** (本 sub-plan §2.2 5 field SSOT を ADR-010 §5 line 200-206 example にも反映、CLAUDE.md §3.1 fact 整合 sweep、PR #99 D2-C0 同型 fact divergence 防止) |
...
desktop_state (query、S4 で wrap 済 / S5 で causedByProjector inject):
  ...
  S5 NEW: include=causal 受領時 →
       ...
       │     │     - queryWallclockMs = Date.now()
       │     │
       │     └─ buildCausedBy(sessionId, snapshot):
       │           - history buffer から最新 entry 抽出 (直前 1 件のみ)
       │           - causal window check (右端 (b) timeout 200ms = wallclockStart + 200 < query 時 → undefined)
...
- 概念設計: `docs/adr-010-presentation-layer-self-documenting-envelope.md` §5 envelope schema (line 200-213 caused_by 5 field) + §5.2 任意拡張 (`include=causal`、line 256) + §5.5 P3 phase (line 358) + §5.6 envelope size SLO (`include=causal` +1KB 以内、line 380)
...
- 既存実装:
  ...
  - `src/tools/desktop-register.ts` (本 PR で `desktop_state` 登録 site を `makeQueryWrapper(..., { causedByProjector, getSessionId })` で wrap)
...
END OF S5 sub-plan (Drafted v0.2)。
