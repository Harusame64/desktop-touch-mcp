...
- 後続: **expansion phase** (S6 完了後、worktree 並走で 24+10 tool wrapper 化 + typed reason 残 36 codes + dry-run integration、長期作業で完了に数週間想定)
...
D. **`docs/walking-skeleton-expansion-plan.md` 新規起草** (本書の続編、tool ごとの worktree 並走計画):
  ...
  - swimlane 着手 priority: L5 commit (最も波及効果高) → L5 query → L4 envelope (P3 invariants) → typed reason 残 36 codes → L1 secondary monitor → L1 emit → L3 view 拡充
...
- **typed reason 残 36 codes** (ADR-010 §5.4): `_errors.ts::SUGGESTS` を `try_next: TypedAction[]` に進化、ADR-010 P2 acceptance criteria 100% mapping
...
| typed reason 残 36 codes | _errors.ts SUGGESTS 連動 | 4 | 4-5 | ✓ 並走可 |
...
- **typed reason 残 36 codes**: ADR-010 P2 work
...
│                  expansion plan 起草                                  │
│      ↓                                                                │
│  expansion phase (worktree 並走、24+10 tool + typed reason 残 36 +    │
