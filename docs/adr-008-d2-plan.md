...
B. **worker_loop tuning (revised)** — batch drain + observed max logical time release で **operator step p99 < 1ms** 達成、partial-order 維持 (release-to-view は `shift_ms` 律速の構造的下限、PR-2 SLO 4 種分解で正確化)
...
**目的**: D1 followups §2.5 の core 課題を **partial-order を壊さない設計** で解決、operator step p99 < 1ms SLO 達成 + release-to-view を構造的下限として正確化 (PR-2 SLO 4 種分解、Opus 諮問判断 §5)、D1 acceptance honest 化。
...
- [x] **views-catalog §3.1 4 種分解 SLO 全達成** (PR-2 で正確化、Opus 諮問判断 §5、option C 永久 defer)
...
- `operator step` p99 < 1ms (D2-A revised tuning と同水準、純計算下限)
- `release-to-view` p99 ≈ `shift_ms` (D1 と同 floor 継承、N3 partial-order contract の構造的下限)
- `MCP round-trip` は `current_focused_element` と同 SLO (< 10ms) を継承 (PR-2 SLO 4 種分解、Opus 諮問判断 2026-05-02 §5)
...
| KPI | 目標 |
|---|---|
| `view_update_latency` p99 (D2-A 後、PR-2 で 4 種分解) | views-catalog §3.1 4 種 SLO 参照 (operator step < 1ms / release-to-view ≈ shift_ms / MCP round-trip < 10ms) |
...
| # | Risk | 影響 | Mitigation |
|---|---|---|---|
| ~~R1~~ | ~~revised tuning でも `update p99 < 1ms` 未達~~ | **Resolved (PR-2、Opus 諮問判断 §5)**: SLO 文言を 4 種分解により正確化、release-to-view は N3 contract の構造的下限 (緩和ではなく物理的真実)、option C は永久 defer (§10 OQ #2 Resolved) |
...
| ~~2~~ | ~~revised tuning でも SLO 未達の場合の option C (`parking_lot::Condvar`) 移行~~ | **Resolved (PR-2、Opus 諮問判断 2026-05-02、案 (2) SLO 文言正確化採択)**: option C (C-2 per-event hold-back queue / C-3 optimistic release + retraction) は **永久 defer**。判断根拠: (a) release-to-view は N3 partial-order contract の必然帰結として `shift_ms` 下限が物理的真実、(b) production read-dominated 前提で update latency 体感影響小、(c) MCP round-trip (production 観測 SLO) は D2-B-4 6.22ms steady-state で TS / 1.5 既達、option C による寄与は 5-10% 程度、(d) N2 violation 同型再発リスク高 + retraction propagation で ADR-010 envelope shape まで波及する重 refactor。views-catalog §3.1 を 4 種分解 SLO に正確化することで本 OQ closure |
...
| ~~16~~ | ~~`view_update_latency` p99 < 1ms SLO の達成可否~~ | **Resolved (PR-2、2026-05-02)**: views-catalog §3.1 SLO を 4 種分解 (operator step / lookup / release-to-view / MCP round-trip) に正確化することで closure。当初の単一 `< 1ms` 表記は **3 種類の latency を曖昧に包含** していた SSOT の不正確さに起因。各種の達成状況: (a) operator step 純計算下限 µs オーダで達成、(b) lookup ~145-300ns で達成 (3000-7000× margin)、(c) release-to-view は N3 partial-order contract により `shift_ms` 下限が **物理的真実** (緩和ではなく正確化)、(d) MCP round-trip < 10ms 達成 (D2-B-4 6.22ms steady-state)。view-hit 経路の精緻 SLO は D2-B-5 (`docs/adr-008-d2-b-5-plan.md`、PR #118 merged `ecea759`) で取得した bench 数値で確定 (≤2ms → `< TS p99 / 5` / 2-5ms → `< TS p99 / 1.5` / ≥5ms → `< 10ms` 維持)、本 PR-2 では default `< 10ms` で land、bench 結果次第で finalize commit |
...
- [x] **views-catalog §3.1 4 種分解 SLO 全達成** (operator step + lookup + release-to-view + MCP round-trip) — PR-2 で正確化、option B 撤回 (N3 維持) + option C 永久 defer (Opus 諮問判断 §5)。release-to-view は `shift_ms` 律速の構造的下限と明示
