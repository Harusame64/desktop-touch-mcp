...
F. **新規 view 2 つ + 1 carry-over** (D2-C0 ゲート結果反映、2026-05-01 確定):
   - `dirty_rects_aggregate` → **carry-over** (P5c-2 prerequisite、§D2-C0-2 matrix 参照)
   - `semantic_event_stream` (`FocusMoved` variant 単独で成立、`WindowChanged` / `ScrollSettled` / `ModalAppeared` 除外)
   - `predicted_post_state` subgraph 構造 (D5 で本格化)
   - 結果 ADR-008 §4 D2「主要 view 4」は本 D2 では **3 view (current_focused / semantic_event[FocusMoved-only] / predicted_post_state subgraph)** に縮小、`dirty_rects_aggregate` は P5c-2 完了後の別 phase で着手
...
**Status (2026-05-01)**: D2-C0 で `EventKind::DirtyRect` (=0) の L1 ring emit site **皆無**確認 (§D2-C0-1)。本 PR は **carry-over**、ADR-007 P5c-2 完了後の別 phase で着手 (§D2-C0-4 で Opus 判断委譲)。`DirtyRectSubscription` の async pull path は `DirtyRectRouter` 経由で production 動作中だが、L1 ring envelope path とは別 channel なので本 view の input としては未配線。
...
| **PR-ε (D2-C)** | ~~dirty_rects_aggregate (D2-C0 go の場合)~~ | ~~中~~ | **Carry-over** — ADR-007 P5c-2 prerequisite、本 D2 から外す |
...
- [ ] **主要 view 3 の declarative 実装** (D2-C0 ゲート確定 2026-05-01 で 4 → 3 に縮小、`dirty_rects_aggregate` carry-over):
  ...
  - **carry-over**: `dirty_rects_aggregate` (D2-C) — ADR-007 P5c-2 prerequisite、§D2-C0-2 matrix 参照
