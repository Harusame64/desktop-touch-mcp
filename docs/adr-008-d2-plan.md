...
F. **新規 view 3 つ** (D2-C0 ゲート 2026-05-01 で carry-over だった `dirty_rects_aggregate` が P5c-2 PR #102 merged で復帰、§3.bis ledger L1 trigger 完了):
   - `dirty_rects_aggregate` (D2-C、本 D2-C plan PR で sub-plan land、impl PR は本 plan merge 後の翌 PR — `docs/adr-008-d2-c-plan.md` 参照)
   ...
   - 結果 ADR-008 §4 D2「主要 view 4」は本 D2 で **4 view 全部実装** (`dirty_rects_aggregate` も復帰)、`semantic_event_stream` の他 variant 拡張のみ §3.bis ledger L2-L4 で carry-over
...
**Status (2026-05-01)**: P5c-2 emit が PR #102 (`c535fc2`) で production land、§3.bis carry-over ledger L1 trigger 完了。本 PR-ε で復帰 PR sub-plan を起草、詳細 acceptance / 設計判断 / sub-batch / risks / OQ は **`docs/adr-008-d2-c-plan.md`** (sub-plan) 参照。

#### D2-C-1〜D2-C-8 (sub-plan §3 で詳細化)

詳細は `docs/adr-008-d2-c-plan.md` §3 sub-batch (8 件)。本 §D2-C は最低限の親 acceptance:

- [ ] `src/l3_bridge/dirty_rect_pump.rs` 新設、focus_pump と同パターン (sub-plan §3.5)
- [ ] `crates/engine-perception/src/views/dirty_rects_aggregate.rs` 新設、`map → reduce(per-(monitor, frame) Vec<Rect> + summary) → inspect` (sub-plan §3.3)
- [ ] partial-order test (out-of-order frame_index、cross-monitor 含む) (sub-plan §3.8 Test 3)
- [ ] SLO `update p99 < 2ms` を bench で計測 (sub-plan §5 follow-up `D2-C-bench` PR で carry-over)
- [ ] **Opus phase-boundary review** (強制命令 3 + 3.1 + 3.2) + **Codex re-review** (CLAUDE.md 3.2 運用 rule、production code 改修 PR)
...
| **PR-ε (D2-C)** | dirty_rects_aggregate view (sub-plan: `docs/adr-008-d2-c-plan.md`、§3.bis ledger L1 復帰 PR、P5c-2 PR #102 trigger 完了) | 中 | 300-450 line (sub-plan §4 で再見積、view + pump + spawn + napi + 6 unit test) — **本 D2-C plan PR (sub-plan land) → impl PR は本 plan merge 後の翌 PR** |
...
|---|---|---|---|---|
| ~~L1~~ | ~~D2-C `dirty_rects_aggregate` view (`crates/engine-perception/src/views/dirty_rects_aggregate.rs` + `src/l3_bridge/dirty_rect_pump.rs`)~~ | **Resolved trigger (P5c-2 PR #102 merged 2026-05-01、`c535fc2`)** + **復帰 PR sub-plan landed (本 D2-C plan PR、`docs/adr-008-d2-c-plan.md`、2026-05-01)** | ~~**PR-ε (D2-C)**~~ — 本 D2-C plan PR で sub-plan 起草、impl PR は本 plan merge 後の翌 PR で着手 | P5c-2 PR でPC-emit Rust integration test (mock context-based、`spawn(0)` + `Next` cmd 経由) pin 完了。D2-C sub-plan §3.8 で view 6 unit test (per-frame aggregation / per-monitor isolation / out-of-order partial-order / 100ms eviction / recent_n / recent_window) で view declarative pin |
...
## 5. `dirty_rects_aggregate` view 仕様 (D2-C-plan 起草で詳細化、2026-05-01 reconcile)
...
- input: `DirtyRectEvent { source_event_id, wallclock_ms, sub_ordinal, timestamp_source, monitor_index, frame_index, rect }`
  - **`monitor_index` 追加** (CLAUDE.md 強制命令 3.2 適用、PR #102 P5c-2 monitor_index 正しい伝搬と整合)
...
- output: `DirtyRectsAggregate { monitor_index, frame_index, rects: Vec<Rect>, summary: { count, total_area } }`
  - **`monitor_index` 追加** (per-monitor で aggregate を分離、cross-monitor で frame_index 衝突しても正しく分離)
- operator: `map → reduce(per-(monitor_index, frame_index) Vec<Rect> + summary) → inspect`
- SLO: `update p99 < 2ms`, memory < 50MB (100ms 窓)
...
- [ ] **主要 view 4 の declarative 実装** (D2-C0 ゲート 2026-05-01 で carry-over だった `dirty_rects_aggregate` が P5c-2 PR #102 merged で復帰、本 D2-C plan PR で sub-plan land):
  - [x] `current_focused_element` (D1 完了)
  - [ ] `dirty_rects_aggregate` (D2-C、本 D2-C plan PR で sub-plan landed、impl PR は本 plan merge 後の翌 PR — `docs/adr-008-d2-c-plan.md` 参照)
