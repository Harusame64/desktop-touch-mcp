...
- Status: **Draft v3 (Codex review v2 反映、Opus、2026-04-30)** — v1 → Codex v1 (P1×2 / P2×3 / P3×1 + 北極星 3 条件) → v2 → Codex v2 (P1×1 残ズレ: spawn→subscribe 競合) → v3 (本書)
- Date: 2026-04-30
...
- [ ] `src/l1_capture/ring.rs` に `pub struct SubscriptionEvent` 追加:
  ```rust
  #[derive(Clone, Debug)]
  ...
  ```
- [ ] `From<&InternalEvent> for SubscriptionEvent` 実装 (payload は `Arc::from(e.payload.as_slice())` で 1 回コピー)
- [ ] `napi` 型 (BigInt / Buffer) は **使わない** (subscribers は Rust 内 only)

#### 3.1.2 `Subscription` 型 (Drop で auto-unsubscribe)
- [ ] `src/l1_capture/ring.rs` に追加:
  ```rust
...
- [ ] `fn unsubscribe(&self, id: u64)` (private、Subscription::Drop 経由のみ呼ばれる)
...
- [ ] 現 `push` 実装の冒頭で `event.event_id` を採番した直後に:
  ```rust
  ...
  drop(subs_guard);
  ```
- [ ] subscribers 0 のとき (`is_empty()`) は **snapshot 構築すら走らない** = 既存 SLO p99 < 1ms 完全維持
- [ ] 既存の `force_push(event)` / push_count / drop_count 経路は不変

#### 3.1.5 mod.rs re-export
- [ ] `src/l1_capture/mod.rs` に `pub use ring::{Subscription, SubscriptionEvent};` 追加

#### 3.1.6 unit test (broadcast 単独、Codex P2-1 / P2-2 反映)
- [ ] `single_subscriber_receives_events`: **subscribe → push 3 件 → `try_recv` 3 回 → kind / event_id 一致** (Codex P2-2: subscribe を先にすること明記)
- [ ] `multi_subscriber_each_receives_all`: 2 subscriber を先に作る、push 3 件、両方が 3 件受信
- [ ] `subscribe_after_push_does_not_replay`: ring.push 3 件 → subscribe → try_recv で空 (broadcast は historical replay ではない、Codex P2-2)
- [ ] `subscriber_drop_removes_slot`: subscribe → drop → ring.subscribers.read().is_empty()
- [ ] `subscriber_full_drops_new_with_counter`: capacity 4、subscribe → push 10 件 → try_recv で 4 件取得、`dropped_count() == 6` (Codex P2-1 反映で「new」)
- [ ] `existing_destructive_poll_unaffected`: subscribe + push 3 件、`ring.poll(0, 100)` も同 3 件取得 (両経路独立、destructive `pop()` は subscribers 状態と無関係)
- [ ] `push_with_no_subscribers_avoids_snapshot`: subscribers 空のときの push が既存 SLO 内 (実時間アサート、loose、SLO は `bench` で厳密)

### 3.2 `engine-perception::input::FocusInputHandle` 本実装

#### 3.2.1 `FocusEvent` 拡張 (Codex P1-1 / N1 反映)
- [ ] `crates/engine-perception/src/input.rs` の `FocusEvent` に **`source_event_id: u64`** + **`timestamp_source: u8`** を追加:
  ```rust
...
- [ ] worker_loop 本体:
  ...
  ```
- [ ] **timely 0.29 / DD 0.23 の正確な API は実装時に確認**:
...
- [ ] D1-3 で view を加えるときは `dataflow_builder` の中身に `arrange_by_key + reduce(...)` を追加するだけ、worker_loop の cmd dispatch は変更不要
...
- [ ] `impl PerceptionWorker { pub fn shutdown(self, timeout: Duration) -> Result<(), &'static str> }`:
  ...
  - JoinHandle を polling join (L1 worker と同型 `is_finished()` polling shape):
    ...
    Ok(())
    ```
- [ ] `impl Drop for PerceptionWorker`: `tx.send(Cmd::Shutdown)` ignore + best-effort `join` (panic 起こさない)
...
- [ ] `spawn_and_shutdown_clean`: **spawn → 即 shutdown → no panic**
- [ ] `push_focus_roundtrip_smoke`: **spawn → handle.push_focus(ev) × 3 → shutdown(2s) Ok**
- [ ] `5_cycle_spawn_shutdown`: **5 回連続で spawn → push 1 件 → shutdown**、leak / panic / deadlock 無し
- [ ] `push_after_shutdown_silently_drops`: shutdown 後の push は silently drop (panic しない)
- [ ] `l1sink_object_safety`: `Arc<dyn L1Sink>` に格納可能 (既存 stub の test を維持)
- [ ] `event_carries_source_event_id_and_timestamp_source`: ev.source_event_id = 12345 + timestamp_source = 1 で push、worker 側で受信して保持される (FocusEvent shape の round-trip 確認、N1)
- [ ] `out_of_order_within_watermark_accepted`: shift=100ms に対し event_time=t-50ms を後から push、out-of-order rejected カウンタが増えない (watermark 範囲内、D1-4 で view 経由 assert は別 PR)
  - 注: 本 PR では view が無いので「dropped にならない」だけ確認、deterministic semantics は D1-4 で assert
- [ ] `out_of_order_exceeding_watermark_dropped`: shift=100ms に対し event_time=t-500ms を後から push、stderr に out-of-order log が出る (assert は loose、D1-4 で counter 化)

### 3.3 `src/l3_bridge/focus_pump.rs` 実装

#### 3.3.1 構造体 / API
- [ ] `src/l3_bridge/focus_pump.rs` 新設:
  ```rust
  use std::sync::Arc;
...
- [ ] thread spawn name `l3-focus-pump`
- [ ] **`subscribe` は parent thread で実行、`Subscription` を thread に move** (Codex v2 P1: `spawn→subscribe` 競合を構造的に消す):
  ```rust
  pub(crate) fn spawn(ring: Arc<EventRing>, sink: Arc<dyn L1Sink>) -> Self {
  ...
  ```
- [ ] thread body は `Subscription` を所有して loop:
  ```rust
  fn run(
...
- [ ] `impl Drop for FocusPump`: best-effort signal + best-effort join
...
#### 3.3.4 mod.rs 統合
- [ ] `src/l3_bridge/mod.rs` の future submodule comment を `pub(crate) mod focus_pump;` 実体化に置換

#### 3.3.5 unit test (synthetic ring + mock sink、Codex v1 P2-2 反映で **spawn → push** の順、**Codex v2 P1 反映で immediate-push race regression**)
- [ ] `spawn_then_immediate_push_arrives`: **spawn focus_pump → 直後 (sleep なし) に ring.push 1 件 → 200ms 内に mock sink で受信** — Codex v2 P1 race の regression test。parent-side subscribe 実装が壊れたら必ず flaky fail する
- [ ] `forwards_uia_focus_to_sink`: **spawn focus_pump → ring.push (`UiaFocusChangedPayload { after: Some(...) }` 1 件) → 200ms 内に mock sink が `FocusEvent` 1 件受信** (Codex v1 P2-2)
- [ ] `forwarded_event_carries_source_event_id`: 上記 test の延長で受信 `FocusEvent.source_event_id == ring.push() の戻り値 event_id` を assert (N1)
- [ ] `forwarded_event_carries_timestamp_source`: `FocusEvent.timestamp_source == env.timestamp_source` を assert
- [ ] `skips_non_focus_events`: spawn → ring.push (`Heartbeat` 1 件) → mock sink が空のまま、forwarded_count == 0
- [ ] `skips_focus_with_no_after`: spawn → ring.push (`after: None`) → mock sink 空、`after_none_skip_count == 1`、forwarded == 0
- [ ] `decode_failure_increments_counter`: spawn → 不正 payload bytes (kind=UiaFocusChanged だが bincode decode fail) を直接 ring.push → `decode_failure_count == 1`
- [ ] `shutdown_within_2s`: spawn → shutdown(2s) で deadlock 無し
- [ ] `5_cycle_spawn_shutdown`: 5 回連続 spawn → push → shutdown、ring の subscribers が 0 に戻る (毎サイクル末で `ring.subscribers.read().is_empty()` 確認)
...
- [ ] `src/l3_bridge/mod.rs` に `pub(crate) fn spawn_perception_pipeline_for_test() -> (PerceptionWorker, FocusPump)`:
...
- [ ] `pub(crate) fn shutdown_perception_pipeline_for_test(worker: PerceptionWorker, pump: FocusPump) -> Result<(), &'static str>`:
...
- [ ] D1 plan §11 / `docs/architecture-3layer-integrated.md` §17 の production lifecycle に合わせ、D2 の MCP tool surface 露出時に napi entrypoint から自動起動するよう拡張 — 本 PR では起動しない (production OFF、N3)
...
- [ ] **`tests/d1_pipeline_lifecycle.rs`** 新規 (Codex P2-3 反映で `tests/integration/...` ではなく `tests/` 直下、Cargo auto-discovery 対象):
...
- [ ] vitest 経由 test は不要 (D2 で MCP tool 露出してから整備)

### 3.7 Vitest regression (既存 napi 経路への影響確認)

- [ ] `node scripts/test-capture.mjs --force` → 既存 2435 pass を維持 (P5c-1 PR 時点と同じ baseline)
- [ ] `l1_poll_events` (destructive) は不変動作、`l1_get_capture_stats` の `push_count` / `drop_count` も不変
- [ ] subscribers 0 のときの push hot-path コストが既存と一致 (snapshot 作らない条件分岐)

### 3.8 Docs flip

- [ ] `docs/adr-008-d1-plan.md` §3 D1-2 の checklist 各項目を `[x]` に flip
- [ ] `docs/adr-008-d1-plan.md` §11 Acceptance のうち本 PR で達成する項目を `[x]` (view / partial-order test / bench は本 PR scope 外なので除く)
- [ ] `docs/views-catalog.md` §3.1 `current_focused_element` 行に「pipeline 配線完了 (D1-2)、view operator graph は D1-3」と注記
- [ ] memory `project_3layer_architecture_design.md` を「ADR-008 D1-2 配線完了、次は D1-3 view operator」に更新
- [ ] memory `MEMORY.md` index 更新 (ADR-008 進捗行)
...
- [ ] `EventRing::subscribe()` で複数 subscriber が同 push を独立に受信 (本書 §3.1.6 unit test pass)
- [ ] `Subscription::Drop` で ring 側 `subscribers` から自動削除
- [ ] subscribers 0 のときの push hot-path コストが既存とほぼ等しい (snapshot 構築されない)
- [ ] subscribe 前の push は subscriber に届かない (broadcast は historical replay ではない、Codex P2-2)
- [ ] subscriber 側 channel full で **新規** snapshot が drop され `dropped_count()` がインクリメント (drop-newest 仕様、Codex P2-1)
- [ ] `FocusPump::spawn` で adapter thread 起動 → ring に `UiaFocusChanged` event を inject すると `FocusInputHandle::push_focus` が呼ばれる (mock sink で確認)
- [ ] **`FocusEvent.source_event_id == ring.push() の戻り値 event_id`** (北極星 N1)
- [ ] **`FocusEvent.timestamp_source == env.timestamp_source`** (北極星 N1 副次)
- [ ] **`FocusEvent.wallclock_ms` / `sub_ordinal` は data field として保持され、frontier `advance_to` の引数にされない** (北極星 N2)
- [ ] **worker_loop の frontier 進行は watermark 経由** (`advance_to((max_wallclock - WATERMARK_SHIFT_MS, 0))`、北極星 N2)
- [ ] watermark 範囲内の out-of-order event は受け入れられる (drop されない)、範囲外は eprintln + counter
- [ ] `payload.after = None` の event は graceful skip (`forwarded_count` 進まない、`after_none_skip_count` 進む)
- [ ] decode 失敗は graceful skip + `decode_failure_count` increment

### 7.2 lifecycle
- [ ] `spawn_perception_pipeline_for_test → push 数件 → shutdown_perception_pipeline_for_test` を 5 cycle 連続実行で leak / crash / deadlock 無し (本書 §3.6 integration test、`tests/d1_pipeline_lifecycle.rs`)
- [ ] shutdown 後 `ring.subscribers` が空
- [ ] `FocusInputHandle::push_focus` が worker shutdown 後に呼ばれても panic しない

### 7.3 既存経路の不変
- [ ] `cargo check --workspace` pass (engine-perception 含む)
- [ ] `cargo test --lib --no-default-features` で既存 test 全 pass (broadcast 化が既存 destructive `pop()` に影響しない)
- [ ] `npm run build:rs` 成功 (rustup auto-target で OK)
- [ ] vitest 2435 pass / 0 regression (`l1_poll_events` / `l1_get_capture_stats` 経由の既存 test)
- [ ] CI green (`check:rs-workspace` 含む)

### 7.4 北極星制約 (§1.5、本 PR 内で必ず満たす)
- [ ] **N1**: bridge から engine-perception に渡る `FocusEvent` は L1 `event_id` を必ず保持 (`source_event_id` field、§3.2.1 / §3.3.2)。test `forwarded_event_carries_source_event_id` で assert
- [ ] **N2**: timely frontier advance は watermark 経由 (`watermark_for(latest_wallclock, shift)`)、event-time は data field 保持。out-of-order event が watermark 内なら受け入れ、外なら drop (§3.2.4)
- [ ] **N3**: napi entrypoint からの自動起動を入れない (本 PR では `spawn_perception_pipeline_for_test` のみ)。MCP tool surface への露出は D2 以降

### 7.5 docs
- [ ] D1 plan §3 D1-2 sub-batch checklist 全項目 `[x]`
- [ ] views-catalog §3.1 注記更新 (operator graph は D1-3)
- [ ] memory `project_3layer_architecture_design.md` / `MEMORY.md` 更新

### 7.6 D1 plan §11 acceptance のうち本 PR で達成する項目 (本 PR merge 時に flip する到達目標、Codex P3-1 反映)
- [ ] **ADR-007 P5c-1 完了** を前提として real L1 input が ring 経由で bridge → engine-perception に流れる経路が成立 (本 PR merge 時に flip)
- [ ] `src/l3_bridge/focus_pump.rs` が動作 (subscribe → decode → push_focus、event_id 保持) (本 PR merge 時に flip)
