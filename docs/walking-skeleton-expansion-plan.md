...
| 4 | **typed reason 残 49 codes** | `_errors.ts::SUGGESTS` を `try_next: TypedAction[]` に進化、ADR-010 P2 acceptance 100% mapping。残 49 codes のうち lease 経路 4 codes (`LeaseGenerationMismatch` / `EntityNotFound` / `LeaseDigestMismatch` / `EntityOutsideViewport`) は trunk pattern コピー | **4** | 4-5 | ✓ 並走可 (code ごと独立) | なし |
...
- **swimlane 4 (typed reason)**: 残 49 codes mechanical、worktree 並走で 1 週間
...
| 3 | ~~**HISTORY_BUFFER_CAPACITY=8 + run_macro 長 macro evict 挙動** — Step 数が capacity 超過時、outer run_macro event が ring から evict され `desktop_state(include=causal).your_last_action` は最終 step に collapse~~ | `src/tools/macro.ts` (caveat 更新済)、`src/tools/_envelope.ts` (HISTORY_BUFFER_CAPACITY=8 SSOT) | ~~**ADR-011** で compound commit 概念の必要性判断~~ → **Resolved** (ADR-011 Phase A A-3 PR #157 で `ToolCallEvent.isCompoundBoundary` flag + `evictOldestNonBoundary` + `selectLastEventForCausalProjection` 共有ヘルパで closed loop 解消、`runMacroRegistrationHandler` に `isCompoundBoundary: true` 配線、long macro でも outer run_macro event が anchor preserve、land 2026-05-07) |
| 4 | ~~**multi-session sessionId source finalize** — `getMcpTransportSessionId` stub が undefined 返却、`_envelope.ts` で「ADR-011 で完全 finalize」と明記、現状 `multi:disabled` sentinel runtime closed loop で動作~~ | `src/tools/_session-context.ts` (新設、ALS resolver SSOT) + `src/tools/desktop-state.ts` + `src/tools/_envelope.ts` (両 stub を delegate 統合) | ~~**ADR-011** wire 必須~~ → **Resolved** (ADR-011 Phase A A-2 PR #158 で AsyncLocalStorage 経路 + SDK `RequestHandlerExtra.sessionId` ハイブリッドで 2-stub unification、`makeCommitWrapper` / `makeQueryWrapper` を `(rawArgs, extra?)` に拡張して runWithSessionContext で wrap、env mode `DESKTOP_TOUCH_SESSION_MODE=single|multi|auto` (default auto) で transport-aware 判定、land 2026-05-07) |
