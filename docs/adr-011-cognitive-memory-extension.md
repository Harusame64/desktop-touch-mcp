...
- Status: **Drafted v0.2 (Round 1 Opus phase-boundary review findings apply 済、Round 2 review pending)**
- Date: 2026-05-07
...
| **A-2** | sessionId source finalize | `desktop-state.ts:753` (`getMcpTransportSessionId` stub 定義)、`desktop-state.ts:768-793` (`desktopStateGetSessionId` resolver)、`desktop-state.ts:790` (sentinel branch `return "multi:disabled"`)、`_envelope.ts:1570` (default `() => "default"`) | MCP request context (McpServer / RequestContext API) からの session_id 取得経路実装 |
...
| 2 | **A-3: run_macro compound commit boundary** | `ToolCallEvent.isCompoundBoundary` + `evictOldestNonBoundary` + `runMacroRegistrationHandler` flag + unit test | `ToolCallEvent` interface (line 920-943) + `pushHistoryStarted` (line 1016-1047) + `evictHistoryIfNeeded` (line 997-1014) + `CommitWrapperOptions` 拡張 | Opus 必須 + Codex 必須 (production code 改修) |
