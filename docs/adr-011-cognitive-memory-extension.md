...
- Status: **Drafted v0.1 (Sonnet draft、Opus phase-boundary review 未実施)**
- Date: 2026-05-07
...
| 北極星要素 | Phase A 寄与 |
|---|---|
| LLM の不安を消す | `include=["causal"]` を 9 query tool 中 8 で受けても caused_by 空返しの **residual contract gap** を閉じる (A-1)。`your_last_action` が常に正しく projection される |
...
| **A-2** | sessionId source finalize | `desktop-state.ts:753, 786-792` (`getMcpTransportSessionId` stub)、`_envelope.ts:1570` (default `() => "default"`) | MCP request context (McpServer / RequestContext API) からの session_id 取得経路実装 |
...
- HISTORY_BUFFER_CAPACITY SSOT → `src/tools/_envelope.ts:955`
- causedByProjector 既存 reference impl → `src/tools/desktop-state.ts:637-732` (`desktopStateCausedByProjector`)
- sessionId stub SSOT → `src/tools/desktop-state.ts:753, 768-793`
...
ring buffer 状態 (capacity = 8):
    [outer run_macro★, s1, s2, s3, s4, s5, s6, s7]   ← 旧挙動: outer evict、s8/s9 が後ろから
    [outer run_macro★, s2, s3, s4, s5, s6, s7, s8, s9]  ← 新挙動: outer は boundary 保護、step は FIFO evict
                       ↑
...
注意点:
- ring effective capacity は boundary 保護分だけ実質減る (boundary 1 件 → effective = 7)。複数同時 outer (run_macro 内 run_macro) は **本 plan では non-goal** (現行 macro.ts の `TOOL_REGISTRY` から run_macro 除外で recursion 防止済、`macro.ts:509-511` 参照)
...
MCP SDK (`@modelcontextprotocol/sdk`) の `RequestContext` API から session_id を取得する経路を実装。現行 stub `getMcpTransportSessionId()` (desktop-state.ts:753) は無条件 `undefined` 返却で sentinel branch (line 788) は dead code (CodeQL #109、PR #120 で "Won't fix" dismiss、ADR-011 で生かす設計)。
...
`ToolCallEvent` interface (`_envelope.ts:920-943`) に **`isCompoundBoundary?: boolean`** field 追加、`evictHistoryIfNeeded` (line 997-1014) と ring `events.shift()` (line 983, 1045) で boundary entry の eviction を skip する logic 追加。
...
- run_macro `MAX_STEPS = 50` (推定、`macro.ts` 参照) より小さいので、step が capacity を超えるケースは依然発生 — boundary 保護で outer は preserve、step は通常 FIFO
- 複数同時 outer (現行 `TOOL_REGISTRY` から run_macro 除外で発生不可、recursion 防止) は non-goal、検出時は `evictOldestNonBoundary` の fallback FIFO で degraded 動作
...
- unit test: 9-step macro で outer event が ring に preserved + step は FIFO evict、`buildCausedBy` の `your_last_action` が outer event を返すことを pin
...
#### 4.3.8 §1.5 causal continuity 整合確認
...
| **A-3** | 9-step run_macro 後の `your_last_action` が outer run_macro event、step ≤ 8 ケース regression なし、`evictOldestNonBoundary` fallback test pin |
...
|---|---|---|---|
| 1 | **A-1: 8 query tool causal wiring** | `_envelope.ts` extract + 8 file registration wire + unit test | Opus 必須 + Codex 推奨 (production code 改修) |
| 2 | **A-2: sessionId source finalize** | AsyncLocalStorage 注入 + `getMcpTransportSessionId` 実装置換 + env mode + unit test | Opus 必須 + Codex 必須 (production code 改修、API contract 軸重要) |
| 3 | **A-3: run_macro compound commit boundary** | `ToolCallEvent.isCompoundBoundary` + `evictOldestNonBoundary` + `runMacroRegistrationHandler` flag + unit test | Opus 必須 + Codex 必須 (production code 改修) |
| 4 | **A-4: Codex 振り返り review** | PR を作らない (review コメントのみ) | review only、user 判断 trigger |

各 PR は worktree 並走可 (A-1 / A-2 / A-3 は disjoint scope、`_envelope.ts` への touch 重複は merge order で吸収)。
...
各 PR land 時:
- A-1 land → `walking-skeleton-expansion-plan §6.1 #1` resolved + ADR-010 §11 OQ #8 resolved
- A-2 land → `walking-skeleton-expansion-plan §6.1 #4` resolved
- A-3 land → `walking-skeleton-expansion-plan §6.1 #3` resolved + ADR-010 §11 OQ #9 resolved
- A-4 完了 → `walking-skeleton-expansion-plan §6.1 #2` resolved
...
- 既存実装 SSOT:
  - `src/tools/_envelope.ts:903-1100` (history buffer + ring + eviction)
  - `src/tools/_envelope.ts:1530-1700` (CommitWrapperOptions / QueryWrapperOptions / makeQueryWrapper)
  - `src/tools/desktop-state.ts:637-820` (causedByProjector + sessionId resolver + registration)
  - `src/tools/macro.ts:487-540` (run_macro registration + caveat)
...
|---|---|---|---|
| Drafted v0.1 | 2026-05-07 | Claude (Sonnet) | 初稿起草。Phase A scope = expansion phase carry-over 4 項目 (A-1 caused_by wiring / A-2 sessionId source finalize / A-3 run_macro compound commit boundary / A-4 Codex review gap clearance) + ADR-010 §11 OQ #8 #9 wire 完了、Phase B (CoALA 全体) は別 sub-plan。9 PR breakdown + acceptance criteria + risks + OQ + carry-over 解消 mark。Opus phase-boundary review 未実施。 |

---

END OF ADR-011 Phase A Plan (Drafted v0.1).
