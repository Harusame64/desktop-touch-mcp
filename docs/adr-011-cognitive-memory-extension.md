...
- Status: **Drafted v0.4 (Round 3 Opus phase-boundary review findings apply 済、Round 4 review pending)**
- Date: 2026-05-07
...
- `getSessionId` default SSOT → `src/tools/_envelope.ts:1380` (commit-axis、`makeCommitWrapper` 内) + `src/tools/_envelope.ts:1598` (query-axis、`makeQueryWrapper` 内)
...
- 既存実装 SSOT:
  - `src/tools/_envelope.ts:920-1100` (history buffer + ring + eviction、`ToolCallEvent` interface 920 起点 + `_seedHistoryForTest` 983 (test seam) + `evictHistoryIfNeeded` 997-1014 + `pushHistoryStarted` 1045 周辺 (production FIFO))
  - `src/tools/_envelope.ts:1380, 1598` (`getSessionId` default 両 axis)
