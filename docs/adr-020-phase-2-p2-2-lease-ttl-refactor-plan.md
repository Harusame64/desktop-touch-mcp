...
B. **`LeaseStore` に `peekObservedRoundTripMs()` / `commitObservedRoundTripMs()` / `consumeObservedRoundTripMs()` 3 method 追加 (read-once セマンティクス、Opus High #2 + Codex Round 2 P2 反映)**:
   - **`peekObservedRoundTripMs(): number | undefined`** — 副作用なし read。`lastActAtMs` 未 set なら `undefined`、set 済なら `nowFn() - lastActAtMs` を返す (clear なし)
   - **`commitObservedRoundTripMs(): void`** — `lastActAtMs = undefined` で clear。no-op safe (sample なしでも throw しない)
   - **`consumeObservedRoundTripMs()`** — peek + commit composite shorthand。tests / 単純な one-shot caller 向け BC API
   - **production `see()` path は peek + commit 2 段階を使う** (Codex Round 2 P2 反映): see() entry で `peek`、TTL 計算 + output 構築まで成功した後 (return 直前) で `commit`。snapshot 失敗で throw すると関数を抜けるため commit が実行されず、sample は staged 状態で次 see() に持ち越される (`act → 失敗 see → 次成功 see` の wallclock を正しく回収)
   - **read-once が必要な理由**: 「act→next see() の wallclock 差」は **次 see() で 1 回だけ消費**するべき値。`peek` だけだと 1 回 act 後の 2 回目、3 回目の see で古い `lastActAtMs` から測り続け = 「最後に act した時刻からの累積経過」になり、本来の round-trip wallclock 意味から離れる (Opus High #2)。production path で `peek` 後 `commit` を return 直前に置くことで per-see サイクルで 1 回だけ消費、未 act 期間 / failure path で sample 保持を両立 (Codex R2)
   - method naming: `peek*` (副作用なし read) / `commit*` (clear) / `consume*` (composite) — 副作用を命名で明示
...
G. **既存 test 修正**:
   ...
   - 既存 lease-store test (もしあれば) は recordAct / consumeObservedRoundTripMs を mock しない、デフォルト挙動 (= 未呼出 = undefined) で pass 維持
...
H. **新規 test 追加**:
   ...
   - `tests/unit/lease-store.test.ts` (もし存在しなければ新規) に `recordAct` + `consumeObservedRoundTripMs` の round-trip test 追加 (3-4 case)
...
| file | 変更内容 | 推定 line |
|------|----------|----------|
| `src/engine/world-graph/lease-store.ts` | `lastActAtMs` field + `recordAct(viewId)` + `consumeObservedRoundTripMs()` 追加 | +30-40 |
...
| `tests/unit/lease-store.test.ts` | (存在すれば修正、なければ新規) recordAct + consumeObservedRoundTripMs round-trip test | +30-50 |
...
- **`LeaseStore` 新 method (`recordAct` / `consumeObservedRoundTripMs`)**: 追加のみ、既存 method 不変
...
- `LeaseStore.recordAct(viewId)` + `LeaseStore.consumeObservedRoundTripMs()` export + JSDoc 完備
...
- 新規 `lease-store.test.ts` (or 既存修正) `recordAct` / `consumeObservedRoundTripMs` round-trip test 追加
