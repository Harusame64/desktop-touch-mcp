...
| Caret-region masking | **partial** — Stage 4 emits whole-rect SSIM; caret-masking refinement is OQ (§10) | future Stage 4 follow-up |
...
Padding constants are tuned values; the sub-plan locks initial defaults and an OQ records the carry-over for empirical refinement (§10 OQ #2).
...
| **`docs/adr-019-anti-fukuwarai-v3-temporal-motion-observation.md`** | §3 SSOT row correction (`src/image/ssim.rs` → `src/ssim.rs`); §7 OQ #6 (SSIM threshold for `local_repaint`) marked **Resolved** by the locked `RESIDUAL_DELIVERED_FRACTION = 0.05`; §4 Stage 4 deliverables list (matches this sub-plan's §3 table). |
