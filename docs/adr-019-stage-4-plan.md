...
- [x] **P11** — `benches/ssim_residual.mjs` AC6 unit bench. **Result**: scalar p99 = 4.215 ms on 400×400 RGBA frame pair (500 iterations, AVX2 host) — **well within the 15 ms G4-6 budget**, so P12 is carried over (not required).
- [x] **P12** — Optimisation pass: AVX2 + SSE2 runtime dispatch in `src/ssim.rs` (the §4.5 SIMD strategy). **Carry-over** to Stage 4 follow-up — scalar P1 implementation meets G4-6 (p99 = 4.215 ms vs 15 ms budget = 28 % utilisation), so the SIMD optimisation is not required for the impl PR. If post-merge dogfood (P14) on lower-spec hosts uncovers latency regressions, the optimisation lands as a separate PR.
- [x] **P13** — ADR-019 main + ADR-018 §2.6 docs sync. ADR-019 §3 SSOT row corrected (`src/image/ssim.rs` → `src/ssim.rs`); §4 Stage 4 deliverables list aligned with sub-plan §3 table; §7 OQ #6 marked Resolved with locked `RESIDUAL_DELIVERED_FRACTION = 0.05` + `MEAN_SSIM_NO_CHANGE_FLOOR = 0.99`. ADR-018 §2.6 names Stage 4 as the first emitter of `source: "ssim_residual"`.
