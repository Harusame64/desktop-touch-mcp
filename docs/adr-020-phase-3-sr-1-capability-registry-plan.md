...
- **新規 test case 追加** (本 PR scope):
  - `entity.preferredExecutors === undefined` (baseline 経路) で 全 既存 `desktop-executor.test.ts` test 無変更 green (bit-equal pin)
  - `entity.preferredExecutors = ["uia","mouse"]` set した UIA-sourced entity で UIA block entry → UIA throw → mouse rescue で `{kind:"mouse",downgrade:{from:"uia"}}` return (case β + 既存 marker 維持)
  - `entity.preferredExecutors = ["mouse"]` set した UIA-sourced entity で UIA block skip → CDP/terminal eligibility なし → mouse block entry → mouseClick → bare `"mouse"` return (eligibility 限定の意義 = `provider_failed` view bias を executor まで届ける)
  - `entity.preferredExecutors = ["cdp"]` set した UIA-sourced entity (multi-source entity) で UIA block skip → CDP eligibility あり → CDP route (eligibility による block 切替えの例)
...
- **R-SR1-2-e** (Round 7 新設): preferredAllows("mouse") false で mouse block skip → text drop 防止 throw が新 entry 条件で発火しない risk。**対策**: text drop 防止 throw を mouse block の **eligibility check より先に評価** (§5.2 outline で順序固定)、text drop 防止経路 bit-equal 維持。Phase 2 contract test に preferredAllows("mouse") false + text 付き action での throw 文言 pin を追加
