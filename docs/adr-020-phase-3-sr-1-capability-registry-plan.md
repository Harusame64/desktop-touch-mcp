...
- Status: **Drafted (2026-05-17、Round 7 = PR-SR1-2 実装着手中の Opus 設計判断諮問 + User 確定で §5.2 outline 全面書換 (outer loop 破棄 → preferredExecutors block entry eligibility 限定)、累積 30 件 closure、本 Round で responsibility boundary 明確化が SR-1 全体 design SSOT に組込)**
...
3. **downgrade marker 既存 contract 範囲維持** (Round 7 User 確定で「任意 [from → to] 一般化」から責務再定義): PR #332 の **UIA → mouse hand-wired marker は既存 contract 範囲で維持** (`tests/unit/desktop-executor.test.ts` PR #330/#332/#296 で land 済の bare `"keyboard"` return / `{kind:"mouse",downgrade}` shape / 特化 error message を bit-equal pin)。内部 keyboard fallback (UIA setValue → keyboardTypeBg) の marker emit 可否は **OQ-SR5-1 で SR-5 再判断** (§10、exit condition 付き)。`createDesktopExecutor` の fallback は単純な routing ladder ではなく **recovery fallback (UIA route 内 local rescue) + 公開 contract (PR #330/#332 で固定された return shape)** を含むため、SR-1 では「preferredExecutors を block 順序の generic 一般化軸」として導入しない。
...
8. **entity bake invariant** (Round 4 P1-1 新設、case β 構造保証): `desktop.ts:466-471` 周辺で registry.lookup 結果を **entity に bake する経路** が **常に `preferredExecutors` / `unsupportedExecutors` / `fallbackHint` を 1 batch で書き戻し** (現状の `unsupportedExecutors` のみ書き戻しを 3 field に拡張)。bake 不完全 (`preferredExecutors` だけ書き忘れ等) は `provider_failed` bias loss 等の silent drift を再発させるため、PR-SR1-2 で `bakeEntityCapabilities(entity, cap)` 共通 helper として中央化、`desktop.ts:469-471` の inline 書き戻しを helper 呼出に置換、helper 内で **`registry.lookup` から取得した cap の 3 field を必ず同時 bake**。bake invariant 違反は executor 側 `entity.preferredExecutors === undefined` 検出時に baseline 4 block sequential 構造を通る (case β 経路と baseline 経路の bit-equal 動作) + Codex review prompt 雛形 (§5.6) で「bake 漏れがないか callsite grep 確認」を必須項目化。
9. **preferredExecutors の責務は block entry eligibility に限定** (Round 7 User 確定で新設): `entity.preferredExecutors` は `createDesktopExecutor` 内で **各 executor block (UIA / CDP / terminal / mouse) の entry eligibility (skip 条件)** にのみ作用する。**fallback ladder の順序一般化 / generic outer loop / generic error aggregator は導入しない** (元 baseline 4 block sequential 構造を bit-equal 維持)。具体的に:
   - `entity.preferredExecutors === undefined`: 現行 baseline と **完全同一** (skip 条件無効化、全 block 通常通り評価)
   - `entity.preferredExecutors` defined: 各 block の入口で「`preferredExecutors.includes(executor)` か」で entry gate、入った block 内部の fallback / error message / return shape は baseline と **bit-equal 維持**
   - skipped block の failure を集約しない (generic aggregator 禁止)、baseline の sequential 4 block 試行 → 該当なし時の `executor_failed` throw 経路をそのまま継承
   - UIA route 内側の keyboard fallback (UIA setValue → keyboardTypeBg) は bare `"keyboard"` return 維持 (PR #330 contract)、外側に observed kind を伝播しない
   - UIA → mouse downgrade marker (PR #332 hand-wired) は **UIA block 内 catch 節での `{kind:"mouse",downgrade:{from:"uia"}}` return をそのまま維持** (eligibility gate の影響なし)
   - 内部 keyboard fallback の marker emit 一般化は **OQ-SR5-1 で SR-5 再判断** (§10、exit condition 3 案付き)
...
PR-SR1-1 (registry interface + rule table extraction + entity bake helper + type narrow 化、~300-450 line)
   ...
   ↓
PR-SR1-2 (executor entity.preferredExecutors block entry eligibility 限定、~150-250 line、Round 7 で outer loop 設計破棄 → scope 縮小)
   ‖ PR-SR1-3 と並走可能 (Round 3 P3-N2、scope disjoint: PR-SR1-2 = executor 改修 / PR-SR1-3 = description 改修)
   = createDesktopExecutor(target, deps?) signature 不変 (Round 4 P1-1 case β、Round 7 で確定維持)
   = baseline 4 block sequential 構造 (UIA / CDP / terminal / mouse) を **bit-equal 維持** (北極星 9)
   = entity.preferredExecutors を **各 block entry の eligibility check** に組込 (preferredAllows(executor) helper、`entity.preferredExecutors === undefined` で baseline 完全同一)
   = 内部 keyboard fallback bare "keyboard" return 維持 (PR #330 contract、北極星 9 (4))
   = UIA → mouse downgrade marker 既存 contract 範囲維持 (UIA block 内 catch 節 hand-wired、北極星 9 (5))
   = generic outer loop / outer aggregator / 一般化 [from → to] marker emit は **導入しない** (Round 7 User 確定: routing ladder ではなく recovery fallback + 公開 contract 含む)
   = error message は baseline と bit-equal 維持 (北極星 9 (2))、skipped block の failure 集約禁止 (北極星 9 (3))
   = Phase 2 contract test C/E green 維持 + 副作用波 8 件 preventive sweep を Codex review に必須化 (case β + eligibility 限定 design 軸に再整理)
   = 内部 keyboard fallback marker emit 一般化は OQ-SR5-1 で SR-5 再判断 (§10、exit condition 3 案付き)
   ↓
...
## 5. PR-SR1-2 詳細 (executor entity.preferredExecutors block entry eligibility 限定、Round 7 で outer loop 設計破棄)

### 5.1 目的

`createDesktopExecutor` の baseline 4 block sequential 構造 (UIA / CDP / terminal / mouse) を **bit-equal 維持**しつつ、`entity.preferredExecutors` (engine internal field、PR-SR1-1 で新設、`see()` 内で registry.lookup 結果を bake) を **各 block の entry eligibility (skip 条件) としてのみ** consume する (北極星 9)。 `entity.preferredExecutors === undefined` の場合は baseline と完全同一動作。

**Round 4 P1-1 反映**: viewConstraints は executor 引数で渡さず、`see() ↔ registry 間で完結 → entity に bake → executor 経由消費` (case β、Round 2 option (b) signature 拡張案は `_sessionOpts()` executorFactory 固定 lifetime と矛盾で棄却)。`createDesktopExecutor(target, deps?)` signature は不変 (北極星 6)。

**Round 7 User 確定 (本 Round で outer loop 設計破棄)**: Round 4 P1-2 で導入した「outer loop + routeExecutor + observed kind 戻り値 + 任意 [from → to] 一般化 marker emit」設計は、PR-SR1-2 実装着手中の Opus 設計判断諮問 + User 確定で **scope 外と判明**。詳細は §5.2 末尾「なぜ outer loop 設計を採用しないか」参照。本 SR-1 では preferredExecutors を block entry eligibility に限定し、内部 keyboard fallback (UIA setValue → keyboardTypeBg) は bare `"keyboard"` return 維持 (PR #330 contract)、UIA → mouse downgrade marker は既存 contract 範囲 (UIA block 内 catch 節 hand-wired) のみ維持。内部 keyboard fallback marker emit 一般化は **OQ-SR5-1 で SR-5 再判断** (§10、exit condition 3 案付き)。

### 5.2 `createDesktopExecutor` 改修 outline (Round 7 = block entry eligibility 限定、baseline bit-equal)

```ts
// src/tools/desktop-executor.ts 改修 (~150-250 line、Round 7 で scope 縮小)
...
export function createDesktopExecutor(
  ...
  return async (entity, action, text) => {
    ...
    // 既存 unsupportedExecutors 負 gate (baseline 維持)
    ...
    // Round 7 新規: preferredExecutors block entry eligibility check (北極星 9)
    // entity.preferredExecutors === undefined → 全 block に対し true 返却 (baseline 完全同一)
    // entity.preferredExecutors defined → 各 block 入口で includes 判定
    ...
    // ── UIA block ──────────────────────────────────────────────────────────
    // baseline と bit-equal: source check + unsupportedExecutors check + preferredAllows("uia") を AND
    // entry した後の内部 logic (setValue/type → uiaSetValue / 失敗時 keyboardTypeBg → bare "keyboard"、
    // click → uiaClick / 失敗時 mouse rect-center hand-wired downgrade marker) は baseline と完全同一
    if (entity.sources.includes("uia") && !uiaBlocked && preferredAllows("uia")) {
      // ... baseline と bit-equal な内部 logic (UIA setValue→keyboard ladder bare "keyboard" return + UIA click→mouse hand-wired marker) ...
    }
    ...
    if (cdpSelector && !cdpBlocked && preferredAllows("cdp")) {
      // ... baseline と bit-equal (cdpClick / cdpFill、throw 直伝播) ...
    }

    // ── terminal block ─────────────────────────────────────────────────────
    if (entity.sources.includes("terminal") && !terminalBlocked && text !== undefined && preferredAllows("terminal")) {
      // ... baseline と bit-equal (terminalSend、throw 直伝播) ...
    }

    // ── mouse fallback ─────────────────────────────────────────────────────
    // text drop 防止 throw + mouseBlocked / preferredAllows("mouse") false + no rect の各 throw 経路は baseline と bit-equal
    // (preferredAllows("mouse") false 時の throw は新規だが、baseline で mouseBlocked と同等動作)
    if (text !== undefined && (action === "type" || action === "setValue")) {
      throw new Error(/* baseline 文言 bit-equal */);
    }
    if (mouseBlocked || !preferredAllows("mouse")) {
      throw new Error(/* baseline 文言 bit-equal (mouseBlocked と同 throw 経路) */);
    }
    // ... mouseClick (baseline 完全同一) ...
  };
}
```

**重要 design 決定** (Round 7 User 確定):
- **baseline 4 block sequential 構造 bit-equal 維持**: UIA / CDP / terminal / mouse の 4 block 試行順 + 各 block 内部の fallback / error message / return shape は baseline と完全同一 (北極星 9 (2)/(3))
- **`preferredAllows(executor)` helper 1 行追加**: 各 block の entry 条件に 1 つの check 追加、`entity.preferredExecutors === undefined` で baseline 完全同一 (北極星 9 (1))
- **UIA route 内の内部 keyboard fallback**: bare `"keyboard"` return 維持 (PR #330 contract、北極星 9 (4))、外側に observed kind 伝播しない
- **UIA → mouse downgrade marker**: UIA block 内 catch 節での hand-wired `{kind:"mouse",downgrade:{from:"uia"}}` return をそのまま維持 (PR #332 既存 contract、北極星 9 (5))。preferredAllows("mouse") false の場合は UIA block 内 mouse rescue を試行しない (= 元 logic で mouseBlocked 相当)
- **generic outer loop / outer aggregator は導入しない** (Round 7 User 確定)
- **内部 keyboard fallback marker emit 一般化は OQ-SR5-1 へ送る** (§10、exit condition 3 案付き)

#### なぜ outer loop 設計 (Round 4 P1-2) を採用しないか (Round 7 User 確定)

Round 4 で「routeExecutor 戻り値で observed kind 保全 + outer loop で `observedKind !== intendedExecutor` 判定 + 任意 [from → to] marker emit」設計を採用したが、Round 7 で **本 SR-1 scope 外**と確定:

- **現 `createDesktopExecutor` の fallback は単純な routing ladder ではない**: UIA route 内の keyboard fallback は **recovery fallback** (route 内 local rescue、bare `"keyboard"` return)、UIA → mouse は **公開 contract に組込まれた特殊 downgrade marker** (PR #332 hand-wired、`{kind:"mouse",downgrade:{from:"uia"}}` shape)、CDP / terminal block の throw は **block の責務として throw 直伝播** (mouse rescue しない)。これらは「block 順序を generic に組替える」ladder ではなく、それぞれ異なる semantic を持つ。
- **outer loop で全 block fallback を generic 化すると公開 contract 破壊**: PR #330 (`return "keyboard"` bare) / PR #332 (`{kind:"mouse",downgrade:{from:"uia"}}` shape) / PR #296 (特化 error message) で land した既存 contract を test が pin しており、generic outer loop に書換える = 公開 contract 破壊 = CLAUDE.md §3.2 carry-over scope shrink + 強制命令 10 違反。
- **preferredExecutors の責務は「routing 順序の generic 一般化」ではなく「block の eligibility 制御」**: registry が advertise する preferredExecutors は **「この entity に対し試行可能 (= eligibility のある) executor の集合 + 優先 hint」** で、baseline 4 block の試行順序自体を組替えるための入力ではない。entity 内に複数 source (UIA + CDP) がある稀ケースでも、baseline 順 (UIA → CDP → terminal → mouse) に従いつつ「`preferredExecutors` に含まれる block のみ entry」とすることで responsibility boundary を保つ。

= **SR-1 範囲は registry SSOT 化 + entity bake invariant + block eligibility gate のみ**。内部 keyboard fallback marker emit / routing ladder 一般化 / outer loop は SR-5 (`"keyboard"` first-class promotion) で再考すべき軸、本 SR-1 では **scope shrink + responsibility boundary 明確化**を優先。
 ...
 */
~~Round 4 outer loop / routeExecutor / observed kind 戻り値 design は Round 7 で破棄、本 outline block は historical reference として残置~~

### 5.3 副作用波 preventive sweep (Round 7 で eligibility 限定 design に再整理、Codex 必須軸)

memory `feedback_codex_side_effect_wave.md` 軸で「fix が新副作用」を事前列挙。Round 7 で outer loop 設計破棄 + eligibility 限定 design 採用 → 一部副作用波が **構造的に発生不能** (該当ロジック削除)、新規副作用波は preferredAllows check の挙動に集中:

| # | 想定副作用 | 検出経路 | 対策 |
|---|----------|---------|------|
| 1 | `preferredAllows(executor)` check が baseline と異なる動作 (entity.preferredExecutors undefined 時に block を skip する等) | Phase 2 contract test + 既存 `desktop-executor.test.ts` 全 case bit-equal 確認 | `entity.preferredExecutors === undefined ? true : entity.preferredExecutors.includes(executor)` で defensive coding、PR-SR1-2 で eligibility unit test 新設 (preferredExecutors undefined / set 両 case pin) |
| 2 | preferredExecutors defined だが空配列で全 block skip → mouse fallback も skip → throw 経路文言が baseline と drift | **北極星 7 (a) で構造的不能化** (PR-SR1-1 invariant test pin、`lookupDefault` 出力 + `assertCapabilitiesInvariant` で空配列 throw) | PR-SR1-1 invariant が production 経路で空配列 lookup を防止 |
| 3 | preferredAllows("mouse") false で mouse fallback の text drop 防止 throw が発火しない (text + mouseBlocked 同型 path) | Phase 2 contract test の "no text-capable executor" expectation で pin | PR-SR1-2 で `if (text !== undefined && (action === "type" || action === "setValue")) throw` を **preferredAllows("mouse") check より先に評価**、text drop 防止経路 bit-equal 維持 |
| 4 | ~~Round 4 outer loop / errors aggregator 副作用 (lastErr 上書き / generic message drift)~~ → **Round 7 で outer loop 設計破棄により該当ロジック削除**、副作用構造的不能化 | (削除済 outer loop に依存) | (該当なし) |
| 5 | `viewConstraints` 受け渡し問題 → **Round 4 case β + Round 7 eligibility 限定で完全解消**: registry.lookup は see() 内 1 回呼出、entity に 3 field bake、executor は entity 経由のみ消費 (registry/viewConstraints 引数なし、signature 不変) | PR-SR1-2 では executor signature 不変、`bakeEntityCapabilities` helper 経由 bake 済前提 | Phase 2 contract test C/E に `entity.preferredExecutors === undefined / set` 両 case 追加 (PR-SR1-2 acceptance §5.4) |
| **6** (Round 2 P2-5、Round 3 P2-N1 対策修正、Round 7 不変) | `defaultRegistry` module-level singleton の test 並列実行 state 共有 race | **北極星 1 + registry interface JSDoc で「pure lookup、internal state 持たない」invariant 明記** | PR-SR1-1 で `CapabilityRegistry` JSDoc に「pure lookup invariant」明記、constant data は **module-level `const`** (Round 3 P2-N1) |
| **7** (Round 2 P1-2 関連、Round 7 不変) | `preferredExecutors` ↔ `unsupportedExecutors` overlap (`["uia"]` + `["uia"]`) → 全 block skip throw | **北極星 7 (b) で構造的不能化** (`assertCapabilitiesInvariant` で overlap throw、PR-SR1-1 invariant test pin) | PR-SR1-1 invariant unit test で overlap 試行 → assertion throw 確認 |
| **8** (Round 2 P1-1 関連、Round 7 不変) | preferredExecutors に `"keyboard"` 混入 (将来 SR-5 後の型拡張) → 4 block どれも eligibility に該当しない skip | **北極星 7 (c) + AdvertisedExecutorKind type narrowing で構造的不能化** (TS compile-time error) | PR-SR1-2 で `preferredAllows("uia"/"cdp"/"terminal"/"mouse")` は narrow type に対する check のみ、`"keyboard"` 混入は AdvertisedExecutorKind narrow で TS compile-time 防止 |
| **9** (Round 7 User 確定で新設) | preferredAllows check で UIA block を skip した entity が CDP block も skip → mouse fallback 直行で内部 keyboard fallback の機会喪失 (UIA setValue rescue path 消失) | Phase 2 contract test E (`preferredExecutors[0] == "uia"` 固定保証) で pin | PR-SR1-2 で preferredAllows("uia") false 時は UIA block 内の keyboard recovery も skip (= recovery fallback は UIA block 内に閉じる、block skip 時は recovery 機会も失う = baseline の `entity.sources.includes("uia") === false` 経路と bit-equal) |

### 5.4 acceptance (PR-SR1-2、Round 2 P2-4 反映で test case 拡張)

- **`createDesktopExecutor(target, deps?)` signature 不変** (北極星 6 + 北極星 8 entity bake 経由消費、Round 7 確定維持)
- **baseline 4 block sequential 構造 bit-equal 維持** (北極星 9 (2)): UIA / CDP / terminal / mouse の試行順 + 各 block 内部の fallback / error message / return shape は baseline と完全同一
- **`preferredAllows(executor)` helper 追加**: 各 block の entry 条件に 1 つの check 追加、`entity.preferredExecutors === undefined` で baseline 完全同一 (北極星 9 (1))
- **内部 keyboard fallback bare `"keyboard"` return 維持** (PR #330 contract、北極星 9 (4))
- **UIA → mouse downgrade marker 既存 contract 範囲維持** (UIA block 内 catch 節 hand-wired `{kind:"mouse",downgrade:{from:"uia"}}` 維持、北極星 9 (5))
- **generic outer loop / outer aggregator / 一般化 [from → to] marker emit は導入しない** (北極星 9、Round 7 User 確定)
- **新規 test case 追加** (本 PR scope、`tests/unit/desktop-executor-preferred-eligibility.test.ts` で **5 軸 10 case** に拡張、Round 8 P3-1 反映で sub-plan acceptance を実装に追随):
  - **(1) undefined → baseline 完全同一** (2 case): `entity.preferredExecutors === undefined` で UIA source / CDP source 両 case が baseline 経路同等動作
  - **(2) set → block entry gate のみ変化** (4 case): `["uia","mouse"]` UIA succeeds / `["mouse"]` UIA skip → mouse direct / `["cdp"]` UIA-and-CDP multi-source で UIA skip → CDP / `["uia"]` でも CDP-only entity だと throw (no eligible block)
  - **(3) UIA → mouse downgrade marker 維持** (2 case): `["uia","mouse"]` + UIA throws + rect → `{kind:"mouse",downgrade:{from:"uia"}}` 維持 / `["uia"]` + UIA throws + rect なし → throws
  - **(4) 内部 keyboard fallback bare 維持** (1 case): UIA setValue throws + keyboardTypeBg succeeds → bare `"keyboard"` return (PR #330 contract)
  - **(5) generic outer loop なし** (1 case): `["cdp","mouse"]` + CDP throws → CDP error 直伝播 (mouse rescue なし、baseline `non-UIA errors propagated directly` 維持)
- **既存 test 全 green 維持** (CLAUDE.md `feedback_sonnet_test_executor.md` 遵守、既存 test 書換禁止)
- Phase 2 contract test 5 件全 green (`c-executor-downgrade.test.ts` 7 case + `e-uia-fallback-ladder.test.ts` 6 case 維持)
- `npm run build` (tsc clean) + `npm test` (vitest pass、baseline 3622 passed と同等)
- **dogfood 実機 smoke 必須** (PR-SR1-2 merge 前に `notification_show` 後 user 確認、R-SR1-2-a 対策)
- Codex Round 1 で副作用波 9 件 sweep 明示確認 (§5.3、§5.6 Codex prompt 雛形 Round 7 版使用)
...
- **R-SR1-2-a** (Round 7 更新): preferredAllows("uia") false により UIA block skip → CDP eligibility なし → mouse 直行になった entity で実環境 (production smoke / dogfood) 挙動差。**対策**: PR-SR1-2 merge 前に **dogfood 実機 smoke** を `notification_show` 後ユーザー確認 (CLAUDE.md ユーザー環境)
- **R-SR1-2-b** (Round 7 更新): UIA block 内 keyboard recovery (`d.keyboardTypeBg`) は baseline と bit-equal 維持、SR-5 で `"keyboard"` first-class promotion 時に再考。本 SR-1 では touch しない
- **R-SR1-2-c**: Phase 2 contract test C/E が wire-level pin、registry lookup 出力の preferred 集合が registry 内部実装変更で drift する可能性。**対策**: PR-SR1-1 land 済の `lookupDefault` 出力 bit-equal pin (既存 `desktop-capabilities.test.ts` 14 case + 新規 `capabilities-registry-invariant.test.ts` 17 case + Phase 2 contract test 5 件) で機械固定
...
- **R-SR1-2-e** (Round 7 新設、Round 8 P3-2 で message 統一): preferredAllows("mouse") false で mouse block skip → text drop 防止 throw が新 entry 条件で発火しない risk。**対策**: text drop 防止 throw を mouse block の **eligibility check より先に評価** (§5.2 outline で順序固定)、text drop 防止経路 bit-equal 維持。Phase 2 contract test に preferredAllows("mouse") false + text 付き action での throw 文言 pin を追加。**Round 8 P3-2**: `!preferredAllows("mouse")` throw の message を mouseBlocked 経路 (`"mouse fallback also blocked by unsupportedExecutors"`) と **完全統一**、LLM 観測時の log 差分を最小化 (R-SR1-2-e の「mouseBlocked と同経路で扱う」の文字通り解釈、新規 surface message 追加を回避)
...
```
ADR-020 Phase 3 SR-1 PR-SR1-2 review (Round 7 = block entry eligibility 限定 + baseline bit-equal 維持版):

【北極星 9 (Round 7 User 確定): preferredExecutors の責務は block entry eligibility のみ】
- baseline 4 block sequential 構造 (UIA / CDP / terminal / mouse) bit-equal 維持確認
- 各 block 入口の `preferredAllows(executor)` check が `entity.preferredExecutors === undefined` で全 block true (baseline 完全同一) 確認
- generic outer loop / outer aggregator / 一般化 [from → to] marker emit が **導入されていない** 確認 (Round 4 outline 残骸混入防止)
- skipped block の failure 集約禁止 (baseline の sequential throw 経路維持) 確認

【block 内部の baseline bit-equal】
- UIA 内部 keyboard fallback (UIA setValue → keyboardTypeBg) で bare "keyboard" return 維持確認 (PR #330 contract、北極星 9 (4))
- UIA → mouse downgrade marker (UIA click throw → mouse rect-center) が UIA block 内 catch 節で hand-wired `{kind:"mouse",downgrade:{from:"uia"}}` return 維持確認 (PR #332 contract、北極星 9 (5))
- CDP / terminal block の throw 直伝播維持確認 (baseline の `non-UIA errors are propagated directly` test pin、generic mouse fallback しない)
- error message 文言 bit-equal 維持確認 ("no rect for mouse fallback" / "no text-capable executor available" / "mouse fallback also blocked" baseline 文言維持)

【副作用波 preventive sweep (sub-plan §5.3 全 9 件)】
1. preferredAllows check が baseline と異なる動作 → eligibility unit test 新設で baseline 完全同一 pin (副作用波 1)
2. 空配列副作用 (北極星 7 (a) + assertCapabilitiesInvariant で構造防止) (副作用波 2)
3. preferredAllows("mouse") false で text drop 防止 throw 不発火 → text drop check を eligibility check より先に評価 (副作用波 3、R-SR1-2-e 対策)
4. (削除済 outer loop に依存) (副作用波 4 - Round 7 で N/A)
5. viewConstraints 受け渡し → case β + eligibility 限定で完全解消 (副作用波 5)
6. defaultRegistry singleton state race → pure lookup invariant + ADVISORY_TEXT module-level const (副作用波 6)
7. preferred ∩ unsupported overlap → 北極星 7 (b) + assertCapabilitiesInvariant 構造防止 (副作用波 7)
8. "keyboard" 混入 → AdvertisedExecutorKind narrow type + TS compile-time 防止 (副作用波 8)
9. preferredAllows("uia") false で UIA keyboard recovery 機会喪失 → baseline の `entity.sources.includes("uia") === false` 経路と bit-equal、recovery は UIA block 内に閉じる仕様 confirm (副作用波 9、Round 7 新設)

【ExecutorDeps + signature 不変軸】
- ExecutorDeps interface shape 不変 (Phase 2 contract test C/E の mock injection 経路保全)
- createDesktopExecutor(target, deps?) signature 不変 (北極星 6、case β + eligibility 限定で第 3/4 引数追加せず)
...
- Phase 2 contract test C/E が real production fallback path を invoke する設計が保たれているか
  (createDesktopExecutor(target, mockDeps) で entity.preferredExecutors === undefined / set 両 case を invoke)
- PR #332 UIA→mouse downgrade marker wire-level pin (outcome.downgrade.from === "uia") が UIA block 内 hand-wired emit で bit-equal 維持か
- PR #330 内部 keyboard fallback bare "keyboard" return 維持か (test 書換禁止、CLAUDE.md `feedback_sonnet_test_executor.md` 遵守)
- 内部 keyboard fallback marker emit 一般化は OQ-SR5-1 で SR-5 carry-over 確認
...
## 9. Open Questions (Round 7 で OQ #5 を OQ-SR5-1 に統合 + outer loop 依存 OQ 整理)

- ~~**OQ #1** (Round 2 で「option (b) 確定」→ Round 4 P1-1 で option (b) は `_sessionOpts()` lifetime 矛盾で棄却 → **case β entity pre-baked capabilities に再確定**)~~ → **Round 4 Resolved + Round 7 確定**: case β + eligibility 限定で完全 closure。`createDesktopExecutor` signature 不変、entity 経由消費、`bakeEntityCapabilities` 共通 helper で bake 完全性保証 (北極星 8)。`desktop.ts:466-473` の inline 書き戻しを helper 呼出に置換。test 直 invoke 経路は `entity.preferredExecutors === undefined` → baseline 完全同一動作 (Round 7 北極星 9 (1))
- **OQ #2** ~~ (Round 2 P3-3 で削除、親 ADR §11 L9 carry-over に昇格)~~
- **OQ #3**: SR-5 (`"keyboard"` first-class promotion) で keyboard route を block 化する際、PR-SR1-2 で baseline 維持した UIA 内部 keyboard fallback (UIA setValue → keyboardTypeBg) との merge 方針を SR-5 sub-plan 起草時確定。同時に `AdvertisedExecutorKind` 型拡張 (`"keyboard"` 追加) も SR-5 範囲
...
- ~~**OQ #5** (Round 2 P2-2 新規、Lesson 1 causal window 軸、Round 7 で outer loop 設計破棄 + OQ-SR5-1 統合により closure)~~ → **OQ-SR5-1 に統合** (intendedExecutor 概念は outer loop 設計依存で消失、SR-5 で再考すべき軸は「内部 keyboard fallback の marker emit 可否」に集約)

### OQ-SR5-1: Should internal keyboard fallback emit downgrade metadata? (Round 7 User 確定で新設)

**Current decision (本 SR-1 PR-SR1-2)**:
- PR-SR1-2 preserves existing bare `kind: "keyboard"` behavior in the UIA setValue → keyboardTypeBg recovery path
- No downgrade marker is emitted for UIA text → keyboard recovery in SR-1
- UIA → mouse downgrade marker (PR #332) remains as the only hand-wired marker, scoped to UIA block's catch clause

**Reason**:
- Existing PR #330 contract and `tests/unit/desktop-executor.test.ts` cases treat the keyboard fallback as the observed executor, not as a downgrade envelope (bare `kind: "keyboard"` shape)
- Generalizing this in SR-1 would require changing public-ish executor result semantics and error expectations (test rewrites that violate CLAUDE.md `feedback_sonnet_test_executor.md` scope)
- The current executor's fallback is not a single uniform routing ladder — UIA-internal keyboard recovery is a **recovery fallback** with its own contract, distinct from the **block entry eligibility** introduced in SR-1

**Exit condition (SR-5 で必ず判断)**:
SR-5 (`"keyboard"` first-class promotion) は以下のいずれかを **明示的に決定して land** する:
1. **keep bare keyboard as final contract**: 現状維持、`kind: "keyboard"` bare return を「first-class executor の observed kind」として確立、downgrade marker は emit しない
2. **emit downgrade metadata and update contract tests intentionally**: 内部 keyboard fallback でも `{kind: "keyboard", downgrade: {from: "uia", reason}}` を emit、PR #330 contract を意図的に更新 (test 書換は SR-5 内で明示計画として実施)
3. **introduce a separate `fallbackChain` / `attemptedExecutors` field without changing `kind`**: `kind` は bare keyboard 維持しつつ、別 field `fallbackChain: ["uia", "keyboard"]` 等の attempt 履歴を `ExecutorOutcome` に追加、downgrade marker shape を変えずに観測性向上

いずれの選択でも SR-5 sub-plan で **本 OQ-SR5-1 への参照 + exit condition 達成の明示** が必須 (carry-over 化を曖昧に放置しない、CLAUDE.md 強制命令 9 「残件は docs/ に書く」整合)。
...
- ADR-020 §11 **L2 (C)**: PR #332 `TouchResult.downgrade` observability marker → **SR-1 で構造除去** (PR-SR1-2 で PR #332 hand-wired marker を **UIA block 内 catch 節として維持**、registry SSOT 化 + entity bake invariant により「advisory ⇔ execution drift」の構造を `desktop-capabilities.ts` / `desktop-executor.ts` / `desktop-register.ts` 3 SSOT 分裂から **registry 1 SSOT** に集約、`provider_failed` view bias が `bakeEntityCapabilities` 経由で executor まで届く構造を確立。Round 7 確定で「任意 [from → to] 一般化」は scope shrink、内部 keyboard fallback marker emit は OQ-SR5-1 で SR-5 carry-over)
...
- Round 7 反映点 (PR-SR1-2 実装着手後の Opus 設計判断諮問 + User 確定で **outer loop 設計破棄 → block entry eligibility 限定** に design 修正):
  - **背景**: PR-SR1-1 (#340) merged 後 PR-SR1-2 実装着手、Round 4 P1-2 で導入した「outer loop + routeExecutor + observed kind 戻り値 + 任意 [from → to] 一般化 marker emit」 design で `tests/unit/desktop-executor.test.ts` 20 failures → first-eligible 修正で 10 failures に半減、CLAUDE.md 強制命令 4 「Trial & Error 2 回上限」適用で Opus 設計判断諮問 → 案 C+E+G (4 block sequential 維持 + bare keyboard return + bit-equal error message) 確定 → User 確認で「**preferredExecutors の責務を block entry eligibility に限定**」と明文化、本 Round 7 で sub-plan 全面修正
  - **§2 北極星 3 解釈再定義**: 「任意 [from → to] 一般化」 → 「PR #332 hand-wired UIA→mouse marker は既存 contract 範囲で維持 + 内部 keyboard fallback は bare return + 一般化軸は OQ-SR5-1 で SR-5 carry-over」
  - **§2 北極星 9 新設**: preferredExecutors の責務は block entry eligibility のみ、baseline 4 block sequential / error message / bare keyboard return / UIA→mouse marker は bit-equal 維持、generic outer loop / aggregator は導入しない
  - **§3 PR-SR1-2 outline 縮小**: ~250-350 line → ~150-250 line、outer loop 設計を baseline 4 block + `preferredAllows(executor)` helper に縮約
  - **§5.1 / §5.2 全面書換**: outer loop / routeExecutor / observed kind 戻り値 / intendedExecutor 概念を破棄、baseline 4 block + eligibility check の implement outline に再起草、「**なぜ outer loop 設計を採用しないか**」短く追記 (recovery fallback vs routing ladder + 公開 contract 含むため)
  - **§5.3 副作用波**: 1-3 / 9 を Round 7 に再整理 (preferredAllows check 軸)、4 を「Round 7 で削除済 outer loop に依存」N/A 化、5 を「case β + eligibility 限定で完全解消」と更新、6-8 不変
  - **§5.4 acceptance**: 「baseline 4 block bit-equal 維持」「preferredAllows helper 1 行追加」「内部 keyboard fallback bare 維持」「UIA→mouse marker 既存 contract 範囲維持」を明示、新規 test case 4 種 (undefined / set 3 種) 列挙、既存 test 全 green 維持 (CLAUDE.md `feedback_sonnet_test_executor.md` 遵守)
  - **§5.5 risks**: R-SR1-2-a/b/c を Round 7 文言に更新、R-SR1-2-e 新設 (preferredAllows("mouse") false + text drop 防止 throw 順序固定)
  - **§5.6 Codex prompt 雛形**: Round 4 outer loop 軸を Round 7 eligibility 軸に全面書換、副作用波 9 件 sweep + block 内部 bit-equal sweep + contract 真意 sweep の 4 block 構成
  - **§9 OQ #5 廃止 + OQ-SR5-1 新設**: 「intendedExecutor 引数化」OQ は outer loop 設計依存で消失、OQ-SR5-1「内部 keyboard fallback marker emit」を **exit condition 3 案付き** で SR-5 に carry-over (User 明示形式)
  - **§10 L2 文言修正**: 「任意 [from → to] 一般化」から「PR #332 hand-wired を UIA block 内維持 + registry SSOT 化で advisory ⇔ execution drift を構造除去」に意味再定義
- Round 7 累積 closure: Round 1 (11) + Round 2 (4) + Round 4 (6) + Round 5 (3) + Round 6 (1) + Round 7 (5 = §2 解釈 + §3 outline + §5 全面書換 + §9 OQ 整理 + §10 文言修正) = **累積 30 件 closure**
- 次 step: 本 Round 7 反映後 → PR-SR1-2 実装着手 (baseline `desktop-executor.ts` に preferredAllows check 4 箇所追加、~30-50 line 改修 + 新規 test case 4 種追加) → tsc clean + vitest pass + dogfood smoke → commit + push + PR + Opus + Codex review
- Round 8 反映点 (PR #342 Opus Round 1 Conditionally Approved + P2×1 + P3×2 反映):
  - **P2-1** (Lesson 4 numeric count sync 違反): PR #342 description が新規 test を「17 case」と 2 箇所記載するが実 file は 10 case (17 は PR-SR1-1 の `capabilities-registry-invariant.test.ts` 数値の copy-paste 混入)。Round 8 で PR description amend (`gh pr edit`) + sub-plan §5.4 acceptance を「4 種」→「**5 軸 10 case**」に実装追随、commit body にも明記 (P3-1 同時解消)
  - **P3-1** (sub-plan acceptance 実装追随): sub-plan §5.4 acceptance「新規 test case 追加」を 4 種箇条 → 5 軸 10 case 構造化、北極星 9 (1)-(5) 全 sub-rule との対応明示 (case 数 + sub-rule 番号併記)
  - **P3-2** (mouse fallback message 統一、Round 7 「baseline 文言 bit-equal」要請の徹底): `!preferredAllows("mouse")` throw message を mouseBlocked 経路 (`"mouse fallback also blocked by unsupportedExecutors"`) と完全統一、新規 surface message 追加を回避、LLM 観測時の log 差分を最小化。新規 test 自己整合修正 (本 PR で新規追加した eligibility test の regex pattern を統一 message に合わせる、既存 test 書換禁止 (`feedback_sonnet_test_executor.md`) 違反なし)
- Round 8 累積 closure: Round 1 (11) + Round 2 (4) + Round 4 (6) + Round 5 (3) + Round 6 (1) + Round 7 (5) + Round 8 (3) = **累積 33 件 closure**
- 次 step: 本 Round 8 反映後 → commit + push + PR description amend (`gh pr edit`) → Codex review 結果確認 → user dogfood 実機 smoke → PR #342 merge → SR-5 sub-plan 起草へ
