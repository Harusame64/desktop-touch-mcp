...
- `working:N` の N が **N_max (= §10 OQ #1 の決定値) を超えると** typed error `WorkingMemoryNUpperBoundExceeded` (Round 2 Codex P2 反映 — `N_max` は B-1 PR で確定、capacity 50 拡張採用なら 50、capacity 8 維持採用なら 8。`> 50` hard-code は drift)
- **default N (= 5) ≤ HISTORY_BUFFER_CAPACITY (= 8)** で silent truncate なし、N=5 default で ring underflow も発生しにくい
- **HISTORY_BUFFER_CAPACITY 拡張判断 (§10 OQ #1)** は B-1 PR の **最初の決定事項** として PR description に明記、acceptance 内 **N_max 値 + N default 値はその決定で確定**。本 plan land 後の B-1 着手 PR で N_max を **layer-constraints `working memory N 上限` SSOT** に書き戻して確定 (本 plan land 時点では N_max は OQ 状態、`> 50` 等 hard-code は本 plan 内に残さない)
...
- `episodic:N` の N が **N_max_episodic を超えると** typed error `EpisodicMemoryNUpperBoundExceeded` (Round 2 Codex P2 同型反映 — `N_max_episodic` は `layer-constraints` SSOT 既定値 100 だが B-2 着手前に bench で再評価、§8 acceptance + `layer-constraints §5` と sync、`> 100` hard-code は drift)
...
| Phase B 項目 | 完了基準 |
|---|---|
| **B-1** | `include=["working:N"]` 動作、**N <= N_max** (N_max = §10 OQ #1 の決定値、`layer-constraints` SSOT で確定、§4.3 acceptance と sync、Round 2 Codex P2 反映)、envelope size +500B 以内 (default N=5 想定)、unit test 10 case pin |
| **B-2** | `include=["episodic:N"]` 動作、**N <= N_max_episodic** (N_max_episodic = `layer-constraints` SSOT 既定値 100 を B-2 着手前 bench で再評価、§5.3 acceptance と sync)、in-flight skip + completed only、envelope size +1.5KB 以内、unit test 12 case pin |
