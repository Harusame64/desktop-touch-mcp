...
- Status: **Drafted v0.1 (Phase A 完了後の sub-plan、Opus phase-boundary review 未実施)**
- Date: 2026-05-07
...
|---|---|---|---|---|
| **Working memory** | `current_state.recent_events: ToolCallEventSummary[]` (recent N event compact) | `include=working:N` (default N=10) | in-memory only (session-scoped) | **B-1** |
| **Episodic memory** | `tool_call_history.episodes: EpisodeSummary[]` (history buffer expose + outcome 含む rich shape) | `include=episodic:N` (default N=5) | in-memory only (session-scoped、TTL 24h) | **B-2** |
...
- HISTORY_BUFFER_CAPACITY (= L4 内 ring 上限) → `src/tools/_envelope.ts` (grep keyword、現状 8、Phase B-1 で再評価)
...
|---|---|---|---|
| Working (B-1) | `_envelope.ts` 既存 history ring (HISTORY_BUFFER_CAPACITY=8、A-3 boundary 保護下) | session-scoped、新 commit で oldest evict | `sessionId` (`_session-context.ts:getMcpTransportSessionIdFromContext()`) |
| Episodic (B-2) | 同上 ring (extension: outcome field rich shape) | TTL 24h (既存 `HISTORY_BUFFER_TTL_MS`) | `sessionId` |
| Semantic (B-3) | 新 store `src/store/ui-pattern-store.ts` + `~/.desktop-touch-mcp/memory/ui-patterns.json` | LRU + opt-in env var (`DESKTOP_TOUCH_MEMORY_PERSIST=1`) | `sessionId` (default) or `global` (opt-in) |
| Procedural (B-4) | 新 store `src/store/macro-outcome-store.ts` + `~/.desktop-touch-mcp/memory/macros.json` | LRU + opt-in、successful only | 同上 |
...
//   default N = 10、上限 N = 50 (layer-constraints §5 working memory N 上限)
```
...
- `include=["working:10"]` で `envelope.current_state.recent_events` に 10 件 projection (実 ring 件数 < 10 なら全件)
- `working:N` の N が `> 50` で typed error `WorkingMemoryNUpperBoundExceeded` (default 50、layer-constraints SSOT 整合)
- envelope size: `working:10` で +500B 以内 (per-event ~50B × 10)
...
//   永続化 opt-in: ~/.desktop-touch-mcp/memory/ui-patterns.json
```
...
| # | 質問 | 検討タイミング |
|---|---|---|
| 1 | B-1 の HISTORY_BUFFER_CAPACITY (現状 8) を **拡張** するか — Working memory N=10 default が capacity 8 を超える場合の挙動 (= 8 件 cap) は acceptable か、それとも capacity 50 に拡張するか (`layer-constraints §5` SSOT 更新が伴う) | B-1 着手前 |
...
各 PR land 時:
- B-1 land → ADR-010 §6 表更新 + ADR-011 Phase A plan §1.4 の "scope 外" 注記更新 (Phase B B-1 で対応 → Resolved 化)
- B-2 land → 同上 (Episodic 行)
- B-3 land → 同上 (Semantic 行)
- B-4 land → 同上 (Procedural 行)

全 4 layer land で **Phase B v1.0 Completed flip**、ADR-011 全体 (Phase A + Phase B) を Status: Completed v2.0 に flip。
