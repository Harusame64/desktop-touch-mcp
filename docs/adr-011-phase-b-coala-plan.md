...
- Status: **Drafted v0.2 (Round 1 Opus phase-boundary review findings apply 済、Round 2 review 待ち)**
- Date: 2026-05-07
...
|---|---|---|---|---|
| **Working memory** | `current_state.recent_events: ToolCallEventSummary[]` (recent N event compact) | `include=working:N` (**default N=5**、capacity 整合 = 現 `HISTORY_BUFFER_CAPACITY = 8` 内に収まる、§4.3 acceptance + §10 OQ #1 参照) | in-memory only (session-scoped) | **B-1** |
...
- HISTORY_BUFFER_CAPACITY (= L4 内 ring 上限) → `src/tools/_envelope.ts` (grep keyword、現状 8、Phase B-1 で再評価 — **§10 OQ #1 + §4.3 acceptance 参照**、B-1 PR の最初の決定事項)
...
本 plan land と **同 PR で** ADR-010 §6 line 402-409 の表を更新 (CLAUDE.md §3.1 fact 整合):
...
|---|---|---|---|
| Working (B-1) | `_envelope.ts` 既存 history ring (`HISTORY_BUFFER_CAPACITY` = 現状 8、§10 OQ #1 で B-1 着手前再評価、A-3 boundary 保護下) | session-scoped、新 commit で oldest evict (boundary は `evictOldestNonBoundary` で skip) | `sessionId` (`_session-context.ts:getMcpTransportSessionIdFromContext()`) |
...
`_envelope.ts:HISTORY_BUFFER_CAPACITY = 8` の ToolCallEvent ring (= A-2/A-3 で session-scoped に整備済) を **直近 N 件 compact projection** として `desktop_state` envelope に append。
...
//   default N = 5、上限 N = 50 (layer-constraints §5 working memory N 上限)
//   ※ default は HISTORY_BUFFER_CAPACITY (= 現状 8) 以下に絞る (§10 OQ #1 + §4.3 acceptance)。
//      ring 内件数 < N の場合は全件返却 + envelope に `_truncation` notation
//      (ADR-010 §5.6.1 truncation 規約) で「N 要求に対し ring に M 件しか
//      なかった」事実を LLM に明示。silently truncate 禁止。
```
...
- `include=["working:5"]` で `envelope.current_state.recent_events` に **min(N, ring 内件数, HISTORY_BUFFER_CAPACITY)** 件 projection (silently truncate しない、ring 内件数 < N で `_truncation: { requested: N, returned: M, reason: "ring_underflow" | "capacity_cap" }` notation を envelope に付与、ADR-010 §5.6.1)
- `working:N` の N が `> 50` で typed error `WorkingMemoryNUpperBoundExceeded` (default 50、layer-constraints SSOT 整合)
- **default N (= 5) ≤ HISTORY_BUFFER_CAPACITY (= 8)** で silent truncate なし、N=5 default で ring underflow も発生しにくい
- **HISTORY_BUFFER_CAPACITY 拡張判断 (§10 OQ #1)** は B-1 PR の **最初の決定事項** として PR description に明記、acceptance 内 N 値はその決定で確定 (例: capacity 50 拡張なら default N=10 / 上限 N=50 復活、capacity 8 維持なら default N=5 / 上限 N=8 で `_truncation` notation 必須)
- envelope size: `working:5` で +250B 以内 (per-event ~50B × 5)
...
- `episodic:N` の N が `> 100` で typed error `EpisodicMemoryNUpperBoundExceeded` (default 100、layer-constraints SSOT 整合)
...
- 永続化 opt-in: `DESKTOP_TOUCH_MEMORY_PERSIST=1` で `~/.desktop-touch-mcp/memory/ui-patterns.json` に flush、起動時 load
...
| Phase B 項目 | 完了基準 |
|---|---|
| **B-1** | `include=["working:N"]` 動作、N <= 50、envelope size +500B 以内、unit test 10 case pin |
| **B-2** | `include=["episodic:N"]` 動作、N <= 100、in-flight skip + completed only、envelope size +1.5KB 以内、unit test 12 case pin |
...
| # | リスク | 影響 | 軽減策 |
|---|---|---|---|
| 1 | envelope size budget の累積 inflation (4 layer 全 enable で +5KB 以内目標が破れる) | High | 各 layer の N/K 上限を layer-constraints SSOT で固定、bench (`benches/l4_envelope_size.mjs`) で per-PR regression 検出 |
...
- **B-1 land** → ADR-010 §6 表「Working」行を「ADR-011 Phase B (B-1)」に flip + Phase A plan §1.4 / §5 内「Working memory ... scope 外」 を Resolved 化 + `architecture-3layer-integrated.md §17.7` OQ #7 更新
- **B-2 land** → 同型 (Episodic 行)、両 doc + architecture grep
...
各 PR の review checklist で「§11.4 grep sweep を実行 + 全ヒット更新」を必須項目に組み込む (Phase A の §3.1 fact 整合 sweep と同型運用)。
