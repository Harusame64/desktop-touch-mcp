...
- typed reason 50 codes (live `_errors.ts` 38 codes + ADR-added 12 codes、Phase 7 reconcile + F3 SpawnFailed 反映済) → ADR-010 §5.4 + `src/tools/_errors.ts`
...
| 3 | ~~B-4 sharing scope: per-session / global / per-app — どこまで cross-session 共有を許容~~ → **Resolved 2026-05-07** (B-4 land 同 PR): **global cross-session shared store 採用** (`MacroOutcomeStore` module-singleton、session_id key 不使用、LRU 100 globally)。判断根拠: (1) macro pattern (= `tools[]` の順序付き sequence) は **OS UI workflow** = user 個人 PII でなく cross-LLM-client 学習価値、(2) sentinel session (`multi:disabled`) は recordOutcome 経路で **skip** (macro.ts handler tail で `defaultQuerySessionId() !== "multi:disabled"` ガード、A-4 hotfix 同型) + projection 経路で **undefined** return = LLM expose 経路でも cross-session leak 防止、(3) `tools[]` 自体は plaintext で disk persist + LLM expose、redact 対象外 (tool 名は PII でない)。**privacy 観点 risk**: cross-session shared なので user A の使用 pattern (e.g. `["browser_navigate","browser_form","browser_click"]` = フォーム送信ワークフロー) が user B の suggest に出る可能性、persist OFF / on 双方で window あり。filter (success>=3 + failure==0 + no destructive) で safe pattern のみ expose する設計で risk 軽減、初回起動 banner は §10 OQ #9 と統合検討 | ~~B-4 着手時~~ → Resolved 2026-05-07 |
| 4 | ~~B-3/B-4 の永続化 backend: JSON ローカル / SQLite / 別 process pipe — 選定~~ → **B-3 については Resolved (B-3 follow-up PR で **JSON ローカル** 採用、`%USERPROFILE%\.desktop-touch-mcp\memory\ui-patterns.json`、`fs.writeFile` + `fs.rename` atomic write、`PERSIST_SCHEMA_VERSION=1`、5s debounced flush + shutdown immediate flush、land 2026-05-07)。B-4 の選定は B-4 着手時に再判断 (macro outcome は B-3 patterns より volume が大きい場合 SQLite 検討余地あり) | ~~B-3 PR-B 着手前~~ → B-4 着手時 |
...
| 8 | ~~**B-4 destructive macro suggest 防止の判定 mechanism**~~ → **Resolved** (B-4 着手時 user 諮問 2026-05-07): **option (a) tool registry flag 採用** + **query allowlist 設計** (default `isDestructiveCandidate: true`、query-safe tool のみ explicit `false`、新 tool 追加時 flag 立て忘れに対する fail-safe inversion)。`src/tools/_tool-flags.ts` で 11 tool を query-safe 指定: `desktop_state` / `screenshot` / `workspace_snapshot` / `desktop_discover` / `browser_search` / `browser_overview` / `browser_locate` / `browser_form` / `wait_until` / `get_windows` / `get_ui_elements`。**他全 tool は default destructive** (= suggest 対象外)、`scroll` / `focus_window` / `window_dock` も destructive 扱い (UI state 変化)。suggest filter は **success_count >= 3** + **failure_count == 0** + **contains_destructive == false** の 3 条件全 pass (1 回でも失敗があれば suggest しない、95% rate ではなく strict 100%)。**MVP scope は query/observation 系 safe repeated workflow 限定**、destructive macro suggest は Phase B では non-goal、将来 explicit consent UX と別 PR で扱う。`run_macro` 完了時 `macroOutcomeStore.recordOutcome` 経路、`include=["procedural:K"]` で envelope expose、persistence は B-3 と同 JSON backend (`%USERPROFILE%\.desktop-touch-mcp\memory\macro-outcomes.json`) | ~~B-4 着手時~~ → Resolved 2026-05-07 |
...
| 10 | ~~**Memory layer security tier framework — env (operator ceiling) + LLM `include` axis (per-call floor) 二重 axis**~~ → **Resolved 2026-05-07** (B-4 land 後 follow-up PR): **option (b) `parseIncludeMemorySecurity(include)` 別 helper 採用** + **`resolveEffectiveSecurityTier(request, env)` resolver** + **`envelope.security_tier_active` field** で LLM 観測可能化。**実装範囲 (軽量 framework limit per user 諮問)**: tier resolver + envelope field + B-3 redact / B-4 procedural expose 既存 gate に接続のみ。**JSON persistence / storage backend は touch しない** (env-controlled at write time、include は per-query expose mask)。**3 tier 定義**: (1) `memory_strict` = redact ON + persist 表示 OFF + procedural expose OFF (env 設定無視で max security 強制)、(2) `memory_balanced` (default) = env 既定値踏襲、(3) `memory_open` = env ceiling 範囲内 max expose、binary axes では balanced と effective 同等 (将来 graduated axis 追加余地)。**security floor 原則**: LLM は env を open 側へ超えられない (env=redact_ON は include=open で OFF にできない、env=persist_OFF は include=open で ON にできない)、strict 方向のみ env を超えられる (security 原則)。**B-1/B-2 は tier 影響なし** (working/episodic projection は tier 関係なく動作、observable 用途のみ)、B-3/B-4 は既存 gate に effective 値を pass | ~~B-3 land 後の follow-up PR~~ → Resolved 2026-05-07 |
