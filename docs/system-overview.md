...
| `value_not_applied` | a `type` / `setValue` went through the native UI Automation client, the control accepted it, and its value read back unchanged right after the write (measured on a WinForms NumericUpDown); nothing else was tried → do not retry it or type into the same control another way (on that control a keystroke landed at its caret); re-call `desktop_discover` and check the field before writing again. The PowerShell client does not make this check |
