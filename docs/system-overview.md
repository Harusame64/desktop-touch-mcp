...
through to a press. Click the item instead. `type` / `setValue` on a control UI Automation
reports as a button, check box, radio button, hyperlink or menu item is refused the same way
(it used to go out as keystrokes and answer `ok:true`). The lease is validated before execution, and the response carries a
...
| `modal_blocking` | `response.blockingElement` names the blocker. `role: "dialog"`: a separate dialog window has disabled the target's window (read from the OS at the moment of the act, not from the discover snapshot) — `desktop_discover` with `target.hwnd = blockingElement.hwnd`, answer it, then retry. Any other role: a window the discover snapshot holds (a UIA `Window`), used only when the OS could not say — `desktop_discover` with `target.hwnd = blockingElement.hwnd` when `hwnd` is present, otherwise `click_element(name=…)`; then re-discover the original target and act on the new lease (the refusal came from that snapshot, so the same lease is refused again) |
...
| `keyboard_target_unsafe` | nothing was typed: the background write would have gone to a different control or window, or to a read-only control, or the field named (or its window) is disabled (`if_unexpected.detail` names which; for disabled, wait out whatever disabled it and re-discover) → put the focus on the field you named, then type again; `if_unexpected.detail` names the way back for the road the act took (on a window named by title, `desktop_act` with `action='click'` does it; on a window named by handle nothing here focuses a text field yet — re-discover by title, which a common dialog's title does not allow either; for another window, `focus_window` first); never a foreground `keyboard` type. A `type` that took the background write route without being confirmed to have reached the field named answers `ok: true` with `landing: { confirmed: false, why }` — that landing is a report, not a state that can be resolved here, and it does not say the characters were sent (an empty `text` sends none and can still carry one). `DESKTOP_TOUCH_KEYBOARD_RUNG_UNCHECKED=1` turns the check off, and the background write goes wherever the focus is, as before; a comma-separated list of `other_control`, `other_window`, `read_only` and `disabled` turns only those grounds into marked successes |
| `value_not_applied` | a `type` / `setValue` went through the native UI Automation client to a control that is not a text field (not `Edit` or `Document`), the control accepted it, and its value read back unchanged for 300 ms after the write (measured on a WinForms NumericUpDown); nothing else was tried → do not retry it or type into the same control another way (on that control a keystroke landed at its caret); re-call `desktop_discover` and check the field before writing again. The PowerShell client does not make this check |
...
`desktop_act`'s BACKGROUND writes (its UIA / CDP / mouse executors are not instrumented), naming the
channel, the addressed handle and the foreground window at that instant. "Immediately before" is
load-bearing: each event sits at the native call boundary, past every guard, validation and early
return, and a character-payload sink writes nothing when the payload is empty — so the log does not
claim a dispatch that did not happen. Records from one tool call
