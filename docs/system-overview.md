...
| `value_not_applied` | nothing was written: a `type` / `setValue` went through UI Automation, the control accepted it, and its value did not change (measured on a WinForms NumericUpDown) → do not retry it or type into the same control another way (a keystroke lands at its caret); re-call `desktop_discover` and act on the field that holds the text |
