...
- Status: **Drafted (2026-05-17、Round 4 Opus + Codex review 反映)**
...
- **scope outline (Round 4 P1-3 確定)**:
  - **(c) converter 中央集中**: `src/tools/_envelope.ts` 内に `toFailureEnvelope(result: Result<Ok, Err>): Envelope` (仮称) を **唯一の export converter** として配置、`buildFailureEnvelope` の caller を `_envelope.ts` 経由のみに絞る (現状 caller 3 file = `_errors.ts` / `desktop-register.ts` / `_envelope.ts` を 1 file 経由に集約)
  - **(b) handler 最外周 try/catch 共通 pattern**: 各 tool handler (~30 件) の最外周 try/catch を `try { ... return ok } catch (e) { return toFailureEnvelope(toResultErr(e)) }` の共通 pattern で統一、handler 内部は throw / Result.err 両方許容
  ...
  - **実コード status (Round 4 確認済)**: `src/server-windows.ts` + `src/server-linux-stub.ts` の 2 file が MCP server entry、`buildFailureEnvelope` 既存 caller = `_errors.ts` / `desktop-register.ts` / `_envelope.ts` の 3 file (Opus P1-3 元 draft `src/server/*` ディレクトリ誤参照を訂正)
- **影響 PR pin**: G (#329)
- **size 見積**: ~800-1200 line (handler 数が多い、慎重 review 必須、sub-plan で 3-4 PR 分割可能性 R2 参照、§10 OQ #7 で境界諮問)
...
- Phase 2 全 6 contract test green + v1.6.1 fix revert で必ず fail (検出力実証、§4.6 で代表 2-3 件に緩和)
- **Phase 3 全 4 SR land** (SR-1 / SR-5 / SR-2 / SR-4) + Item A dormancy fix revive 完了 + Phase 2 contract test 全 green 維持
...
- 次のステップ: Round 4 改訂を 1 commit でまとめ push → 再 Opus review + Codex re-trigger → P1 ゼロ達成で auto-mode merge (memory `feedback_auto_mode_merge_opus_judgment.md` 準拠)、残存 OQ #7/#8 は SR-2 sub-plan / 後続 epic 起草時諮問
