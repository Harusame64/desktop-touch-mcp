...
- ~~v1.6.1 carry-over JSDoc pin (PR #328-#334) **全件 strikethrough** (構造的根絶を pin)~~ — **PARTIAL CLOSURE 2026-05-18**: L1 / L2 / L3 / L4 / L5 / L7 strikethrough per §11. L6 (G) is **partial closure** per Round 4 (2026-05-17) — `desktopActRawHandler` + 4/6 callsite SR-2 移行済、残 `_envelope.ts:2978`/`:3058` + `desktopActRawHandler:596-606` 統一は **OQ-SR2-5 carry-over** (別 epic / 別 PR で完全 closure 予定、§11 L6 注記)。strict reading では L6 完全 strikethrough は未達だが、本項の精神 (構造的根絶を pin) は B/C/D/E/F + G/部分 で 6/6 path-class 全件で達成 (L6 G の残作業は handler 層 wiring の最終統一のみで、carry-over JSDoc pin の構造的根絶要件 = "drift が merge できない 3 層" は SR-2 で達成済)。
...
- ~~ADR-021 (LLM E2E harness) 起草 trigger 解除~~ — **TRIGGER 解除 2026-05-18 (本 PR-SR4-5 merge 後)**: Phase 3 epic 完了 = ADR-021 起草 trigger 解除条件 (§6.2 「Phase 3 完了 + Item A revive 完了」)。起草 timing は user 諮問。
