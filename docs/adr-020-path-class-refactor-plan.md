...
- Status: **Drafted (2026-05-17、Round 3 user feedback 反映)**
...
| **F** | **(a)** `∀ observedRoundTripMs ≤ cap. computeLeaseTtlMs({...input, observedRoundTripMs}) ≥ observedRoundTripMs` **(b)** `∀ observedRoundTripMs > cap. computeLeaseTtlMs({...}) == cap ∧ response.refreshRequired == true` (cap 整合 + refresh-required marker) | 純粋関数 | **純粋関数 unit test** + property-based (observedRoundTripMs arbitrary を `[0, 2*cap]` で展開、2 branch 両方を覆う) | #328 |
| **E** | (a) `∀ entity ∈ uiaSetValue.advertised. executionPlan(entity).executors[0] == "uia"` (advertise した executor が plan trail に現れる) (b) `∀ failure ∈ ladder. response.fallback OR response.downgrade marker 必須` (silent success 禁止) | observable marker | **table + generated variants** (advertised entity / ladder step 各組合せ) | #330 |
...
**Round 3 feedback §1 (F cap 整合)**: 現実装 `src/engine/world-graph/lease-ttl-policy.ts:23` は **cap 60_000ms** を持ち、`computeLeaseTtlMs` 出力は `clamp(raw, floor, cap)` で頭打ち。元 Round 2 contract `TTL ≥ observedRoundTripMs` だと 90s 入力で必ず fail (正しく fail する)。Round 3 修正:
- **(a) `observedRoundTripMs ≤ cap` 範囲**: 純粋関数 contract `TTL ≥ observedRoundTripMs` を保持
- **(b) `observedRoundTripMs > cap` 範囲**: TTL は cap に saturate + **`refreshRequired: true` marker** を response に乗せる (LLM に「>60s 思考は see() し直し」のシグナル、cap 注釈の docs 通り)
...
- **代表 2-3 件 (D + F + 1 件 user 選定)** で v1.6.1 fix を意図的 revert/diff し対応 test が必ず fail (検出力実証、6 件全部の revert PR は作らず手元 diff 確認 + PR-P2-3 説明に commit hash + 期待 fail 一覧で pin)
...
**warm-up 役の喪失について**: 元 Round 2 で SR-3 を warm-up に置いた狙いは「reviewer の refactor pattern 慣熟」だったが、Phase 2 全体 (PR-P2-1 + PR-P2-2 + PR-P2-3) で D/F 小 refactor + contract test 確立を経るため、Phase 3 SR-1 着手時には refactor pattern は既に十分慣熟されている (warm-up を別途置く必要なし)。
...
#### SR-2: 内部 Result 寄せ + 最外周 boundary central converter (Round 2 feedback §4 反映)

- **目的**: 全 handler の failure path を **内部は `Result.err(typedError)` に寄せる**、**最外周 boundary だけが Result.err と unexpected throw の両方を `buildFailureEnvelope` に変換**。`executor_failed` return path のような silent bypass を構造的に根絶しつつ、throw-only 統一の過剰さを回避。
- **scope outline**: `src/types/result.ts` (新規) で `Result<Ok, Err>` 型 + helper 定義 → 全 handler 内部 control flow を `Result.err(typedError)` に寄せ (`try / catch` で個別 envelope 構築する箇所を削除) → 最外周 boundary (`src/server/*` の tool call entry) で 1 箇所だけ `Result.err` または unexpected throw を catch し `buildFailureEnvelope` を呼ぶ central converter 化 → `executor_failed` の `return` を `Result.err(new ExecutorFailedError(...))` に統一。
- **影響 PR pin**: G (#329)
- **size 見積**: ~800-1200 line (handler 数が多い、慎重 review 必須、sub-plan で 3-4 PR 分割可能性 R2 参照)。
- **修正前 vs 修正後 (Round 2 feedback §4)**:
  - 元 draft: 「全 handler failure path を throw に統一」← throw 統一は逆方向 / 過剰
  - 修正後: 「内部は Result.err 寄せ、最外周 boundary だけが Result.err + unexpected throw 両方を buildFailureEnvelope に変換」← buildFailureEnvelope 唯一 hook 化目的を保ちつつ、内部 control flow の自然さを維持
...
- 5 SR 全 land + Item A dormancy fix revive 完了
...
| R6 | SR-5 (`"keyboard"` first-class promotion) で既存 LLM client が `preferredExecutors: ["uia"]` 既定挙動に依存 | CHANGELOG 警告 (CLAUDE.md 強制命令 10)、既存 envelope shape は backward compatible 維持 |
| R7 | Phase 2 で先行切り出しした `classifyModal` / `computeLeaseTtl` が後続 SR-3 / Phase 3 の方針と矛盾する API shape に固まる | PR-P2-1 review で **SR-1〜SR-5 sub-plan 起草前の API shape 確認** を Opus checklist に追加 (CLAUDE.md §3.1 fact 整合 sweep の拡張) |

---

## 9. Acceptance criteria (epic 全体、Round 2 feedback §5 反映)

- Phase 2 全 6 contract test green + v1.6.1 fix revert で必ず fail (検出力実証)
- Phase 3 全 5 SR land + Item A dormancy fix revive 完了 + Phase 2 contract test 全 green 維持
- v1.6.1 carry-over JSDoc pin (PR #328-#334) **全件 strikethrough** (構造的根絶を pin)
- **本 epic で対象化した path-class (B/C/D/E/F/G) に限り、SSOT / central boundary / contract test の 3 層により silent drift が merge できない** (全 path-class に拡張する強い言い切りは避ける、Round 2 §5)
- ADR-021 (LLM E2E harness) 起草 trigger 解除
...
- ~~**OQ #1**: Phase 3 SR-1〜SR-5 着手順序~~ → **SR-3 → SR-1 → SR-5 → SR-2 → SR-4 で確定** (SR-4 先行は DXGI lifecycle 不確実性が大きく、Phase 2 safety net + 小型 refactor 経験値が入ってから着手)
...
- **OQ #9 (Round 3 新規)**: F refactor で `computeLeaseTtlMs` return shape を `number` → `{ ttlMs, refreshRequired }` に変更すると **全 callsite 移行**が PR-P2-2 scope に入る (現 callsite は数値 return 前提の `expiresAtMs = issuedAtMs + ttlMs` 計算)。callsite 数次第で PR-P2-2 が膨らむ場合、(a) shape は数値維持 + refreshRequired を別 export 関数化 / (b) shape 変更を受け入れ全 callsite 移行、どちらを採るか? PR-P2-2 起草時に callsite grep で再諮問。
...
- [ ] **L1 (B)**: PR #333 `DirtyRectSubscriptionCache.invalidate()` negative-backoff marker + `cacheState` 5-value instrumentation → SR-4 (broker 経由 cache lifecycle 統合) で構造除去
- [ ] **L2 (C)**: PR #332 `TouchResult.downgrade` observability marker → SR-1 (CapabilityRegistry 一本化、downgrade not silent) で構造除去
- [ ] **L3 (D)**: PR #331 `isChromeControlType` helper 共有 → **Phase 2 PR-P2-1** で `classifyModal` 切り出し + 両 callsite 移行 + 旧 2 関数 deprecate で構造除去 (Round 3 OQ #6 で SR-3 を Phase 2 統合確定、Phase 3 SR-3 削除)
- [ ] **L4 (E)**: PR #330 `keyboardTypeBg` internal fallback ladder + `ExecutorKind: "keyboard"` (internal-only) → SR-5 (first-class promotion) で構造除去
- [ ] **L5 (F)**: PR #328 `LEASE_TTL_POLICY.baseMs` 15_000 既定値 → **Phase 2 PR-P2-2** で `computeLeaseTtlMs` input 拡張 (`observedRoundTripMs?`) + cap 整合 2 branch + `refreshRequired` marker 追加で構造除去 (cap 60s は不変、実測 p95-aware 化は telemetry 側責務で本 epic 範囲外、Round 3 §1 反映)
...
- 次のステップ: 本 Round 3 改訂版を chat に表示 → user 目視確認 → ExitPlanMode → 残存 OQ #7/#8/#9 諮問 → Phase 2 PR-P2-1 (D refactor) 着手判断
