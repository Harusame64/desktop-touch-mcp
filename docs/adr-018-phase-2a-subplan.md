...
2. **New helper `parseActionArgsOrFail()` in `src/tools/_envelope.ts`** — the strict-validation gate that the flat wire schema no longer provides:
   - **Signature**: `parseActionArgsOrFail<T>(unionWithInclude: ZodDiscriminatedUnion, args: unknown, toolName: string): T | ToolResult` — `safeParse`s `args` against the **include-injected union** `withEnvelopeIncludeForUnion(<bareUnion>)` (which carries every `.refine()` / `z.literal()` / per-action `required` constraint **and** the `include` field — so the parsed result keeps `include`, no strip). On success, returns the strongly-typed parsed value. On failure, returns a **typed `InvalidArgs` error `ToolResult`** via **`failArgs`** (`_errors.ts:712` — the dedicated validation/input-error path that sets `code:"InvalidArgs"` + `SUGGESTS.InvalidArgs`). **Do NOT use `failWith`** for this path (Codex Round 2 P2): `failWith` falls back to generic classification and would mislabel a per-action schema violation as a generic tool error, degrading the LLM's invalid-argument recovery. `failArgs` returns a `ToolResult` — it does **not** throw; the `ZodError` issues go in the error `context` for LLM-recoverability.
   - Each of the 7 `*DispatchHandler`s calls it **as the first statement, before the `switch`**: `const parsed = parseActionArgsOrFail(scrollUnionWithInclude, args, "scroll"); if (isToolResult(parsed)) return parsed; switch (parsed.action) {…}`. **Ordering invariant**: it runs *inside the innermost handler* — the `makeCommitWrapper`/`withRichNarration` wrappers have already seen the raw `args` (their `windowTitleKey` extraction is unaffected), and the typed-error `ToolResult` flows out through the handler's normal return path (the wrappers do not need to catch a throw).
   ...
   - Unit-tested in `tests/unit/flatten-union-schema.test.ts` — "per-action-invalid input (e.g. `scroll({action:'capture'})` with no `windowTitle`; `terminal({action:'run'})` with neither `input` nor `command`; `keyboard({action:'sequence', method:'background'})`) is rejected as a **typed** `ToolResult` error" + "a valid call with `include` set returns a parsed value that still has `include`."
...
| `src/tools/terminal.ts` | `terminalRegistrationSchema` → flatten (`.refine()` unwrap + nested `until` union intact); `terminalDispatchHandler` → `parseActionArgsOrFail(terminalUnionWithInclude, …)` + update the now-stale "unreachable thanks to schema-level refine" comment to reference the re-parse |
