...
1. **New helper `flattenUnionToObjectSchema()` in `src/tools/_envelope.ts`** (adjacent to `withEnvelopeIncludeForUnion`):
   - **Signature**: `flattenUnionToObjectSchema(union: ZodDiscriminatedUnion): ZodObject` — takes a discriminated union (already passed through `withEnvelopeIncludeForUnion`), returns a flat `z.object` (the **wire schema** — what `tools/list` shows; NOT the validation gate).
   - **Algorithm**:
   ...
   - Unit-tested in `tests/unit/flatten-union-schema.test.ts` (new) — one case per collision class (§2.2) + a round-trip "the flat wire schema accepts every input the real union accepts, i.e. is strictly *looser*, never rejects a previously-valid call."
...
2. **New helper `parseActionArgsOrFail()` in `src/tools/_envelope.ts`** — the strict-validation gate that the flat wire schema no longer provides:
   - **Signature**: `parseActionArgsOrFail<T>(realUnion: ZodDiscriminatedUnion, args: unknown, toolName: string): T` — `safeParse`s `args` against the **real `z.discriminatedUnion`** (`scrollSchema`, `terminalSchema`, …, which carry every `.refine()` / `z.literal()` / per-action `required` constraint). On success, returns the strongly-typed parsed value. On failure, returns/throws a **typed error** (via `failWith` / `_errors.ts` — never a raw `Error`); the `ZodError` issues go in the error `context` for LLM-recoverability.
   - Each of the 7 `*DispatchHandler`s calls it **at the top, before the `switch`**: `const parsed = parseActionArgsOrFail(scrollSchema, args, "scroll"); switch (parsed.action) {…}`.
   ...
   - Unit-tested in `tests/unit/flatten-union-schema.test.ts` — "per-action-invalid input (e.g. `scroll({action:'capture'})` with no `windowTitle`; `terminal({action:'run'})` with neither `input` nor `command`; `keyboard({action:'sequence', method:'background'})`) is rejected as a **typed** error."

3. **All 7 tools' `registerTool` `inputSchema` switched** to `flattenUnionToObjectSchema(withEnvelopeIncludeForUnion(<union>))` (`scrollRegistrationSchema` / `keyboardRegistrationSchema` / `excelRegistrationSchema` / `browserEvalRegistrationSchema` / `windowDockRegistrationSchema` / `terminalRegistrationSchema` / `clipboardRegistrationSchema`), **and** each `*DispatchHandler` updated to call `parseActionArgsOrFail` at its top (deliverable 2).
   - **The real `z.discriminatedUnion` (`scrollSchema` etc.) source definitions are retained unchanged** — they stay the `*Args` type source AND become the argument to `parseActionArgsOrFail`. What changes: (a) `registerTool`'s `inputSchema` arg, (b) each `*DispatchHandler` gains the re-parse call. **This IS a scoped handler change** — the Draft's "no handler logic change" was wrong.
   - `terminal`'s `terminalDispatchHandler` has a manual `throw new Error("…neither input nor command…")` guard commented "Should be unreachable thanks to the schema-level refine" (`terminal.ts:1690-1695`). After Phase 2a it stays unreachable — but now because `parseActionArgsOrFail(terminalSchema, …)` runs the `.refine()` first. Update that comment to reference the re-parse, not the registered schema.

4. **`src/tools/macro.ts` — `TOOL_REGISTRY` path made consistent.** `macro.ts` does `entry.schema.parse(params)` and currently sets `entry.schema` to the discriminatedUnion (strict). Set `TOOL_REGISTRY[*].schema` for the 7 tools to the **flat wire schema** (same instance as `registerTool` — PR #112 shared pattern). Strict validation for `run_macro` is then the in-handler `parseActionArgsOrFail` (deliverable 2), identical to the `server.tool` path — no validation asymmetry between the two entry points. (`feedback_tool_registry_include_strip.md` axis: verify the `include` field still survives the flat schema on the registry path.)
...
| `src/tools/scroll.ts` | `scrollRegistrationSchema` → `flattenUnionToObjectSchema`; `scrollDispatchHandler` → `parseActionArgsOrFail(scrollSchema, …)` at top; `direction`/`windowTitle` description updates |
...
| `src/tools/terminal.ts` | `terminalRegistrationSchema` → flatten (`.refine()` unwrap + nested `until` union intact); `terminalDispatchHandler` → `parseActionArgsOrFail(terminalSchema, …)` + update the now-stale "unreachable thanks to schema-level refine" comment to reference the re-parse |
...
- [ ] `terminal` flattened (structural-wrinkle tier — `.refine()` unwrap, `until` nested union verified) + `parseActionArgsOrFail` wired + stale comment updated
