...
| Block motion vectors napi | **no** (deferred) | yes (only when `finalChangedFraction` saturates) |
...
- [ ] **P2** — `src/tools/_input-pipeline.ts:postWheelToHwnd`: when chain-trust branch emits `observation.motion === "no_change"`, return `null` instead of the `delivered_via_postmessage` outcome. Caller's existing fall-through emits `target_unreachable`. **Wire the existing observation into the null-return caller path** (the dispatcher's `null` return loses the observation in the current architecture — Stage 2b must surface the `no_change` observation on the `target_unreachable` outcome; investigate `DispatchOutcome` shape or thread observation through a sibling channel like `mouse.ts:scrollHandler.ts`'s outcome wiring).
...
- [ ] **P4** — `docs/adr-018-input-pipeline-3tier.md` §2.6.2: add Stage 2b emission row note for `target_unreachable`.
...
- [ ] **P7** — CLAUDE.md §3.1 sweep: grep `"present iff"` across ADR-019 / ADR-018 / src/ / tests/ to confirm Option A's docstring relaxation is consistently applied.
...
- **R3 — `DispatchOutcome` shape doesn't surface observation on `null` return**: the `postWheelToHwnd` function returns `DispatchOutcome | null`; today `null` means "fall through to next tier" and the observation is lost. Stage 2b's `motion: "no_change"` decision must surface the observation on the `target_unreachable` outcome (otherwise LLM sees `status: "not_delivered"` with no `observation` field — same as a destination-resolution failure, ambiguous with TMOL-observed silent drop). **Mitigation**: Stage 2b impl PR investigates threading observation through a sibling channel (e.g. extend `postWheelToHwnd` return type to `{ outcome: DispatchOutcome | null; observation?: VisualMotionObservation }`) or attach observation directly on `mouse.ts:scrollHandler`'s `target_unreachable` emission point via a shared closure / context. **This is the load-bearing implementation detail of Stage 2b** — sub-plan flags it as P2 priority for impl PR.
...
4. **Stage 2c trigger** — when does Stage 2c start? **Decision criteria**: at least one of (a) a new chain-trust target's dogfood shows `finalChangedFraction` saturation (no separation between real-scroll and idle), OR (b) a new chain-trust target shows `stableReached: false` rate > 10 % despite real user-visible motion, OR (c) user / operator report of a silent-drop case Stage 2b missed. Until any trigger fires, Stage 2c is dormant.
...
- **Round 0 (this PR, 2026-05-16)** — initial draft. Decisions §0.1 #1-6 locked via Stage 2a PoC + dogfood evidence. Carry-over of Stage 2c trigger criteria + `compute_block_motion_vectors` deferral + ADR-019 §2.1 `shift?` docstring relaxation.
- **Round 1+** — Opus / Codex review iteration TBD.
