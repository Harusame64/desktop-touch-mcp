...
- `visual_provider_unavailable` → 200-500ms 後に retry（G4 対象）または structured lane で継続
- `visual_provider_warming` → 200-500ms 後に retry（G4 対象、`unavailable` と同様）または structured lane で継続
- `uia_provider_failed` / `terminal_provider_failed` → V1 tools で代替

注: `ok:true` でも `diff[]` に `focus_shifted` / `modal_appeared` 等が含まれる場合がある。これらは失敗ではなく post-touch の観測シグナル。
...
visual_provider_unavailable / visual_provider_warming → retry once (~200ms) then continue with structured lane;
