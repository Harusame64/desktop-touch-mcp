...
- typed reason 63 codes (PascalCase、live `_errors.ts` SUGGESTS 51 codes [39 baseline + 12 ADR-015 Phase 4] + ADR-added 12 codes、Phase 7 reconcile + F3 SpawnFailed + Issue #245 ImeOnDuringType + ADR-015 Phase 4 VBA bridge 累積) → §5.4
...
本 §5.4 catalog は live `src/tools/_errors.ts` SUGGESTS dictionary (**51 codes**、Phase 6 cleanup 後 = PR-A 削除 3 件 + PR-B AutoGuardBlocked 追加 1 件 + post-issue #178/#179/#180/#181/#197/#207 + Phase 5 I1 + Phase B B-1〜B-4 + Phase 7 F3 SpawnFailed + Issue #245 ImeOnDuringType + ADR-015 Phase 4 12 codes 累積) を **bit-equal で全反映** + 本 ADR で追加する 12 codes を列挙して、**合計 63 codes** を定義する。Phase 7 catalog reconcile (`docs/adr-010-followups.md` §3) で従来 ADR baseline 視点 subset (23 codes) と live (37 codes) の数値乖離を解消、live SSOT を直接吸収した。Phase 7 F3 fix (Phase 6 dogfood で発見) で workspace_launch spawnDetached 失敗が typed `SpawnFailed` enum に昇格、+1 code。Issue #245 (keyboard type IME-on detection) で `ImeOnDuringType` 追加、+1 code。ADR-015 Phase 4 (v1.5.0) で VBA Extensibility bridge 12 typed errors (8 crate-level + 3 napi-binding-level + 1 TS-binding-only) 追加、+12 codes。

#### live `_errors.ts` SUGGESTS 全 51 codes (PascalCase 維持、SSOT — 39 baseline + 12 ADR-015 Phase 4)
...
ScopeNotFound | BrowserClickNotDelivered | BrowserFillNotDelivered
...
VirtualScrollExhausted | ScrollNotDelivered
...
BackgroundInputNotDelivered | BackgroundKeyNotDelivered |
ForegroundRestricted | ClipboardWriteNotDelivered |
MouseClickNotDelivered | MouseDragNotDelivered |
SetValueAllChannelsFailed | FocusLostDuringType

// IME (Issue #245 — keyboard({action:'type'}) forceKeystrokes against IME-on target)
ImeOnDuringType
...
合計 39 + 12 (ADR-added) + 12 (ADR-015 Phase 4) = **63 codes** (live `_errors.ts` SUGGESTS 51 codes + 本 ADR-added 12 codes、Phase 7 reconcile + F3 SpawnFailed + Issue #245 ImeOnDuringType + ADR-015 Phase 4 12 typed errors で full sync 達成)。
...
| `include=episodic:N` (N=5 default、上限 N=100、ADR-011 B-2 land) | +N×0.3KB 以内 (rich shape: lease_token + event_id + elapsed_ms、N=5 で +1.5KB 以内) | (差分 KPI) |
...
### 10.1 Walking skeleton expansion phase 完了状態 (2026-05-03)

P1 (全 tool envelope 形式応答) のうち expansion phase (PR #126-#146) で **28 public tool 全て** が L5 envelope に統一適用済 (commit 19 + query 9)。`include=["envelope"]` per-call opt-in も 28 entry 全 stub-tool-catalog 行で揃い (`src/stub-tool-catalog.ts` で `Optional response-shape opt-in` description × 38 occurrence、4 query tool で discriminatedUnion variant 込み)。

**Review gap notation** (CLAUDE.md §3.3 Step 2 整合): PR #141-#146 (browser_locate / browser_search / wait_until / workspace_snapshot / server_status / run_macro) の 6 PR は Codex usage limit (2026-05-03) 中の merge につき Opus 単独 review。`memory/feedback_pr_review_loop_merge_criteria.md` 「Codex 応答なしなら Opus 単独 OK」基準に従って merge 可と判定したが、Codex usage 復帰後に振り返り review 機会を ADR-011 着手前に設けるか user 判断 (`docs/walking-skeleton-expansion-plan.md` §6.1 #2 参照)。
...
| 8 | ~~**`include=causal` を 9 query tool 中 8 で受けても caused_by が空で返る挙動の所属 phase** — `desktop_state` 1 件のみ caused_by 配線済 (`src/tools/desktop-state.ts:817-818`)、残 8 query tool (browser_overview / browser_locate / browser_search / screenshot / server_status / wait_until / workspace_snapshot / desktop_discover) は `causedByProjector + getSessionId` 両 omit の S4 fast path (`src/tools/_envelope.ts:1587-1589`)~~ | ~~ADR-011 着手前~~ → **Resolved** (ADR-011 Phase A A-1 PR #156 で 8 query tool 全配線、`genericQueryCausedByProjector` を `_envelope.ts` に extract + `defaultQuerySessionId` 経由で wire、residual contract gap 解消、land 2026-05-07) |
| 9 | ~~**HISTORY_BUFFER_CAPACITY=8 + run_macro long macro (≥8 steps) の causal envelope 挙動** — outer run_macro event が ring から evict され `desktop_state(include=causal).your_last_action` が最終 step に collapse する挙動を §1.5「causal continuity」原則と整合と判定するか、ADR-011 で **compound commit** 概念導入を検討するか~~ | ~~ADR-011 着手時~~ → **Resolved** (ADR-011 Phase A A-3 PR #157 で `ToolCallEvent.isCompoundBoundary` flag + `evictOldestNonBoundary` + `selectLastEventForCausalProjection` 共有ヘルパで closed loop 解消、`runMacroRegistrationHandler` に `isCompoundBoundary: true` 配線、`buildCausedBy` / `buildBasedOn` 両者で boundary 優先 + 完了済 + 末尾 fallback 選択、long macro でも outer run_macro event が anchor preserve、land 2026-05-07) |
| 10 | **`workspace_snapshot` 副作用の causal/working memory invisibility** (A-4 retrospective Codex P2 #3、PR #144 follow-up) — `workspace_snapshot` は query 分類だが handler 内で `clearLayers()` / `noteInvalidation()` を呼んで screenshot diff baseline (I-frame) と identity tracker を update する副作用を持つ。Phase B Working/Episodic memory expose 経由でも「baseline reset」操作が記録されない。**選択肢: (a) `makeCommitWrapper` 化** (Tool Surface 不変原則 read-only orientation を破る) **/ (b) `clearLayers` / `noteInvalidation` を別 explicit reset tool に分離** (workspace_snapshot 純 read-only 化) **/ (c) Phase B Working memory に `side_effect: "baseline_reset"` field 追加** で causal trail visible 化 | Phase B B-2 (Episodic memory) 着手時 or 別軸独立 PR、A-4 hotfix では caveat 強化のみで carry-over (`src/tools/workspace.ts` registration site docstring + L317 caveats 強化済) |
