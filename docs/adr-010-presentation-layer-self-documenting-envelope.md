...
| 9 | **HISTORY_BUFFER_CAPACITY=8 + run_macro long macro (≥8 steps) の causal envelope 挙動** — outer run_macro event が ring から evict され `desktop_state(include=causal).your_last_action` が最終 step に collapse する挙動を §1.5「causal continuity」原則と整合と判定するか、ADR-011 で **compound commit** 概念導入を検討するか | **ADR-011 着手時** (walking skeleton expansion phase 完了 2026-05-03 で発見、`src/tools/macro.ts:498-505` caveat 記述済、`src/tools/_envelope.ts:955` HISTORY_BUFFER_CAPACITY=8 SSOT、`docs/walking-skeleton-expansion-plan.md` §6.1 #3) |
