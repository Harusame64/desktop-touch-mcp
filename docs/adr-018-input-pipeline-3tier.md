...
| `delivered_via_postmessage` | Tier 3 succeeded by **either**: (i) `GetScrollInfo` confirmed a position change of ≥ `POSTMESSAGE_SCROLL_DELIVERY_EPSILON_NPOS` (the legacy Win32 SB_VERT path used by apps with a standard scrollbar); **OR** (ii) the post target was resolved via `SCROLL_LEAF_CHAINS` (Excel `XLMAIN→XLDESK→EXCEL7`, Word `OpusApp→_WwF→_WwG`) AND a dHash diff over the leaf's window rect (Hamming distance ≥ `POSTMESSAGE_CHAIN_TRUST_HASH_THRESHOLD`) confirmed visual movement. When chain-trust applies but dHash capture is unavailable (mixed-version `.node` build, capture call failed), the dispatcher falls back to the bare chain-table assertion (PostMessage queued + receiver in table → presumed delivered) — the table-listed leaves are documented scroll receivers that use custom-painted scrollbars (`NUIScrollbar` etc.) that `GetScrollInfo(SB_VERT)` cannot observe. Phase 5+N chain-trust path with PR #308 dHash verification follow-up; see `docs/adr-018-phase-5-followup-leaf-walker-subplan.md` §2.1. | `delivered` |
...
| `target_unreachable` | Either (a) `InputDestination.kind === 'unresolved'` AND Tier 4 SendInput produced no observable delta — emits `channel='send_input'`; **OR** (b) destination resolved but every applicable tier (1/2/3) was exhausted without observable delta, including the chain-trust dHash verification: a chain-table leaf whose pre/post dHash diff is BELOW threshold (`< POSTMESSAGE_CHAIN_TRUST_HASH_THRESHOLD`) emits `target_unreachable` because PostMessage queued but the receiver did not act (boundary / non-scrollable content) — Codex PR #308 P1 guard. Non-chain-table resolved HWNDs whose Win32 scrollbar is null also fall here (Word `_WwG` only when reached without chain-walker retarget, or any custom-paint HWND not in the table). Emits `channel` ∈ {`uia`, `cdp`, `postmessage`} reflecting the last-attempted tier. Tier 4 SendInput is **not** invoked in path (b). | `not_delivered` |
