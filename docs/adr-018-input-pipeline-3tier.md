...
| `tests/unit/flatten-union-schema.test.ts` | **new** | `flattenUnionToObjectSchema` + `parseActionArgsOrFail` unit tests — collision classes + "flat wire schema is strictly looser than the real union" + "re-parse rejects per-action-invalid input as a typed error" (Phase 2a) |
...
- `parseActionArgsOrFail(realUnion, args, toolName)` in `_envelope.ts` — `safeParse` against the real `z.discriminatedUnion`, returns a **typed error** on failure (per §2.5.2). This is the strict-validation gate that the flat wire schema no longer provides.
...
- `tests/unit/flatten-union-schema.test.ts` — helper unit tests incl. "the flat wire schema is strictly looser than the real union" and "`parseActionArgsOrFail` rejects per-action-invalid input as a typed error".
