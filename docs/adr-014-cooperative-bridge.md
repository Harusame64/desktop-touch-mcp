...
- Related:
  - ADR-013 (`docs/adr-013-wt-bg-input.md` v1.4.5、§3.6 Option F outline + §7 OQ #9 を本 ADR で本実装化)
...
| 実装規模 (推定) | 中 (`src/win32/foreground_flash.rs` ~830 line + test、本 PR 完了) | **大 (helper module 配布 + protocol + lifecycle + fallback、4-6 週間)** |
...
- **fallback**: handshake 失敗 / timeout (5s) は `cooperative_bridge_handshake_failed` で fail、§3.9 fallback ladder へ degrade

### 3.8 Nonce 管理 lifecycle (Option F-A 内 sub-decision)

- **生成**: helper 起動時に 16-byte cryptographically random nonce を生成、`%USERPROFILE%\.desktop-touch-mcp\bridges\<nonce>.json` に discovery file 書き出し (`{nonce, helperPid, pipeName, startedAt, version}`)
...
| 実装規模 | 中 (4-6 週間) | 中-大 | 大 (proto + grpc dep) | 大 (COM apartment) |
...
- [ ] **fallback ladder 設計 confirm**: §3.9 のロジックを TS 側 design doc (`docs/adr-014-phase0-spike.md` で本 ADR とは別 file) に書き出し、Phase 1 へ baseline 提供

### 5.1 Phase 1 acceptance (POC 実装)

- [ ] **helper module 本実装**: F-A-1 (npm separate package) baseline + F-A-3 fallback (Phase 0 結果次第で確定、本 phase は baseline 1 候補で focused)
- [ ] **DTM MCP layer 実装**: `src/engine/cooperative-bridge.ts` + `src/engine/background-channel-resolver.ts` の `cooperative_bridge` variant を currently-narrow-rejected → live channel に flip、`injectViaCooperativeBridge` 実装
...
- [ ] **WT default-on E2E pass**: `tests/e2e/cooperative-bridge-verification.test.ts` で 100 連続 BG injection 全 success (flaky < 1%、helper 起動済 pane 限定で実行)
- [ ] **stress test**: 連続 1000+ injection は別 env / 別 flag で実施、`benches/adr014_cooperative_bridge_throughput.mjs` で latency / throughput 計測
...
5. **`cooperative_bridge_strict` method の API surface**: §3.9 で言及の "fallback 一切なし" variant、別 method として独立させるか `method:'cooperative_bridge'` + `allowFallback:false` option にするか Phase 1 で確定 (memory `feedback_clipboard_flash_design_pitfalls.md` の "契約名は spec 違反 vector の第一防御層" 整合、強い分離は別 method 派、option 1 つで済むなら field 派)
...
| 2. Phase 0 — Protocol design spike + 配布方式実機評価 | 1-2 週間 | spike branch + `docs/adr-014-phase0-spike.md` + 5 環境 helper 起動成功率 + protocol format 確定 | §5.0 acceptance 全 ✓ |
...
- ADR-013 (`docs/adr-013-wt-bg-input.md` v1.4.5、§3.6 Option F outline + §7 OQ #9)
...
- `microsoft/terminal#9368` (Microsoft Reject、"arbitrary process → arbitrary WT" security objection、本 ADR は opt-in helper + nonce + ACL で構造的回避)
- `microsoft/terminal#20106` (laffo16 PR、Option D close 済、本 ADR scope 外)
