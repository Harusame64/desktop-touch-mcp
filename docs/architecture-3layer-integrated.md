...
`desktop_act` の9つの拒否は `desktop-register.ts` が**理由コードから文面を作り直す**ため、engine が持っていた
...
| 7-B2 | ~~episodic memory shape (compact vs rich)~~ → **Resolved** (ADR-011 Phase B B-2 で rich shape 採用、`EPISODIC_MEMORY_DEFAULT_N = 5` / `EPISODIC_MEMORY_N_MAX = 100`、tool_call_history field 確定、land 2026-05-07) | — | ADR-010 + ADR-011 Phase B |
| 7-B3 | ~~semantic memory 抽出 rule~~ → **Resolved** (ADR-011 Phase B B-3 で rule-based 抽出採用、`SEMANTIC_MEMORY_DEFAULT_K = 3` / `SEMANTIC_MEMORY_K_MAX = 10`、in-memory LRU 100、`learned_ui_pattern.patterns` field 確定、永続化 + redact 実 hash + security tier framework は B-3 follow-up carry-over、land 2026-05-07) | — | ADR-010 + ADR-011 Phase B |
