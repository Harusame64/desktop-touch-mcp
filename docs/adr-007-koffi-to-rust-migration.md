...
| **P4: sensors + 撤去** | sensors-win32.ts、P3 残留 5 utility (`GetWindowHwnd` / `GetAncestor` / `IsWindowEnabled` / `GetLastActivePopup` / `DwmGetWindowAttribute`) + `_dwmapi` ロード、koffi 依存削除、launcher zip size 計測、`package.json` から koffi 削除、Linux stub 維持 | 0.5-1 週 | **`git grep "koffi\\."` 行数 0**、koffi 行数 33→0、zip size 削減量を memory に記録 |
...
#### 6.0 P1 status follow-up (2026-04-29)

P1 (Hot path 高頻度系) は本 ADR §10 Opus review §10.6 で「bench は本 PR 内」と確定。`scripts/bench-enum-title.mjs` で post-migration baseline を採取済 (p50 0.24ms / p99 0.50ms、451 windows / iteration)。「半減」比較は P4 で koffi 完全撤去後に koffi vs windows-rs 並列計測で確定する (P4 acceptance に持ち越し)。

P1 で **本 ADR §3.4 acceptance のうち**「panic-fuzz CI でプロセス全滅 0 件」のみ達成。残り 3 件 (auto-restart 1s / shutdown 3s / clippy deny) は **P5a follow-up** (worker thread が新設されるまで該当機能が無いため検証不能)。

既存 sync `#[napi]` exports (`compute_change_fraction` / `dhash_from_raw` / `hamming_distance`) の `napi_safe_call` ラップは **P5a に持ち越し** (`#[napi_safe]` proc_macro 化と同時に一括移行、scope 制御)。`scripts/check-napi-safe.mjs` の検査範囲を P5a 着手時に `src/` 全体に拡大する。
