...
| `dirty_rects_aggregate` | DXGI dirty/move rect event | 直近 dirty 矩形集約 | `screenshot` の差分判定 | 🚧 **D2-C で実装中** (sub-plan landed 本 D2-C plan PR、impl PR 本 plan merge 後翌 PR — `docs/adr-008-d2-c-plan.md`) |
...
| **D2: 主要 view 4** (D2-C0 ゲート PR #99 で `dirty_rects_aggregate` 一旦 carry-over → P5c-2 PR #102 で trigger 完了 → 本 D2-C plan PR で復帰) | `current_focused_element` (D1 完了) + `dirty_rects_aggregate` (D2-C、本 plan PR で sub-plan land) + `semantic_event_stream` skeleton (FocusMoved seed) + `predicted_post_state` subgraph (D5 で本格化) を declarative に。`semantic_event_stream` の他 variant (`WindowChanged` / `ScrollSettled` / `ModalAppeared`) のみ §3.bis ledger L2-L4 で carry-over | `desktop_state` の focus path を view 経由化 (bit-equal 回帰 0)、modal/attention は D4 carry-over (`docs/adr-008-d2-plan.md` §1.3 / §3 PR 表 / §3.bis carry-over ledger) |
