...
### 3.2 主要 view (D2 で実装する 4 view、D2-C0 ゲート carry-over 復帰)

> **D2-C0 ゲート (2026-05-01、PR #99)** で `dirty_rects_aggregate` は ADR-007 P5c-2 prerequisite として一旦 carry-over 化したが、**P5c-2 PR #102 merged 2026-05-01 (`c535fc2`) で trigger 完了 → §3.bis ledger L1 strikethrough**、`docs/adr-008-d2-c-plan.md` で復帰 PR sub-plan land。`semantic_event_stream` は引き続き `FocusMoved` 単独 variant を **skeleton seed** として land、`WindowChanged` / `ScrollSettled` / `ModalAppeared` variant 拡張は P5c-3/4 + UIA dialog/structure event 配線後の D2 後継 phase で carry-over (§3.bis ledger L2-L4)。判断・復帰レールは `docs/adr-008-d2-plan.md` §D2-C0 + §3.bis carry-over ledger + §10 OQ #1 / #3 / #17。本書 D2-G phase で本表の最終整合 (status 列の [x] flip) を実施。
...
| `dirty_rects_aggregate` | DXGI dirty/move rect event | 直近 dirty 矩形集約 | `screenshot` の差分判定 | 🚧 **D2-C で実装中 (walking skeleton S2 contract spike、count-only)** (sub-plan landed 本 D2-C plan PR、impl PR 本 plan merge 後翌 PR — `docs/adr-008-d2-c-plan.md`、完成形 (`rects` / `total_area` / 100ms window) は trunk 完了後 expansion) |
| `semantic_event_stream` (FocusMoved seed) | UIA focus event (本 D2 では `UiaFocusChanged` のみ) | "focus moved" semantic、後続で modal/window/scroll 系拡張 | `desktop_state.attention` の前段 | 🚧 **D2-D で skeleton + FocusMoved seed**、他 variant は §3.bis ledger L2-L4 で carry-over |
...
| **D2: 主要 view 4** (D2-C0 ゲート PR #99 で `dirty_rects_aggregate` 一旦 carry-over → P5c-2 PR #102 で trigger 完了 → 本 D2-C plan PR で walking skeleton S2 contract spike として復帰) | `current_focused_element` (D1 完了) + `dirty_rects_aggregate` (D2-C、**walking skeleton S2 として count-only 簡易版** で本 plan PR sub-plan land、完成形は trunk 完了後 expansion) + `semantic_event_stream` skeleton (FocusMoved seed) + `predicted_post_state` subgraph (D5 で本格化) を declarative に。`semantic_event_stream` の他 variant (`WindowChanged` / `ScrollSettled` / `ModalAppeared`) のみ §3.bis ledger L2-L4 で carry-over | `desktop_state` の focus path を view 経由化 (bit-equal 回帰 0)、modal/attention は D4 carry-over (`docs/adr-008-d2-plan.md` §1.3 / §3 PR 表 / §3.bis carry-over ledger、`docs/walking-skeleton-trunk-selection.md` Proposed v0.3 §4 trunk 全体図) |
