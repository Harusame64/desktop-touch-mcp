...
| `browser_open` | Chrome/Edge に CDP 接続してタブ一覧取得 |
...
browser_open()                        → タブ一覧 + tabId 取得
browser_locate("#submit")          → CSS セレクター → 物理ピクセル座標
browser_click("#submit")         → 検索 + クリックを 1 ステップで
...
- `entity_outside_viewport` → `scroll` / `scroll_to_element` してから `desktop_discover` を再実行
- `executor_failed` → `click_element` / `mouse_click` / `browser_click` にフォールバック

`desktop_discover` が warnings（`visual_provider_unavailable`、`visual_provider_warming`、`cdp_provider_failed` 等）を返した場合も、V1 ツール（`screenshot`、`click_element`、`get_ui_elements`、`terminal_send` など）がエスケープハッチとして使えます。
