...
> **ステータス: v0.17 からデフォルト ON。** `desktop_see` / `desktop_touch` はインストール直後から使えます。
...
1. `browser_click_element(selector)` — Chrome / Edge（CDP 経由、リペイントで座標がずれない）
2. `desktop_touch(lease)` — ネイティブウィンドウ・ダイアログ・視覚のみの対象（`desktop_see` 後に使用）
3. `click_element(name | automationId)` — `desktop_touch` が `ok:false` の場合の UIA フォールバック
4. `mouse_click(x, y)` — 最終手段（`dotByDot` スクリーンショットの `origin`・`scale` が必要）

### V2 を無効にする（キルスイッチ）

`desktop_see` / `desktop_touch` をツールカタログから外したい場合は、disable フラグを追加して再起動します:
...
`desktop_touch` が `ok: false` を返した場合は `reason` を確認し、ツール説明のリカバリヒントに従ってください。よくあるパターン:

- `lease_expired` / `*_mismatch` / `entity_not_found` → `desktop_see` を再実行してリースを更新
- `modal_blocking` → `click_element` でモーダルを閉じてから retry
- `entity_outside_viewport` → `scroll` / `scroll_to_element` してから `desktop_see` を再実行
- `executor_failed` → `click_element` / `mouse_click` / `browser_click_element` にフォールバック

`desktop_see` が warnings（`visual_provider_unavailable`、`visual_provider_warming`、`cdp_provider_failed` 等）を返した場合も、V1 ツール（`screenshot`、`click_element`、`get_ui_elements`、`terminal_send` など）がエスケープハッチとして使えます。
