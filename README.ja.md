...
- **ツール呼び出しの実行中**: サーバは終了します (exit code 1) — 暴走オートメーションへのブレーキ。バルーン通知と診断ログ（カーソル座標付き）が停止理由を記録します。
- **アイドル時**: サーバは生存したまま、カーソルがコーナーを離れるまで新しいツール呼び出しを拒否します。背景の credential 自動入力 (`key_locker`) も**ダイアログが開く前に**中止されます — コーナーに置いている間、credential 入力ダイアログは表示されず、credential も入力されません。
