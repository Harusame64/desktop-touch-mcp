...
**表示スケールが 100% 以外の場合**、`screenshot(displayId=…)` / `screenshot(region=…)` が返す画像サイズは、従来の縮小されたサイズではなくモニタの実ピクセルサイズになります。例えば 150% スケールの 3840×2160 モニタは、以前は 2560×1440 で返っていたものが 3840×2160 で返ります。`screenshot(windowTitle=…)` は元からこの実ピクセルサイズなので、両者のスケールがようやく一致します。画面座標は変わりません — `screenshot(detail='meta')` が返す座標をそのまま渡し、そのままクリックできます。以前のバージョンで得たサイズや切り出し座標をハードコードしている場合は、古い数値を使い回さずスクリーンショットを取り直してください。
