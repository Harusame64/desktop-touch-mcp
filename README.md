...
- `keyboard_target_unsafe` → nothing was typed: the characters would have gone to a different control or window, or to a read-only control (`if_unexpected.detail` says which). Put the focus on the field you named (by title: `desktop_act` with `action='click'`; by handle: re-discover by title first), then type again — not through a foreground `keyboard` type
- `executor_failed` → fall back to `click_element` / `mouse_click` / `browser_click`
