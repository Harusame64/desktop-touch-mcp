...
export const STUB_TOOL_CATALOG: StubToolCatalogEntry[] = [
  ...
  },
  {
    ...
    "description": "Deregister a lens by lensId. Removes it from the dependency graph and cleans up sensor subscriptions. Call when you no longer need perception tracking for a window/tab. Returns {ok, removed, lensId}.",
    "inputSchema": {
      "type": "object",
      "properties": {
        "lensId": {
  ...
  },
  {
    ...
    "description": "Force-refresh Win32/CDP fluents for a lens and return a full perception envelope. Use after an action that may have changed window state, or when post.perception.attention is 'dirty' or 'stale'. Returns {ok, seq, attention, guards, latest, changed}.",
    "inputSchema": {
      "type": "object",
      "properties": {
        "lensId": {
