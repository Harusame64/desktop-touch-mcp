...
#[cfg(windows)]
pub(crate) mod dwm;
// ADR-027: Windows.Graphics.Capture (WGC) capture layer — real pixels for
// GPU-composited / occluded windows that PrintWindow returns black for.
// Dedicated worker thread (device reuse) + free-threaded frame pool. See
// `src/win32/wgc.rs`.
#[cfg(windows)]
pub(crate) mod wgc;
