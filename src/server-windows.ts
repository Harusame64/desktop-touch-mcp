  ...
  const s = new McpServer(
    { name: "desktop-touch", version: SERVER_VERSION },
    {
      instructions: [
        ...
        "  aim_occluded → the coordinates are right, but Windows' own hit test says a press there would land in another window drawn over that point; nothing was done. An overlay that lets presses pass through is not counted. Bring the intended window forward (focus_window), or act with click_element, which does not use coordinates. Re-calling desktop_discover by itself changes nothing — the entity's rect is correct;",
