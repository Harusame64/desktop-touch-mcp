  ...
  const s = new McpServer(
    { name: "desktop-touch", version: SERVER_VERSION },
    {
      instructions: [
        ...
        "  keyboard_target_unsafe → the background write would not have reached the field this act named — the focus is on a different control or in a different window, or the receiving control does not take typed text — so nothing was typed. if_unexpected.detail names which. Put the focus on the field you named, then type again: on a window named by title, desktop_act(action='click') on the same entity does it; on a window named by handle no route here focuses a text field yet, so re-call desktop_discover by the window's title and click it from there; do NOT type through the foreground instead, whatever holds the focus would take the characters. A type that answers ok:true with landing {confirmed:false, why} was sent in the background but not confirmed to have reached the field named: read the field back before relying on it;",
