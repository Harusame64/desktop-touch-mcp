  ...
  const s = new McpServer(
    { name: "desktop-touch", version: SERVER_VERSION },
    {
      instructions: [
        ...
        "  aim_route_failed → the route to the window this act named by handle failed (UIA for a click; UIA setValue and the background write for type/setValue), and the act was NOT finished as a coordinate press; nothing was clicked or typed. if_unexpected.detail names the failure when this server recognises it — not found, no pattern for this action, disabled, or read-only. When it says not found or names none: re-call desktop_discover, or try click_element(name=…) which re-resolves the element; for the others, trying the same element again gives the same answer until its state changes. Do NOT press the entity's rect — a coordinate is aimed at no window;",
