  ...
  const s = new McpServer(
    { name: "desktop-touch", version: SERVER_VERSION },
    {
      instructions: [
        ...
        "  keyboard_target_unsafe → the background write would not have reached the field this act named — the focus is on a different control or in a different window, or the receiving control does not take typed text — so nothing was typed. Put the focus on the field you named, then type again — if_unexpected.detail names the ground and the way back for the road this act took: on a window named by title, desktop_act(action='click') on the same entity does it; on a window named by handle no route here focuses a text field yet, so re-call desktop_discover by the window's title and click it from there (a common dialog's title resolves to a handle as well, so that road does not open there). For other_window, bring the field's window forward first (focus_window) — it comes forward with the focus it last had, and the window holding the focus is usually over the field, which makes a click answer aim_occluded; do NOT type through the foreground instead, whatever holds the focus would take the characters. A type that answers ok:true with landing {confirmed:false, why} was sent in the background but not confirmed to have reached the field named: read the field back before relying on it — but a read-back confirms nothing on its own. `desktop_state` answers about the FOREGROUND window, from a focus row that is sticky and asynchronous, so it can name an element focused before your write, in that window or in another with the same title; and a value can be absent for reasons unrelated to delivery (check hints.focusedElementValueAbsent: 'view_road_has_no_value' means that road carries no values at all, and a value can also be missing with no hint at all). A read-back reports what the field holds NOW and never establishes that your write put it there: a BACKGROUND type inserts at the caret and replaces the selection, exactly as typing does, so its result is not predictable without knowing the caret, which nothing here reports; and a write of empty text sends nothing, so an already-empty field reads back as a match. Use it to see what the field currently holds, not to prove delivery. `diff.value_changed` on the same response is not delivery either: its baseline is your `desktop_discover` snapshot, not the moment of the write, so a change anyone made in between looks identical. NOTHING on this response establishes that the characters arrived. AND NOTHING ELSE HERE DOES EITHER. A retry appends rather than replaces, so writing again can double the text that already arrived; a clear is a write of empty text, which sends nothing and can come back unconfirmed in its own turn; and taking the focus makes the next write confirmable only for a field that has a window of its own, which `landing.why` does not reliably tell you — the same word covers a field that structurally has none and a window that merely had no focused child at that moment. So treat this landing as a REPORT and not a state you can resolve here: the same paragraph says the read may come back value-less, may omit a value the provider never served, and may name a field in another window with the same title, so 'read it and act on what it holds' is not available either;",
        ...
        "- RegionOutsideCapturableBounds → the screenshot region cannot be captured as given. Read the message: it says whether the region is on no monitor at all (stale coordinates — re-run desktop_discover or take a fresh screenshot), or overlaps a monitor but runs past the capturable area (coordinates are current; shrink the region, or use screenshot(windowTitle=…)), or whether this server captures the primary monitor only. In that last case the message also says WHY: an env override typically still leaves screenshot(windowTitle=…) working on every monitor, while a missing capture module usually breaks that route too — move the window onto the primary monitor or reinstall the server. Do not infer it from the cause, though: per-window capture and region capture are separate parts of the module and a build can have one without the other, so the message states outright whether screenshot(windowTitle=…) is available — follow what it says",
        ...
        "screenshot(region=…) and screenshot(displayId=…) take absolute desktop coordinates, and a monitor placed left of or above the primary one has negative ones — pass them as reported by screenshot(detail='meta'), do not clamp them to 0. screenshot() without a region is the PRIMARY monitor only; ask for a display or a region to capture any other.",
        "",
        "## Scroll capture",
