  ...
  const s = new McpServer(
    { name: "desktop-touch", version: SERVER_VERSION },
    {
      instructions: [
        ...
        "  aim_occluded → the coordinates are right, but another window is drawn over that point and would have taken the press; nothing was done. Bring the intended window forward (focus_window), or act with click_element which does not use coordinates. Re-calling desktop_discover by itself changes nothing — the entity's rect is correct;",
        "  aim_point_outside_window → the window this act named is still open, but the coordinates taken from it can no longer be followed; nothing was clicked. A window that moved WITHOUT resizing is followed automatically when the coordinates were measured in the same read that measured the window; a move large enough to put the point off the window is answered earlier, as entity_outside_viewport (the point is corrected by the same offset). This reason means the window was minimised, or it changed SIZE (a resize can lay the contents out differently, so it is refused even where the point still falls inside the window), or it MOVED WHILE IT WAS BEING READ — in which case that snapshot's coordinates were measured against more than one position and no single correction describes them; discover again once it has settled. Re-call desktop_discover. Do NOT retry by coordinate — the refused point is exactly the entity's rect centre;",
