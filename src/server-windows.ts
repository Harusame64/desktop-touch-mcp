...
import {
  landingAdvice,
  LANDING_ADVICE_SERVER_INSTRUCTIONS,
  ...
  const s = new McpServer(
    { name: "desktop-touch", version: SERVER_VERSION },
    {
      instructions: [
        ...
        "  aim_occluded → the coordinates are right, but another window drawn over that point would take the press; nothing was done. Windows' own hit test decides this and does not count an overlay that lets presses through there; a build whose native addon cannot ask Windows judges from the window list, where any window on top counts unless it has both WS_EX_TRANSPARENT and WS_EX_LAYERED. Bring the intended window forward (focus_window), or act with click_element, which does not use coordinates. Re-calling desktop_discover by itself changes nothing — the entity's rect is correct;",
        ...
        "- MCP error -32602: Input validation error: Invalid arguments for tool … → the arguments did not match the tool's schema, so the request was refused before the tool ran and nothing was done. This answer is plain text marked isError, not the JSON envelope: it has no ok, code or suggest, and include and the other envelope options do not change it. The text says what did not match; correct the arguments and call again",
        "",
