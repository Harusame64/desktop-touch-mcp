  ...
  const s = new McpServer(
    { name: "desktop-touch", version: SERVER_VERSION },
    {
      instructions: [
        ...
        "  action_not_offered → the target does not offer the action this act named, and NOTHING WAS DONE — no road was taken, so this is not a failed executor. Ask for the action you mean: desktop_act(action='click') or action='invoke' presses it. The entity's affordances in the desktop_discover response say which actions it offers, and action='auto' picks one of them. No provider here advertises 'select', so a select on any target is this refusal — reach a list or combo box by clicking the item you want;",
