...
export function registerWaitUntilTool(server: McpServer): void {
  server.tool(
    ...
    buildDesc({
      ...
      caveats: "terminal_output_contains, element_matches, and url_matches require a browser CDP connection (open --remote-debugging-port=9222 first). element_appears/value_changes spawn a UIA process per poll — interval clamped to 500ms minimum. On failure the response is {ok:false, code:'WaitTimeout', error, suggest:[...]}; the suggest[] array typically lists: increase timeoutMs, verify the target shape (windowTitle / pattern / scope), and observe intermediate state via screenshot(detail='meta') or desktop_state to debug the polling condition before retry.",
      examples: [
