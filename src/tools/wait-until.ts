...
export function registerWaitUntilTool(server: McpServer): void {
  server.tool(
    ...
    buildDesc({
      ...
      caveats: "terminal_output_contains, element_matches, and url_matches require a browser CDP connection (open --remote-debugging-port=9222 first). element_appears/value_changes spawn a UIA process per poll — interval clamped to 500ms minimum. On elapsed-timeout the response is {ok:false, code:'WaitTimeout', error, suggest:[...]}; the suggest[] array typically lists: increase timeoutMs, verify the target shape (windowTitle / pattern / scope), and observe intermediate state via screenshot(detail='meta') or desktop_state. Non-timeout failures may also surface — pre-poll validation as InvalidArgs and CDP probe errors as code:'BrowserNotConnected' (re-attach via browser_open) — so callers should branch on code rather than assume WaitTimeout.",
      examples: [
