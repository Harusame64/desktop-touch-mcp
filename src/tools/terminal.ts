  ...
  server.registerTool(
    "terminal",
    {
      description: buildDesc({
        ...
        caveats: "Do not screenshot the terminal — terminal(action='read') is cheaper and structured. action='run' supports completion reasons: quiet | pattern_matched | timeout | window_closed | window_not_found | send_failed (send rejected on a live window — see warnings for the underlying error code). When outputIntegrity:'baseline_lost' is returned, output is forced to '' and readError.code='BaselineMarkerLost' is set: rerun with until:{mode:'pattern',...} or longer timeoutMs. Default quiet mode quietMs=1500 (issue #196 raised from 800 to reduce premature completion on startup gaps); long silent intervals during the command body still require pattern mode. preferClipboard=true (send default) overwrites user clipboard. Hidden-input prompts (password/sudo) auto-emit hints.verifyDelivery={status:'unverifiable',reason:'hidden_input_prompt'} — credentials must use method:'foreground'. action='read' typed errors: TerminalWindowNotFound, TerminalTextPatternUnavailable (force source:'ocr'), TerminalMarkerStale (omit sinceMarker).",
        examples: [
