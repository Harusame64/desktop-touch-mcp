...
export const scrollSchema = z.discriminatedUnion("action", [
  ...
  z.object({
    ...
    amount: z.coerce.number().int().positive().default(3).describe("Number of scroll notches (default 3). One notch = one physical wheel detent on every dispatch path. Typical distance per notch: ≈1 text line in UIA-capable apps (Notepad, Explorer, WPF), ≈100 px in browsers and WebView-based apps (Chrome, Electron, Tauri). amount:3 (default) is a small nudge; amount:10 is roughly half a viewport. Exact distance depends on the app and the OS wheel-speed setting."),
  ...
  server.registerTool(
    "scroll",
    {
      description: buildDesc({
        ...
        details: "action='raw': send raw mouse-wheel notches at (x,y) or current cursor, optional window focus. Scroll scale — one amount unit is one wheel notch on every dispatch path: ≈1 text line per notch in UIA-capable apps (Notepad, Explorer, WPF), ≈100 px per notch in browsers and WebView-based apps (Chrome, Electron, Tauri). amount:3 (default) is a small nudge, amount:10 is roughly half a viewport. action='to_element': scroll a named element into viewport (UIA or CDP). action='smart': handles nested scroll layers, virtualised lists, sticky-header occlusion. action='capture': stitches full-page images (caps at ~700KB raw); sizeReduced=true means downscaled. action='read': scrolls page-by-page, OCRs each viewport, deduplicates overlapping lines, returns stitched text; language auto-detected from OS locale if omitted.",
