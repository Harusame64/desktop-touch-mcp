...
export const scrollSchema = z.discriminatedUnion("action", [
  ...
  z.object({
    ...
    amount: z.coerce.number().int().positive().default(3).describe("Number of scroll notches (default 3). Visual change per notch: UIA-capable apps (Notepad, Explorer, WPF) ≈ 10% of the visible viewport; legacy apps (SendInput path) ≈ 3 text lines. Practical guide — amount:1 ≈ small nudge, amount:3 (default) ≈ ⅓ viewport, amount:10 ≈ one full page."),
  ...
  server.registerTool(
    "scroll",
    {
      description: buildDesc({
        ...
        details: "action='raw': send raw mouse-wheel notches at (x,y) or current cursor, optional window focus. amount=notches: 1 notch ≈ 10% of visible viewport (UIA path) or ≈3 text lines (legacy SendInput); amount:3 (default) ≈ ⅓ page, amount:10 ≈ one full page. action='to_element': scroll a named element into viewport (UIA or CDP). action='smart': handles nested scroll layers, virtualised lists, sticky-header occlusion. action='capture': stitches full-page images (caps at ~700KB raw); sizeReduced=true means downscaled. action='read': scrolls page-by-page, OCRs each viewport, deduplicates overlapping lines, returns stitched text; language auto-detected from OS locale if omitted.",
