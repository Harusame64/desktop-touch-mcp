 ...
 * A masked element's value is not read, and neither an entry's own text — a <textarea>'s text node
 * is its initial value, an editor's is what was typed — nor a masked element's text is offered as
 * its name, so `buildElementInfoFromCdp` falls back to the name attribute, the id or the tag. MEASURED 2026-09-11 win2 (internal `dev/cdp-password/`): a password
...
${ELEMENT_NAME_JS}
  ...
  return {tag:el.tagName,id:el.id,name:el.name||el.getAttribute('name')||'',
          ...
          text:(field||__isMasked(el))?'':(el.innerText||el.textContent||'').slice(0,60)};
})()`;
