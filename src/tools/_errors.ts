...
const SUGGESTS: Record<string, string[]> = {
  ...
  KeyboardTargetUnsafe: [
    ...
    "other_control / other_window: put the focus on the field you named, then type again — the background write goes to whatever holds the focus. When this act named its window by title, desktop_act action='click' on the same entity does that: a text field has no UIA invoke, so it is clicked at its position, checked against the window it was captured in. When it named its window by handle, no route here moves the focus to a text field yet (that click answers aim_route_failed): re-run desktop_discover by the window's title and click the field from there. For other_window, focus_window on the field's window can be enough, since a window comes forward with the focus it last had.",
