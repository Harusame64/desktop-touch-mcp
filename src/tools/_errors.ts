...
const SUGGESTS: Record<string, string[]> = {
  ...
  AimPointOutsideWindow: [
    ...
    "A window that moved WITHOUT resizing does not reach here — the point is carried with it by the same offset, when the coordinates were measured in the same read that measured the window. This reason means the window was minimised; or it changed SIZE, and a resize may have reflowed the contents, so it is refused even where the point still falls inside; or it moved WHILE it was being discovered, in which case that snapshot's coordinates were measured against more than one position and no correction can describe them — discover again once the window has settled. A move large enough to put the point off the window is usually answered earlier, as entity_outside_viewport — that check passes uia / cdp / terminal entities without looking, so it is not a second guarantee.",
