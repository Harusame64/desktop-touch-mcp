...
const SUGGESTS: Record<string, string[]> = {
  ...
  AimRouteFailed: [
    ...
    "if_unexpected.detail says which failure it was when the backend's answer is one this server recognises — the element was not found, it does not support this action through UI Automation, or it is disabled — and says nothing more when it is not: on this road an unrecognised answer can be a shell rejection carrying the command that produced it. Not found: re-discover. No pattern for this action (a custom-drawn button, a canvas): act on a different affordance or reach it by keyboard navigation. Disabled: find what enables it before trying again.",
  ],
