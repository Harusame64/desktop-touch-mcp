...
const SUGGESTS: Record<string, string[]> = {
  ...
  AimRouteFailed: [
    ...
    "When the detail names no failure, or says the element was not found: re-run desktop_discover — this act named its window by handle, the attempt on it failed, and the entity may have changed name, moved in the tree, or gone. For the other failures it names, re-discovering alone finds the same element giving the same answer until its state changes.",
