...
const SUGGESTS: Record<string, string[]> = {
  ...
  AimRouteFailed: [
    "if_unexpected.detail says which failure it was when the backend's answer is one this server recognises — the element was not found, it does not support this action through UI Automation, it is disabled, or it refused a write as not enabled — and says nothing more when it is not: on this road an unrecognised answer can be a shell rejection carrying the command that produced it. Not found: re-discover. No pattern for this action (a custom-drawn button, a canvas): act on a different affordance or reach it by keyboard navigation. Disabled: find what enables it before trying again. Refused a write as not enabled: a read-only field answers the same way, so check whether it takes typed text at all before looking for what enables it.",
    "When the detail names no failure, or says the element was not found: re-run desktop_discover — this act named its window by handle, the attempt on it failed, and the entity may have changed name, moved in the tree, or gone. For the other failures it names, a fresh discover finds the same element giving the same answer.",
