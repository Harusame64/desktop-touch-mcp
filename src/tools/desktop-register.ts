  ...
  if (!result.ok && result.reason === "aim_point_outside_window") {
    const failure = toFailureEnvelope(
      Err(new AimPointOutsideWindowError(
        ...
        "Re-run desktop_discover; the window was minimised, was resized so its contents may have moved independently of its origin, was moving while it was being read, or the coordinates came from a lane whose measurement moment cannot be established — a stored visual snapshot may have been captured while the window was somewhere else, and no correction can describe it"
      )),
      { optIn: false },
    );
    return {
...
export function registerDesktopTools(server: McpServer): void {
  ...
  server.tool(
    ...
    [
      ...
      "  aim_point_outside_window → the window is still open but its coordinates can no longer be followed (minimised; resized, so the contents may have reflowed — refused even where the point still falls inside; moved while it was being read, so that snapshot has no single origin; or measured by a lane whose moment cannot be established, such as a stored visual snapshot); nothing was clicked. A window that moved WITHOUT resizing is followed automatically when the coordinates were measured in the same read that measured the window; a move large enough to put the point off the window is answered earlier, as entity_outside_viewport. Re-call desktop_discover — do NOT retry by coordinate;",
