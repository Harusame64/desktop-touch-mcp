  ...
  if (!result.ok && result.reason === "aim_occluded") {
    const failure = toFailureEnvelope(
      Err(new AimOccludedError(
        "AimOccluded: Windows' own hit test says a press at this point would land in another window drawn over it — nothing was done. An overlay that lets presses pass through is not counted as in the way. " +
...
export function registerDesktopTools(server: McpServer): void {
  ...
  server.tool(
    ...
    [
      ...
      "  aim_occluded → Windows' own hit test says the press would land in another window drawn over the point, so nothing was done (an overlay that lets presses pass through is not counted). Bring the intended window forward, or use V1 click_element, which does not use coordinates — re-calling desktop_discover alone does not help, the coordinates are already right;",
