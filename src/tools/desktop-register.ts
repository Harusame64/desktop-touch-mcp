...
export function registerDesktopTools(server: McpServer): void {
  ...
  server.tool(
    ...
    [
      ...
      "  lease_expired (after grace check) / lease_generation_mismatch / lease_digest_mismatch / entity_not_found → re-call desktop_discover;",
      "  Note: an *expired* lease succeeds when the live entity's digest still matches — response.warnings will include 'lease_was_expired_but_entity_unchanged' in that case (proceed normally).",
