...
export function registerDesktopTools(server: McpServer): void {
  ...
  server.tool(
    ...
    [
      ...
      "A type/setValue that answers ok=true with 'landing' {confirmed:false, why} was sent by the background keyboard write but not confirmed to have reached the field named: read the field back before relying on it — with desktop_state, and check hints.focusedElementValueAbsent before believing an empty answer. A read-back on a window whose title does not change is answered from the perception view, which carries no values at all, so the field comes back with no value even though the write landed; 'view_road_has_no_value' is that case and is not evidence the write failed.",
