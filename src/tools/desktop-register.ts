...
export function registerDesktopTools(server: McpServer): void {
  ...
  server.tool(
    ...
    [
      ...
      "  aim_point_outside_window → the window is still open but its coordinates can no longer be followed (minimised; resized, so the contents may have reflowed — refused even where the point still falls inside; or moved while it was being read, so that snapshot has no single origin); nothing was clicked. A window that moved WITHOUT resizing is followed automatically when the coordinates were measured in the same read that measured the window; a move large enough to put the point off the window is answered earlier, as entity_outside_viewport. Re-call desktop_discover — do NOT retry by coordinate;",
