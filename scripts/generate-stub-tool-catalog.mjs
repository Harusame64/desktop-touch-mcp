...
const TOOL_FILES = [
  ...
  'excel.ts',
  // ADR-026 Phase 3: screenshot disk-cache observe + reclaim
  'screenshot-query.ts',
  'screenshot-gc.ts',
  // ADR-014 R3: key locker management tool
  'key-locker-tool.ts',
];
...
// ADR-014 R3 added key_locker → 30 stub catalog entries. (Public surface =
// 30 stub + 2 dynamic v2 = 32 tools.) The bound is kept tight so dropping a
// tool fails the build.
if (tools.length < 30) {
  throw new Error(`Expected at least 30 tools, generated ${tools.length}`);
}
